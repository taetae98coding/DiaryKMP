...
class DeleteMemoUseCaseTest :
    BehaviorSpec({
        Given("로그인한 계정과 현재 시각이 준비되어 있다") {
            ...
            When("메모를 삭제한다") {
                Then("TC-MEMO-HOME-DOMAIN-002 TC-MEMO-HOME-DOMAIN-005 TC-MEMO-FINISHED-LIST-DOMAIN-002 TC-MEMO-FINISHED-LIST-DOMAIN-005 TC-TAG-MEMO-FINISHED-LIST-DOMAIN-002 현재 계정의 메모를 삭제 상태와 동작 시점 수정 시각으로 갱신한다") {
                    ...
                    coVerify(exactly = 1) {
                        accountMemoRepository.updateDeleted(account = account, memoId = memoId, isDeleted = true, updatedAt = now)
                    }
                }
            }
        }
        ...
        Given("메모 삭제와 동기화 요청이 성공하도록 준비되어 있다") {
            val account = fixtureMonkey.giveMeOne<Account.User>()
            val getAccountUseCase = mockk<GetAccountUseCase>()
            every { getAccountUseCase(parameter = Unit) } returns flowOf(Result.success(account))
            val requestSyncUseCase = mockk<RequestSyncUseCase>()
            ...
            When("메모를 삭제한다") {
                Then("메모 갱신 후 동기화를 한 번 요청한다") {
                    ...
                    result.shouldBeSuccess(1)
                    coVerifyOrder {
                        accountMemoRepository.updateDeleted(account = account, memoId = any(), isDeleted = true, updatedAt = any())
