...
class UnselectCalendarFilterTagUseCaseTest :
    BehaviorSpec({
        Given("현재 계정이 준비되어 있다") {
            ...
            coEvery { accountCalendarFilterRepository.delete(account = account, tagId = tagId) } just Runs
            val useCase =
                UnselectCalendarFilterTagUseCase(
                    getAccountUseCase = getAccountUseCase,
                    accountCalendarFilterRepository = accountCalendarFilterRepository,
                )

            When("태그의 캘린더 필터 선택을 해제한다") {
                Then("현재 계정의 캘린더 필터 선택에서 제거한다") {
        ...
        Given("현재 계정 조회에 실패하도록 준비되어 있다") {
            ...
            When("태그의 캘린더 필터 선택을 해제한다") {
                Then("계정 조회 실패를 전달하고 저장소에 해제를 요청하지 않는다") {
                    ...
                    coVerify(exactly = 0) {
                        accountCalendarFilterRepository.delete(account = any(), tagId = any())
                    }
                }
            }
        }
