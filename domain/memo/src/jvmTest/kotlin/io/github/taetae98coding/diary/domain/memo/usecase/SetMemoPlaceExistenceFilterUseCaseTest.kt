...
class SetMemoPlaceExistenceFilterUseCaseTest :
    BehaviorSpec({
        Given("유무 필터 저장소가 준비되어 있다") {
            val memoExistenceFilterRepository = mockk<MemoExistenceFilterRepository>()
            coEvery { memoExistenceFilterRepository.updatePlace(existence = any()) } just Runs
            val useCase = SetMemoPlaceExistenceFilterUseCase(memoExistenceFilterRepository = memoExistenceFilterRepository)

            When("TC-MEMO-HOME-DATA-010 장소 축을 바꾼다") {
                Then("장소 축만 기기에 저장한다") {
                    MemoFilterExistence.entries.forEach { existence ->
                        useCase(parameter = existence).shouldBeSuccess()

                        coVerify(exactly = 1) {
                            memoExistenceFilterRepository.updatePlace(existence = existence)
                        }
                    }
                }
            }
        ...
        Given("유무 필터 저장이 실패하도록 준비되어 있다") {
            ...
            val memoExistenceFilterRepository = mockk<MemoExistenceFilterRepository>()
            coEvery { memoExistenceFilterRepository.updatePlace(existence = any()) } throws failure
            val useCase = SetMemoPlaceExistenceFilterUseCase(memoExistenceFilterRepository = memoExistenceFilterRepository)
