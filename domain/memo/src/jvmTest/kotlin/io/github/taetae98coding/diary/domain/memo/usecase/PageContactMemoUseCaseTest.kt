...
class PageContactMemoUseCaseTest :
    BehaviorSpec({
        Given("현재 계정과 대상 연락처의 메모 페이지가 준비되어 있다") {
            val account = fixtureMonkey.giveMeOne<Account.User>()
            val contactId = fixtureMonkey.giveMeOne<Uuid>()
            val pagingData = PagingData.from(listOf(memo()))
            val getAccountUseCase = mockk<GetAccountUseCase>()
            val repository = mockk<AccountContactMemoRepository>()
            every { getAccountUseCase(parameter = Unit) } returns flowOf(Result.success(account))
            every { repository.page(account = account, contactId = contactId, sort = ListSort.DEFAULT) } returns flowOf(pagingData)
            val useCase = PageContactMemoUseCase(getAccountUseCase = getAccountUseCase, accountContactMemoRepository = repository)

            When("대상 연락처의 메모 페이지를 조회한다") {
                Then("TC-CONTACT-DETAIL-MEMO-DATA-001 현재 계정과 대상 연락처의 메모 페이지를 반환한다") {
                    useCase(parameter = PageContactMemoUseCase.Parameter(contactId = contactId, sort = ListSort.DEFAULT)).test {
                        awaitItem().getOrThrow() shouldBeSameInstanceAs pagingData
                        awaitComplete()
                    }

                    verify(exactly = 1) { repository.page(account = account, contactId = contactId, sort = ListSort.DEFAULT) }
                }
            }
        }
        ...
        Given("현재 계정 조회에 실패하도록 준비되어 있다") {
            ...
            val getAccountUseCase = mockk<GetAccountUseCase>()
            val repository = mockk<AccountContactMemoRepository>(relaxed = true)
            ...
            When("대상 연락처의 메모 페이지를 조회한다") {
                Then("계정 조회 실패를 전달하고 저장소에 페이지를 요청하지 않는다") {
                    ...
                    verify(exactly = 0) { repository.page(account = any(), contactId = any(), sort = any()) }
                }
            }
        }

        Given("대상 연락처의 메모 페이지 조회가 실패하도록 준비되어 있다") {
            val account = fixtureMonkey.giveMeOne<Account.User>()
            val contactId = fixtureMonkey.giveMeOne<Uuid>()
            val failure = IllegalStateException(fixtureMonkey.giveMeOne<String>())
            val getAccountUseCase = mockk<GetAccountUseCase>()
            val repository = mockk<AccountContactMemoRepository>()
            every { getAccountUseCase(parameter = Unit) } returns flowOf(Result.success(account))
            every { repository.page(account = account, contactId = contactId, sort = ListSort.DEFAULT) } returns flow { throw failure }
            val useCase = PageContactMemoUseCase(getAccountUseCase = getAccountUseCase, accountContactMemoRepository = repository)
        ...
        Given("현재 계정이 바뀌고 이전 계정의 페이지도 이후에 변경되는 상황이 준비되어 있다") {
            ...
            every { repository.page(account = account, contactId = contactId, sort = ListSort.DEFAULT) } returns accountPagingFlow
            every { repository.page(account = otherAccount, contactId = contactId, sort = ListSort.DEFAULT) } returns otherAccountPagingFlow
            val useCase = PageContactMemoUseCase(getAccountUseCase = getAccountUseCase, accountContactMemoRepository = repository)
            ...
            When("계정이 바뀐 뒤 이전 계정과 현재 계정의 페이지가 차례로 바뀐다") {
                Then("이전 계정 관찰을 중단하고 현재 계정의 페이지 변경만 전달한다") {
                    ...
                    verify(exactly = 1) { repository.page(account = account, contactId = contactId, sort = ListSort.DEFAULT) }
                    verify(exactly = 1) { repository.page(account = otherAccount, contactId = contactId, sort = ListSort.DEFAULT) }
                }
            }
        }
    })
