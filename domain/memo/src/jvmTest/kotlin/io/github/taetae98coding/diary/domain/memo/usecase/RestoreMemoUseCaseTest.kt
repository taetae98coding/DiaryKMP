...
class RestoreMemoUseCaseTest :
    BehaviorSpec({
        Given("로그인한 계정과 현재 시각이 준비되어 있다") {
            ...
            When("삭제를 실행 취소한다") {
                Then("TC-MEMO-HOME-DOMAIN-004 TC-MEMO-HOME-DOMAIN-005 TC-MEMO-FINISHED-LIST-DOMAIN-004 TC-MEMO-FINISHED-LIST-DOMAIN-005 TC-TAG-MEMO-FINISHED-LIST-DOMAIN-002 현재 계정의 메모를 미삭제 상태와 동작 시점 수정 시각으로 되돌린다") {
                    ...
                    coVerify(exactly = 1) {
                        accountMemoRepository.updateDeleted(account = account, memoId = memoId, isDeleted = false, updatedAt = now)
                    }
                }
            }
        }
        ...
        Given("계정 조회가 실패하도록 준비되어 있다") {
            ...
            When("삭제 복구를 실행한다") {
                Then("계정 조회 실패를 전달하고 메모 상태를 변경하지 않는다") {
                    ...
                    coVerify(exactly = 0) {
                        accountMemoRepository.updateDeleted(account = any(), memoId = any(), isDeleted = any(), updatedAt = any())
                    }
        ...
        Given("삭제 실행 취소와 동기화 요청이 성공하도록 준비되어 있다") {
            val account = fixtureMonkey.giveMeOne<Account.User>()
            val getAccountUseCase = mockk<GetAccountUseCase>()
            every { getAccountUseCase(parameter = Unit) } returns flowOf(Result.success(account))
            val requestSyncUseCase = mockk<RequestSyncUseCase>()
            ...
            When("삭제를 실행 취소한다") {
                Then("메모 갱신 후 동기화를 한 번 요청한다") {
                    ...
                    result.shouldBeSuccess(1)
                    coVerifyOrder {
                        accountMemoRepository.updateDeleted(account = account, memoId = any(), isDeleted = false, updatedAt = any())
