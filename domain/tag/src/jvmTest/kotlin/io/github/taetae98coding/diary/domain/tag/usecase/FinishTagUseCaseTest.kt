...
class FinishTagUseCaseTest :
    BehaviorSpec({
        Given("로그인한 계정과 현재 시각이 준비되어 있다") {
            ...
            When("태그를 완료한다") {
                Then("TC-TAG-DETAIL-DATA-002 현재 계정의 태그를 완료 상태와 동작 시점 수정 시각으로 갱신한다") {
                    ...
                    coVerify(exactly = 1) {
                        accountTagRepository.updateFinished(account = account, tagId = tagId, isFinished = true, updatedAt = now)
                    }
                }
            }
        }
        ...
        Given("태그 완료와 동기화 요청이 성공하도록 준비되어 있다") {
            val account = fixtureMonkey.giveMeOne<Account.User>()
            val getAccountUseCase = mockk<GetAccountUseCase>()
            every { getAccountUseCase(parameter = Unit) } returns flowOf(Result.success(account))
            val requestSyncUseCase = mockk<RequestSyncUseCase>()
            ...
            When("태그를 완료한다") {
                Then("태그 갱신 후 동기화를 한 번 요청한다") {
                    ...
                    result.shouldBeSuccess(1)
                    coVerifyOrder {
                        accountTagRepository.updateFinished(account = account, tagId = any(), isFinished = true, updatedAt = any())
