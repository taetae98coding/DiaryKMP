...
class RestoreTagUseCaseTest :
    BehaviorSpec({
        Given("현재 계정과 삭제된 태그가 준비되어 있다") {
            ...
            coEvery {
                accountTagRepository.updateDeleted(
                    account = account,
                    tagId = id,
            ...
            When("태그의 삭제를 실행 취소한다") {
                Then("TC-TAG-HOME-DOMAIN-017 삭제 여부를 미삭제로 되돌리고 수정 시각을 갱신한다") {
        ...
        Given("게스트 계정이 준비되어 있다") {
            ...
            val accountTagRepository = mockk<AccountTagRepository>()
            coEvery {
                accountTagRepository.updateDeleted(account = account, tagId = id, isDeleted = any(), updatedAt = any())
            } returns 1
            val useCase =
                RestoreTagUseCase(
                    getAccountUseCase = getAccountUseCase,
            ...
            When("태그의 삭제를 실행 취소한다") {
                Then("TC-TAG-HOME-DATA-015 게스트 계정 기준으로 기기에만 실행 취소를 반영한다") {
                    useCase(parameter = id).shouldBeSuccess(1)

                    coVerify(exactly = 1) {
                        accountTagRepository.updateDeleted(account = account, tagId = id, isDeleted = false, updatedAt = any())
                    }
                }
            }
        }
        ...
        Given("계정 조회가 실패하도록 준비되어 있다") {
            ...
            When("태그의 삭제를 실행 취소한다") {
                Then("TC-TAG-HOME-DOMAIN-018 실행 취소가 성공으로 다뤄지지 않고 태그의 삭제 여부를 바꾸지 않는다") {
        ...
        Given("실행 취소 저장이 실패하도록 준비되어 있다") {
            ...
            When("태그의 삭제를 실행 취소한다") {
                Then("TC-TAG-HOME-DATA-016 실패를 그대로 전달하고 동기화를 요청하지 않는다") {
                    useCase(parameter = fixtureMonkey.giveMeOne<Uuid>())
                        .shouldBeFailure()
                        .shouldBeSameInstanceAs(throwable)
