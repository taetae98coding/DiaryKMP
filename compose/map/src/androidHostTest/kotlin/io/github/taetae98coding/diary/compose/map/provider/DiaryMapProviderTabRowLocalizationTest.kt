...
import io.github.taetae98coding.diary.compose.map.provider.DiaryMapProviderTestFixture.KOREAN_GOOGLE_LABEL
import io.github.taetae98coding.diary.compose.map.provider.DiaryMapProviderTestFixture.KOREAN_NAVER_LABEL
import io.github.taetae98coding.diary.compose.map.provider.DiaryMapProviderTestFixture.KOREAN_PROVIDER_CONTENT_DESCRIPTION
...
@Config(sdk = [36])
class DiaryMapProviderTabRowLocalizationTest {
    @get:Rule
    val composeRule = createComposeRule()

    @Test
    @Config(qualifiers = "ko")
    fun `TC-DIARY-MAP-FEATURE-020 겹치지 않는 배치의 한국어 환경 전환 컨트롤 문구를 표시한다`() {
        setDiaryMapProviderTabRow()
    ...
    @Test
    fun `TC-DIARY-MAP-FEATURE-020 겹치지 않는 배치의 기본 환경 전환 컨트롤 문구를 표시한다`() {
        setDiaryMapProviderTabRow()
    ...
    private fun setDiaryMapProviderTabRow() {
        composeRule.setContent {
            DiaryTheme {
                DiaryMapProviderTabRow(state = rememberDiaryMapState())
            }
        }
    }
}
