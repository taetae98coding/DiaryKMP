...
`Result.retry()`와 `BackoffPolicy`는 쓰지 않는다. 스펙([데이터 동기화](../../docs/spec/client/data-sync.md)의 `동기화 계기`)은 같은 계기 안에서 다시 시도하지 않고 다음 계기에 맡기도록 정했다. 재시도를 플랫폼 실행 수단에 두면 이 규칙이 플랫폼마다 달라진다.
