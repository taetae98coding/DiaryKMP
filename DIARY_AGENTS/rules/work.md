...
`commonMain`의 작업 내용(`SyncWork.doWork()`, `DailyMemoNotificationWork.doWork()`)은 실패를 예외로 끝낸다. 실패를 어디에 보고할지도 작업 내용이 정한다. 실패 보고는 스펙이 플랫폼과 무관하게 한 번으로 정한 제품 동작이므로, 실행 수단마다 따로 보고하면 플랫폼 수만큼 갈라진다.
