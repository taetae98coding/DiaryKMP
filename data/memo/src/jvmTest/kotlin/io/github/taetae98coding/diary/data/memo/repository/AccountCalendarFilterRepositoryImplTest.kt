...
class AccountCalendarFilterRepositoryImplTest :
    FunSpec({
        test("선택 태그 조회는 로컬 태그를 도메인 모델로 변환한다") {
            val account = fixtureMonkey.giveMeOne<Account.User>()
            val localTagList = listOf(tag(), tag())
            val localDataSource = mockk<AccountCalendarFilterLocalDataSource>()
            every { localDataSource.getTagList(accountId = account.id) } returns flowOf(localTagList)
            val repository = AccountCalendarFilterRepositoryImpl(accountCalendarFilterLocalDataSource = localDataSource)
