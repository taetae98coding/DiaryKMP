...
class AccountPlaceTagRepositoryImplTest :
    FunSpec({
        test("연결된 태그 조회는 현재 계정의 로컬 태그를 도메인 모델로 변환해 전달한다") {
            ...
            val localDataSource = mockk<AccountPlaceTagLocalDataSource>()
            val transaction = mockk<AccountPlaceTagTransaction>()
            every {
                localDataSource.getTagList(accountId = account.id, placeId = placeId)
            } returns flowOf(localTagList)
            val repository =
                AccountPlaceTagRepositoryImpl(
                    accountPlaceTagLocalDataSource = localDataSource,
                    accountPlaceTagTransaction = transaction,
                )

            repository.getTagList(account = account, placeId = placeId).first() shouldBe
                localTagList.map { local -> local.toDomain() }

            verify(exactly = 1) { localDataSource.getTagList(accountId = account.id, placeId = placeId) }
        }
        ...
        test("연결할 수 있는 태그 페이지 조회는 검색어와 현재 계정으로 조회한 로컬 태그를 도메인 모델로 변환해 전달한다") {
            ...
            val transaction = mockk<AccountPlaceTagTransaction>()
            every {
                localDataSource.pageSelectableTag(accountId = account.id, placeId = placeId, query = query)
            ...
            repository.pageSelectableTag(account = account, placeId = placeId, query = query).first().items() shouldBe
                localTagList.map { local -> local.toDomain() }

            verify(exactly = 1) { localDataSource.pageSelectableTag(accountId = account.id, placeId = placeId, query = query) }
        }
