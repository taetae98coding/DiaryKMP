...
class SearchHomePlaceActionViewModelTest : FunSpec() {
    ...
    init {
        ...
        test("TC-SEARCH-HOME-FEATURE-032 장소를 삭제하면 삭제를 요청하고 삭제 안내를 한 번 보낸다") {
            runTest(mainDispatcher) {
                ...
                coVerify(exactly = 1) { environment.deletePlaceUseCase(parameter = id) }
            }
        }

        test("TC-SEARCH-HOME-FEATURE-036 장소 삭제가 저장되지 못하면 안내를 보내지 않는다") {
            runTest(mainDispatcher) {
                val id = searchId()
        ...
        test("TC-SEARCH-HOME-FEATURE-033 장소 삭제를 실행 취소하면 복구를 요청하고 새 안내는 보내지 않는다") {
            runTest(mainDispatcher) {
                val id = searchId()
                val environment = Environment()
                ...
                coVerify(exactly = 1) { environment.restorePlaceUseCase(parameter = id) }
            }
        }
    }
    ...
    private class Environment(
        ...
        val deletePlaceUseCase: DeletePlaceUseCase = mockk { coEvery { this@mockk(parameter = any()) } returns result }
        val restorePlaceUseCase: RestorePlaceUseCase = mockk { coEvery { this@mockk(parameter = any()) } returns result }

        val viewModel: SearchHomePlaceViewModel =
            SearchHomePlaceViewModel(
                searchPlaceUseCase = mockk(),
