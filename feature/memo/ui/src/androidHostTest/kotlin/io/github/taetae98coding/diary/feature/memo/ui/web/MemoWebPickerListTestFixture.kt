 ...
 */
internal fun ComposeContentTestRule.awaitWebPickerRows() {
    awaitWebPicker(description = "첫 페이지가 준비되지 않았다") {
        webPickerList().fetchSemanticsNode().children.isNotEmpty()
    }
}

private fun ComposeContentTestRule.awaitWebPicker(
    description: String,
    condition: () -> Boolean,
) {
    repeat(WEB_PICKER_WAIT_ATTEMPT_COUNT) {
        waitForIdle()
        if (condition()) return
        advanceWebPicker()
    }
 ...
 */
private fun ComposeContentTestRule.advanceWebPicker() {
    mainClock.advanceTimeByFrame()
    @Suppress("ForbiddenMethodCall")
    Thread.sleep(WEB_PICKER_WAIT_INTERVAL_MILLIS)
}
