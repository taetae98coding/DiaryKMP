...
class PlaceAddTagViewModelTest : FunSpec() {
    ...
    init {
        ...
        test("TC-PLACE-ADD-FEATURE-017 TC-PLACE-ADD-FEATURE-035 초기 태그가 없으면 태그를 하나도 고르지 않은 상태로 시작한다") {
        ...
        test("TC-PLACE-ADD-FEATURE-018 TC-PLACE-ADD-FEATURE-036 고른 태그는 선택 상태로만 반영된다") {
            runTest(mainDispatcher) {
                val tag = tag()
        ...
        test("TC-PLACE-ADD-FEATURE-018 고른 태그를 해제하면 선택에서 빠진다") {
            runTest(mainDispatcher) {
                val firstTag = tag()
                val secondTag = tag()
                val viewModel = viewModel(initialTagId = null)
