...
@Config(sdk = [36])
class PlaceDetailScreenMemoReturnTest {
    @get:Rule
    val composeRule = createComposeRule()

    @Test
    fun `TC-PLACE-DETAIL-MEMO-FEATURE-007 화면에 돌아오면 바뀐 날짜 기준으로 오늘 날짜 그룹을 다시 표시한다`() {
                    ...
                    placeMemoPagingData(
                        itemList =
                            listOf(
                                ...
                                MemoListItem.Content(memo = placeMemo(title = newTodayMemoTitle, dateTime = allDayMemoDateTime(newToday))),
                                MemoListItem.DateHeader(date = previousToday),
                                MemoListItem.Content(memo = placeMemo(title = previousTodayMemoTitle, dateTime = allDayMemoDateTime(previousToday))),
                            ),
                    ),
            )
    ...
    @Test
    fun `TC-PLACE-DETAIL-MEMO-FEATURE-019 PlaceDetail 화면에서 메모 탭을 선택하면 연결된 메모 목록을 표시한다`() {
        val entryMemoTitle = memoTitle(prefix = "Entry")

        setScreenOnMemoTab(memoPagingData = placeMemoPagingData(itemList = listOf(MemoListItem.Content(memo = placeMemo(title = entryMemoTitle)))))
    ...
    private fun setScreenWithLifecycle(
        ...
        preparePlaceDetailTabViewModels(memoPagingData = memoPagingData)

        composeRule.setContent {
            CompositionLocalProvider(
                LocalLifecycleOwner provides lifecycleOwner,
                LocalUriHandler provides mockk<UriHandler>(relaxed = true),
            ) {
                PlaceDetailScreenTestTheme {
                    PlaceDetailScreen(
                        ...
                        detailViewModel = screenTestViewModel(MutableStateFlow(content(id = FIRST_PLACE_ID))),
    ...
    private fun setScreenOnMemoTab(memoPagingData: PagingData<MemoListItem>) {
        composeRule.setPlaceDetailScreen(
            viewModel = screenTestViewModel(MutableStateFlow(content(id = FIRST_PLACE_ID))),
    ...
    private companion object {
        ...
        fun memoTitle(prefix: String): String = "PlaceMemo$prefix${fixtureMonkey.giveMeOne<String>().filter(Char::isLetterOrDigit)}"
    }
}
