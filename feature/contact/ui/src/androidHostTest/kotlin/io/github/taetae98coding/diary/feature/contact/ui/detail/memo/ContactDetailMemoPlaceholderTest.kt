...
@Config(sdk = [36])
class ContactDetailMemoPlaceholderTest {
    @get:Rule
    ...
    @Test
    fun `TC-CONTACT-DETAIL-MEMO-FEATURE-002 자리 표시 상태의 메모는 선택하거나 완료·삭제를 요청하지 않는다`() {
        val eventList = mutableListOf<MemoListEvent>()
        val memo = contactMemo(title = LOADED_MEMO_TITLE)
        ...
        composeRule.onAllNodesWithText(LOADED_MEMO_TITLE).fetchSemanticsNodes() shouldHaveSize 1
    ...
    private companion object {
        ...
        const val LOADED_MEMO_TITLE = "ContactMemoLoaded"
