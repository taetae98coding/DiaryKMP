...
### TC-PLACE-DETAIL-FEATURE-059: 여섯째 자리보다 긴 자리수로 저장된 좌표도 바꾸지 않으면 수정 동작을 제공하지 않는다

- 근거: `domain > 수정 동작의 제공 기준`
- Given: 좌표가 위도 `37.1234567`, 경도 `127.1234564`로 저장된 장소를 조회해 입력에 저장된 내용이 채워져 있다.
- When: 사용자가 어떤 입력도 바꾸지 않고 화면을 확인한다.
- Then: 위도 입력과 경도 입력에 `37.123457`과 `127.123456`이 표시되고, 수정 반영 동작이 제공되지 않는다.
...
### TC-PLACE-DETAIL-FEATURE-054: 수정 저장에 실패하면 안내 없이 입력을 유지하고 진행 상태만 해제한다

- 근거: `feature > 내용 수정`
- Given: 기기 저장이 실패하도록 설정되어 있고, 사용자가 제목과 좌표를 저장된 값과 다르게 바꿨다.
- When: 사용자가 수정 반영을 실행한다.
- Then: 화면에서 빠져나가지 않고 바꾼 제목과 좌표가 그대로 남으며 진행 상태가 해제된다. 성공 피드백과 저장 실패 안내는 제공되지 않고 화면 제목도 바뀌지 않으며, 사용자는 수정 반영을 다시 실행할 수 있다.
...
### TC-PLACE-DETAIL-FEATURE-058: 좌표가 성립하지 않아도 지도가 보고 있는 위치를 옮기지 않는다

- 근거: `feature > 좌표 오류`
- Given: 장소 조회에 성공해 지도가 저장된 좌표를 보고 있고, 사용자가 위도를 `abc`로 바꿨다.
- When: 사용자가 수정 반영을 실행한다.
- Then: 장소가 수정되지 않고, 지도를 다른 위치로 옮기도록 지정하지 않는다.
...
### TC-PLACE-DETAIL-FEATURE-056: 태그 연결이 다른 경로에서 바뀌면 태그 입력에 최신 연결을 반영한다

- 근거: `feature > 외부 변경`
- Given: 장소 조회에 성공해 태그 입력에 저장된 연결이 표시되어 있다.
- When: 같은 장소의 태그 연결이 다른 경로에서 새로 만들어진다.
- Then: 사용자가 아무것도 하지 않아도 태그 입력에 새로 연결된 태그가 나타난다.
...
- When: 사용자가 PlaceDetail 화면을 확인한다.
- Then: 지도 영역이 나타나지 않고 장소 디테일 탭의 본문에는 입력 영역만 표시되며, 별도의 로딩 안내나 오류 안내도 표시되지 않는다. 좌표 입력과 수정·삭제 동작은 사용할 수 있다.
...
### TC-PLACE-DETAIL-FEATURE-057: 지도 제공자를 바꾸면 그 뒤로는 바뀐 제공자의 지도가 열린다

- 근거: `feature > 외부 지도로 열기`, `domain > 외부로 여는 지도`
- Given: 장소 조회에 성공해 유효 범위 안의 좌표가 입력되어 있고, 기본 지도가 네이버 지도로 확인되어 지도가 표시되어 있다.
- When: 사용자가 화면의 지도에서 제공자를 Google 지도로 바꾼 뒤 외부 지도로 열기를 실행한다.
- Then: Google 지도에서 입력된 좌표를 가리키는 위치가 앱 밖에서 한 번 열리고, 네이버 지도의 위치는 열리지 않는다.
- 작성하지 않는 이유: 제공자를 바꾸는 조작은 지도가 표시된 화면의 제공자 전환 컨트롤에서만 할 수 있는데, 현재 호스트 테스트 환경은 지도 제공자의 실제 표시 요소를 제공하지 않아 지도가 표시된 PlaceDetail 화면을 구성할 수 없다. 선택된 제공자별로 여는 위치를 정하는 것은 `TC-PLACE-DETAIL-DOMAIN-022`, `TC-PLACE-DETAIL-DOMAIN-032`, `TC-PLACE-DETAIL-DOMAIN-034`가 검증한다. 지도 표시를 대체할 수 있는 테스트 환경이 제공되면 자동화한다.
...
### TC-PLACE-DETAIL-FEATURE-055: 탭을 전환해도 메모 탭에서 보던 목록 위치가 유지된다

- 근거: `feature > 탭 전환`
- Given: 메모 탭에 한 화면을 넘는 메모가 표시되어 있고, 사용자가 목록을 내려 맨 위가 아닌 메모를 보고 있다.
- When: 사용자가 장소 디테일 탭으로 전환한 뒤 다시 메모 탭으로 돌아온다.
...
- When: 사용자가 화면에서 실행할 수 있는 동작을 확인한다.
- Then: 선택한 탭과 관계없이 외부 지도로 열기와 삭제를 실행할 수 있고, 기본 지도를 확인해 지도를 표시할 수 있으면 지도가 보이지 않는 메모 탭에서도 장소 검색을 실행할 수 있다.
- 테스트 데이터:
...
기본 지도를 확인하지 못해 지도를 표시할 수 없으면 장소 검색을 제공하지 않는 것은 [장소 검색 다이얼로그 테스트 케이스](./place-search-dialog.md)의 TC-PLACE-SEARCH-DIALOG-FEATURE-002가 다룬다.

### TC-PLACE-DETAIL-FEATURE-050: 메모 탭에서 검색 결과를 골라도 입력에 반영되고 탭은 바뀌지 않는다

- 근거: `feature > 탭 전환`, `feature > 장소 검색으로 채우기`
- Given: 제목이 채워진 장소가 조회된 PlaceDetail 화면에서 메모 탭이 선택되어 있고 검색 결과가 표시되어 있다.
- When: 사용자가 검색 결과 하나를 고른다.
- Then: 주소와 좌표 입력에만 고른 장소의 값이 반영되고, 제목은 그대로이며, 메모 탭이 그대로 선택되어 있다.
...
- Given: 대상 장소가 테스트 데이터의 상태다.
- When: PlaceDetail 화면이 대상 장소를 조회한다.
- Then: 현재 계정과 연결된 장소만 조회되고, 다른 계정과만 연결된 장소는 조회할 수 없는 것으로 다룬다.
- 테스트 데이터:
...
- 근거: `domain > 상세 대상과 내용 표시`
- Given: 대상 장소가 저장되어 있으나 현재 계정을 확인할 수 없다.
- When: PlaceDetail 화면이 대상 장소를 조회한다.
...
### TC-PLACE-DETAIL-DOMAIN-038: 소수 여섯째 자리보다 긴 좌표는 여섯째 자리로 반올림해 수정한다

- 근거: `domain > 수정되는 내용`
- Given: 저장된 장소가 있고 공백이 아닌 제목이 입력되어 있다.
- When: 위도 `37.1234567`, 경도 `127.1234564`의 좌표로 수정 반영을 실행한다.
- Then: 수정이 성립하고 저장된 좌표가 위도 `37.123457`, 경도 `127.123456`으로 바뀐다.
...
- 근거: `domain > PlaceDetail의 지도`
- Given: PlaceDetail 화면에 지도가 표시되어 있고 사용자가 제목, 설명, 컬러를 바꾸고 좌표를 유효 범위 안의 다른 값으로 바꿨다.
- When: 저장된 기본 지도가 다른 지도로 바뀌고 입력 정지 대기 시간이 지난다.
- Then: 초기 지도 제공자가 바뀐 기본 지도로 지정되고, 새로 시작한 지도의 초기 위치는 그 시점의 저장된 좌표이며, 수정 중인 제목, 설명, 컬러와 좌표는 그대로 유지된다. 대기 시간이 지난 뒤 입력한 좌표에 지점이 지정되고 지도가 그 좌표로 옮겨지도록 지정된다.
...
### TC-PLACE-DETAIL-DATA-001: 계정과 대상 장소를 기준으로 기기에 저장된 장소를 찾는다

- 근거: `data > 대상 장소 조회`
- Given: 현재 계정이 있고 대상 장소가 기기에 저장되어 있다.
- When: PlaceDetail 화면이 대상 장소를 조회한다.
- Then: 현재 계정의 대상 장소 한 건이 조회된다.
...
- When: 사용자가 수정 반영을 실행한다.
- Then: 같은 장소가 새 내용으로 덮어써져 저장되고 장소가 새로 추가되지 않으며, 계정 연결과 메모 연결은 그대로 남는다.

### TC-PLACE-DETAIL-DATA-004: 삭제를 실행하면 기기에서 지우지 않고 삭제 상태로 저장한다

- 근거: `data > 삭제의 저장`
- Given: 현재 계정과 연결되고 메모와도 연결된 장소가 저장되어 있다.
- When: 사용자가 삭제를 실행한다.
- Then: 장소가 삭제 상태로 저장되고, 장소 자체와 계정 연결, 메모 연결이 기기에서 지워지지 않는다.

### TC-PLACE-DETAIL-DATA-005: 수정이나 삭제의 저장에 실패하면 성공으로 다루지 않는다

- 근거: `data > 저장 실패`
- Given: 현재 계정과 연결된 장소가 저장되어 있으나 기기 저장이 실패한다.
- When: 사용자가 테스트 데이터의 동작을 실행한다.
- Then: 그 동작이 성공한 것으로 다뤄지지 않고 성공 피드백도 제공되지 않으며, 기기에 저장된 장소의 내용과 연결은 그 동작이 반영되지 않은 채로 남는다.
- 테스트 데이터:
...
- Then: 연결 대상 표시에는 연결된 완료 태그가 나누어 불러오지 않고 한꺼번에 나타나고, 태그 선택 목록에는 미완료·미삭제 태그와 연결된 완료 태그가 나누어 불러와지며 함께 나타난다.
