...
이 문서에서 `domain > 추가 항목의 동작 판정`, `domain > 추가한 태그의 자동 선택`, `domain > 태그 검색어`, `feature > 선택 상태 표시`, `feature > 태그 검색`, `feature > 태그 상세 이동`, `feature > 태그 선택 목록`, `feature > 태그 선택과 해제`, `feature > 태그 추가 이동` 절은 [태그 선택 입력 공통 스펙](../spec/tag-select-input.md)이 소유한다. 나머지 케이스의 절은 기준 스펙이 소유한다.
...
- 근거: `domain > 선택한 것으로 표시하는 태그`
- Given: 선택되고 대표 태그로 지정되어 있던 태그가 삭제되어 태그 입력의 선택에서 제외된 상태이며, 그 선택과 대표 태그 지정은 저장소에 유지되어 있다.
...
### TC-MEMO-TAG-INPUT-DOMAIN-019: 화면이 재생성되어도 열려 있는 목록의 검색어가 유지된다
