...
검색어의 정규화와 일치 판정, 검색 결과 없음 판정은 [검색어 일치 판정 스펙](../spec/client/search-match.md)에 위임되어 있으며, 웹 선택 목록에서의 결과를 이 문서의 케이스로 덮는다. 아직 준비되지 않은 자리의 표시는 [페이지 조회 목록의 자리 표시 스펙](../spec/client/paged-list-placeholder.md)에 위임되어 있다.
...
### TC-MEMO-WEB-INPUT-FEATURE-031: 목록을 연 적이 없어도 화면에 들어온 뒤 대상이 없다고 확인되면 첫 누름에 WebAdd 이동을 요청한다

- 근거: `domain > 추가 항목의 동작 판정`
- Given: 계정과 연결되고 삭제되지 않은 웹 항목이 하나도 저장되어 있지 않은 상태로 화면에 들어왔고, 웹 선택 목록을 한 번도 열지 않은 채 선택할 수 있는 웹 항목이 없다는 확인이 끝났다.
- When: 사용자가 추가 항목을 처음 누른다.
- Then: 웹 선택 목록이 열리지 않고 WebAdd 화면으로의 이동이 한 번 요청된다.
...
### TC-MEMO-WEB-INPUT-FEATURE-030: 목록을 열면 검색어 입력에 초점이 놓인다
...
- 작성하지 않는 이유: 사용자가 웹 목록에서 정렬을 고른 상태를 만들려면 웹 목록 화면을 거친 뒤 메모 화면의 웹 선택 목록을 열어야 해, 여러 화면을 통과하는 검증이 된다. 한 화면 안에서는 웹 목록에서 고른 정렬이라는 사전 상태를 만들 수 없다. 웹 선택 목록이 항상 제목 오름차순으로 나타난다는 결과는 `TC-MEMO-WEB-INPUT-DOMAIN-002`로 검증한다. 여러 화면을 이어서 검증하는 테스트 환경이 갖춰지면 작성한다.
...
- When: 사용자가 그 웹 항목의 제목과 설명 어느 쪽도 만족하지 않는 검색어를 입력한다.
...
### TC-MEMO-WEB-INPUT-DOMAIN-012: 화면이 회전해도 열려 있는 목록의 검색어와 좁힌 결과가 유지된다

- 근거: `domain > 웹 검색어`, `domain > 진행 상태 유지`
- Given: 웹 선택 목록이 열려 있고 일부 웹만 남도록 검색어가 입력되어 있다.
- When: 화면이 회전하거나 창 크기가 바뀐다.
- Then: 목록이 열린 채로 검색어와 그 검색어로 좁힌 결과가 그대로 유지된다.

### TC-MEMO-WEB-INPUT-DOMAIN-017: 메모리 정리 뒤 복원하면 검색어가 다시 나타나고 기다리지 않고 그 검색어로 좁힌 목록을 보여 준다

- 근거: [검색어 일치 판정 스펙의 검색어 반영 시점](../spec/client/search-match.md#검색어-반영-시점), `domain > 웹 검색어`, `domain > 진행 상태 유지`
- Given: 웹 선택 목록이 열려 있고 일부 웹만 남도록 검색어가 입력되어 있다.
...
### TC-MEMO-WEB-INPUT-DOMAIN-015: 검색어는 제목과 설명으로 판정한다
...
### TC-MEMO-WEB-INPUT-DATA-001: 웹 선택 목록을 나누어 불러온다
