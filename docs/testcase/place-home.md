...
기준 스펙: [PlaceHome 화면 스펙](../spec/place-home.md)

보기 모드 전환, 지도 표시, 두 보기 모드의 목록과 지도 핀, 장소 추가와 상세로 넘기는 것, 정렬과 새로고침을 다루는 케이스의 `근거`가 가리키는 절은 [장소 보기 모드 스펙](../spec/place-view-mode.md)이 소유하고, 나머지 케이스의 절은 기준 스펙이 소유한다. 목록 모드의 자리 표시 케이스의 절은 공통 규칙을 [페이지 조회 목록의 자리 표시 스펙](../spec/paged-list-placeholder.md)에, 빈 상태 케이스의 절은 [목록 빈 상태 스펙](../spec/list-empty-state.md)에 위임한다.
...
- 근거: `domain > 목록 갱신`
- Given: 지도가 보이는 영역을 알려왔고 장소 목록 조회가 실패하도록 정해져 있다.
- When: 사용자가 장소 목록을 확인한다.
- Then: 목록에 장소가 표시되지 않고, 오류 안내도 표시되지 않는다.
...
- 작성하지 않는 이유: 장소가 컬러 색과 제목 라벨의 핀으로 변환되는 것은 자동화한다. 핀이 지도 기능에 지정되는 것은 지도가 표시된 화면을 구성해야 관찰할 수 있는데, 현재 호스트 테스트 환경은 실제 지도의 플랫폼 표시 요소를 제공하지 않는다. 지도 표시를 포함한 PlaceHome을 반복 가능하게 구성할 수 있는 테스트 환경이 제공되면 자동화한다.
...
- 작성하지 않는 이유: 핀이 지도 기능에 지정되는 것은 지도가 표시된 화면을 구성해야 관찰할 수 있는데, 현재 호스트 테스트 환경은 실제 지도의 플랫폼 표시 요소를 제공하지 않는다. 목록 갱신 자체는 TC-PLACE-HOME-FEATURE-019에서 검증한다. 지도 표시를 포함한 PlaceHome을 반복 가능하게 구성할 수 있는 테스트 환경이 제공되면 자동화한다.
...
- 작성하지 않는 이유: 핀이 지도 기능에 지정되지 않는 것은 지도가 표시된 화면을 구성해야 관찰할 수 있는데, 현재 호스트 테스트 환경은 실제 지도의 플랫폼 표시 요소를 제공하지 않는다. 빈 목록 자체는 TC-PLACE-HOME-FEATURE-020에서 검증한다. 지도 표시를 포함한 PlaceHome을 반복 가능하게 구성할 수 있는 테스트 환경이 제공되면 자동화한다.
...
- 작성하지 않는 이유: 장소 목록은 지도가 표시된 뒤에만 함께 표시되고, 현재 호스트 테스트 환경은 실제 지도의 플랫폼 표시 요소를 제공하지 않아 목록이 있는 PlaceHome을 결정적으로 구성할 수 없다. 장소 카드를 눌렀을 때 선택이 전달되는 것은 장소 카드 단위 테스트에서 검증한다. 지도 표시를 포함한 PlaceHome을 반복 가능하게 구성할 수 있는 테스트 환경이 제공되면 자동화한다.
...
- When: 사용자가 장소 목록을 아래로 당겨 새로고침을 요청한다.
- Then: 계정 데이터의 서버 동기화가 한 번 요청되고 진행 표시가 나타난다.
- 진행 표시가 나타나는 것은 `TC-PLACE-HOME-FEATURE-030`이 다룬다.

아래 세 케이스도 같은 이유로 자동화하지 않는다. 지도와 함께 표시되는 장소 목록 영역이 필요하며, 지도 표시를 대체할 수 있는 테스트 환경이 제공되면 자동화한다.
...
### TC-PLACE-HOME-DOMAIN-003: 화면이 재생성되어도 현재 위치를 다시 확인하지 않는다
...
### TC-PLACE-HOME-DATA-001: 계정과 보이는 영역을 기준으로 저장소에서 장소를 조회한다

- 근거: `data > 목록 조회`
- Given: 현재 계정과 연결된 장소들이 로컬에 저장되어 있고 지도가 보이는 영역을 알려왔다.
- When: 장소 목록을 조회한다.
...
- When: 조회 기준을 만족하는 장소가 로컬에 새로 저장된다.
...
### TC-PLACE-HOME-DATA-004: 목록 모드의 목록은 계정 기준 페이지 조회 결과로 구성된다

- 근거: `data > 목록 조회`
- Given: 현재 계정과 연결된 장소들이 로컬에 저장되어 있고 목록 모드가 표시되어 있다.
- When: 저장소에서 계정 기준으로 조회된 장소가 목록에 전달된다.
