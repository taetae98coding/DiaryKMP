...
- 작성하지 않는 이유: 기본 지도를 고르지 않았을 때 제공되는 기본 지도로 공용 지도 기능의 기본 제공자에서 시작하는 것은 자동화한다. MoreHome의 `장소` 항목을 거쳐 진입하는 경로는 여러 기능의 화면과 전환 이력을 함께 구성해야 판정할 수 있어 현재 테스트 환경에서는 결정적으로 검증할 수 없다. 여러 기능의 화면과 전환 이력을 함께 구성할 수 있는 테스트 환경이 갖춰지면 진입 경로까지 자동화한다.
...
### TC-PLACE-HOME-FEATURE-054: 목록 모드에서 장소 카드를 삭제 방향으로 밀면 삭제되고 안내와 실행 취소가 표시된다

- 근거: `feature > 삭제`
- Given: PlaceHome 화면이 목록 모드로 장소를 표시하고 있고 삭제가 성공하도록 설정되어 있다.
- When: 사용자가 그 장소 카드를 오른쪽에서 왼쪽으로 민다.
- Then: 그 장소의 삭제가 한 번 요청되고, 삭제되었다는 안내와 실행 취소 동작이 표시된다.
- 테스트 데이터:
...
### TC-PLACE-HOME-FEATURE-055: 지도 모드의 장소 목록에서 장소 카드를 삭제 방향으로 밀면 그 장소의 삭제가 요청된다

- 근거: `feature > 삭제`
- Given: 지도 모드의 장소 목록에 보이는 영역 안의 장소가 표시되어 있다.
- When: 사용자가 그 장소 카드를 오른쪽에서 왼쪽으로 민다.
- Then: 그 장소의 삭제가 한 번 요청되고 상세 이동은 요청되지 않는다.

### TC-PLACE-HOME-FEATURE-056: 지도 모드에서 삭제한 장소는 목록과 지도 핀에서 함께 사라진다

- 근거: `feature > 삭제`, [장소 보기 모드 스펙의 목록 갱신](../spec/client/place-view-mode.md#목록-갱신)
- Given: 지도 모드의 장소 목록과 지도 핀에 같은 장소가 표시되어 있다.
- When: 사용자가 목록에서 그 장소를 삭제한다.
- Then: 그 장소의 카드와 지도 핀이 함께 사라진다.
- 작성하지 않는 이유: 지도 핀이 사라지는 것은 지도 제공자의 지도가 실제로 표시되어야 관찰할 수 있고, 현재 호스트 테스트 환경은 실제 지도의 플랫폼 표시 요소를 제공하지 않는다. 목록과 핀이 같은 장소로 함께 갱신되는 것과 삭제 상태가 된 장소가 조회 결과에서 사라지는 것은 각각 기존 목록·핀 케이스와 TC-PLACE-HOME-DATA-003에서 검증한다. 지도 표시를 대체할 수 있는 테스트 환경이 제공되면 자동화한다.

### TC-PLACE-HOME-FEATURE-057: 장소 카드를 반대 방향으로 밀면 아무 동작도 실행하지 않는다

- 근거: `feature > 삭제`
- Given: PlaceHome 화면에 장소 카드가 표시되어 있다.
- When: 사용자가 그 장소 카드를 왼쪽에서 오른쪽으로 민다.
- Then: 삭제가 요청되지 않고 장소가 목록에 그대로 남으며 안내도 표시되지 않는다.

### TC-PLACE-HOME-FEATURE-058: 아직 준비되지 않은 자리는 밀어도 삭제되지 않는다

- 근거: `feature > 삭제`
- Given: 목록 모드에 아직 준비되지 않은 자리가 표시되어 있다.
- When: 사용자가 그 자리를 오른쪽에서 왼쪽으로 민다.
- Then: 삭제가 요청되지 않는다.

### TC-PLACE-HOME-FEATURE-065: 지도 핀을 밀어도 삭제가 요청되지 않는다

- 근거: `feature > 삭제`
- Given: 지도 모드의 지도에 장소의 지도 핀이 표시되어 있다.
...
### TC-PLACE-HOME-FEATURE-059: 삭제를 실행 취소하면 그 장소의 삭제가 되돌려진다

- 근거: `feature > 삭제`
- Given: PlaceHome 화면에서 장소를 삭제해 안내와 실행 취소 동작이 표시되어 있다.
- When: 사용자가 실행 취소를 선택한다.
- Then: 그 장소를 삭제되지 않은 상태로 되돌리는 요청이 한 번 일어난다.

### TC-PLACE-HOME-FEATURE-060: 안내가 보이는 동안 다른 장소를 삭제하면 마지막 삭제의 안내만 남는다

- 근거: `feature > 안내와 실행 취소`
- Given: PlaceHome 화면에서 장소 A를 삭제해 안내가 표시되어 있다.
- When: 안내가 사라지기 전에 장소 B가 삭제되고, 이어서 사용자가 실행 취소를 선택한다.
- Then: 삭제 안내는 하나만 표시되고, 실행 취소는 장소 B에만 적용되며 장소 A는 되돌려지지 않는다.

### TC-PLACE-HOME-FEATURE-061: 실행 취소를 선택하지 않으면 안내가 잠시 뒤 사라지고 되돌릴 수 없다

- 근거: `feature > 안내와 실행 취소`
- Given: PlaceHome 화면에서 장소를 삭제해 안내가 표시되어 있다.
- When: 사용자가 실행 취소를 선택하지 않고 안내가 저절로 사라질 때까지 기다린다.
- Then: 안내와 실행 취소 동작이 사라지고, 삭제를 되돌리는 요청은 일어나지 않는다.

### TC-PLACE-HOME-FEATURE-062: 삭제가 저장되지 못하면 안내를 표시하지 않는다

- 근거: `feature > 안내와 실행 취소`
- Given: 장소 삭제의 저장이 실패하도록 설정되어 있다.
- When: 사용자가 목록에서 장소를 삭제한다.
- Then: 삭제 안내와 실행 취소 동작이 제공되지 않는다.

### TC-PLACE-HOME-FEATURE-063: 안내가 보이는 동안 화면을 떠나면 안내가 닫히고 되돌릴 수 없다

- 근거: `feature > 안내와 실행 취소`
- Given: PlaceHome 화면에서 장소를 삭제해 안내와 실행 취소 동작이 표시되어 있다.
- When: 사용자가 PlaceHome 화면을 떠났다가 다시 돌아온다.
- Then: 안내와 실행 취소 동작이 보이지 않고, 삭제를 되돌리는 요청은 일어나지 않는다.

### TC-PLACE-HOME-FEATURE-064: 안내가 보이는 동안 보기 모드를 바꿔도 안내가 남고 실행 취소할 수 있다

- 근거: `feature > 안내와 실행 취소`
- Given: PlaceHome 화면의 목록 모드에서 장소를 삭제해 안내와 실행 취소 동작이 표시되어 있다.
- When: 사용자가 지도 모드로 바꾼 뒤 실행 취소를 선택한다.
- Then: 보기 모드를 바꾼 뒤에도 안내와 실행 취소 동작이 표시되어 있고, 그 장소를 되돌리는 요청이 한 번 일어난다.

### TC-PLACE-HOME-FEATURE-066: 정렬 기준은 두 보기 모드가 함께 사용한다

- 근거: `feature > 정렬 선택`, `domain > 목록 정렬`
- Given: PlaceHome 화면이 제목 오름차순으로 정렬되어 있고, 보이는 영역 안에 장소가 여러 개 있다.
- When: 사용자가 한 보기 모드에서 정렬을 최근 수정순으로 바꾼다.
- Then: 지도 모드의 목록과 목록 모드의 목록이 모두 최근 수정순으로 구성된다.
...
- 근거: `domain > 보기 모드의 유지`, `feature > 검색으로 이동`
...
### TC-PLACE-HOME-DOMAIN-028: 목록의 삭제는 삭제 여부를 삭제로 바꾸고 수정 시각을 갱신한다

- 근거: `domain > 삭제와 실행 취소의 상태 전이`
- Given: 현재 계정과 연결된 장소가 저장되어 있고 현재 시각이 고정되어 있다.
- When: 목록에서 그 장소를 삭제한다.
- Then: 현재 계정의 그 장소만 삭제 여부가 삭제로, 수정 시각이 고정된 현재 시각으로 저장된다.

### TC-PLACE-HOME-DOMAIN-029: 실행 취소는 삭제 여부를 미삭제로 되돌리고 수정 시각을 갱신한다

- 근거: `domain > 삭제와 실행 취소의 상태 전이`
- Given: 현재 계정과 연결된 장소가 삭제되어 있고 현재 시각이 고정되어 있다.
- When: 그 삭제를 실행 취소한다.
- Then: 현재 계정의 그 장소만 삭제 여부가 미삭제로, 수정 시각이 고정된 현재 시각으로 저장된다.

### TC-PLACE-HOME-DOMAIN-030: 현재 계정을 확인할 수 없으면 실행 취소가 저장되지 않아 장소가 삭제된 채 남는다

- 근거: `domain > 삭제와 실행 취소의 상태 전이`, `data > 삭제와 실행 취소의 저장`
- Given: 장소를 삭제했고, 현재 계정을 확인할 수 없는 상태다.
- When: 사용자가 그 삭제를 실행 취소한다.
- Then: 장소의 삭제 여부가 바뀌지 않아 삭제된 채로 남고, 저장하지 못했다는 별도 안내도 표시되지 않는다.
...
### TC-PLACE-HOME-DATA-006: 삭제와 실행 취소를 저장하면 동기화가 요청된다
...
### TC-PLACE-HOME-DATA-007: 로그인하지 않은 상태의 실행 취소는 게스트 계정 기준으로 기기에 저장된다

- 근거: `data > 삭제와 실행 취소의 저장`
- Given: 로그인하지 않아 게스트 상태의 계정이 확인된다.
- When: 장소 삭제의 실행 취소를 저장한다.
- Then: 게스트 계정 기준으로 그 장소의 삭제 여부가 미삭제로 기기에 저장된다.

### TC-PLACE-HOME-DATA-008: 실행 취소의 저장에 실패하면 실패를 전달하고 동기화를 요청하지 않는다
