...
- 작성하지 않는 이유: 앱 전체를 띄워 앱 시작 지점이 이 요청을 시작하는지까지는 자동화하지 않는다. 앱 전체는 모든 기능 화면과 그 화면이 쓰는 저장소, 서버 연결을 함께 준비해야 띄울 수 있어 현재 테스트 환경에서 결정적으로 실행할 수 없다. 요청 지점이 시작될 때 알림 권한 요청이 한 번 시작되는 것은 자동화한다. 기능 화면 없이 앱 시작 지점만 띄울 수 있는 테스트 환경이 갖춰지면 앱 시작까지 자동화한다.
