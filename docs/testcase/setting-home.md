...
- Then: `공휴일`, `지도`, `Gemini` 항목을 각각 선택할 수 있다.
...
- Then: `공휴일`, `지도`, `Gemini` 순서로 배치되고, 그 밖의 설정 항목은 표시되지 않는다.
