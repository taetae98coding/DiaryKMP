...
### TC-CALENDAR-MEMO-MOVE-FEATURE-015: 이동 중 달 이동으로 옮겨질 기간이 달라지면 추가 촉각 피드백을 받는다

- 근거: `feature > 이동 피드백`
- Given: 촉각 피드백을 제공하도록 설정된 기기의 캘린더 홈에 2026년 7월이 표시되어 있고, 사용자가 7월 15일 위치에서 메모 `회의`의 이동을 시작해 시작 촉각 피드백을 받은 뒤 그 위치를 누르고 있다.
- When: 사용자가 메모를 누른 손가락을 그대로 둔 채 다른 손가락으로 왼쪽으로 스와이프해 2026년 8월로 이동하고, 옮겨질 기간이 이동한 달에서 가리키는 날짜 기준으로 달라진다.
- Then: 기간 변경을 알리는 촉각 피드백이 한 번 더 제공된다.
...
- 근거: `feature > 이동 중단`
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 7월 14일부터 7월 16일까지의 메모 `회의`가 표시되어 있다.
- When: 사용자가 7월 15일 위치의 `회의` 제목에서 이동을 시작해 7월 22일로 드래그한 뒤, 손을 떼지 않은 상태에서 화면 상태를 저장하지 않은 채 시스템이 조작을 끊는다.
...
### TC-CALENDAR-MEMO-MOVE-FEATURE-014: 공휴일 이름·생일·날씨 위에서 길게 누르면 날짜 기간 선택이 시작된다

- 근거: `feature > 이동 시작`, `domain > 이동 대상 메모`, [CalendarHome 스펙의 날짜 기간 선택으로 메모 추가 시작](../spec/client/calendar-home.md#날짜-기간-선택으로-메모-추가-시작)
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 7월 14일부터 7월 16일까지의 메모 `회의`와 테스트 데이터의 아이템이 표시되어 있다.
- When: 사용자가 테스트 데이터의 아이템 위에서 길게 누르기를 시작한 뒤 손을 뗀다.
- Then: 길게 누르고 있는 동안 그 아이템이 놓인 날짜 하루가 선택되어 있고 메모 이동은 시작되지 않는다. 손을 떼면 그 하루의 기간으로 MemoAdd 화면 이동이 요청되고, 그 아이템을 눌렀을 때의 선택 결과는 실행되지 않는다.
- 테스트 데이터:

  | 아이템 | 놓인 날짜 | 실행되지 않는 선택 결과 |
  | --- | --- | --- |
  | 공휴일 이름 `제헌절` | 7월 17일 | 공휴일 이름 검색 결과 열기 |
  | 연락처 `홍길동`의 생일 | 7월 8일 | ContactDetail 화면 이동 |
  | 7월 21일의 날씨 | 7월 21일 | 날씨 검색 결과 열기 |

### TC-CALENDAR-MEMO-MOVE-FEATURE-016: 이동 중 가장자리 영역에 머무는 동안 달 이동이 반복된다

- 근거: `feature > 이동 중 달 이동`
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 사용자가 7월 15일 위치에서 7월 14일부터 7월 16일까지의 메모 `회의`의 이동을 시작해 오른쪽 가장자리 영역에 머물러 2026년 8월로 이동한 상태다.
- When: 사용자가 달 전환이 끝난 뒤에도 오른쪽 가장자리 영역에 계속 머무른다.
- Then: 캘린더가 2026년 9월로 한 번 더 이동하고, 메모 이동은 중단되지 않으며 9월에서 가리키는 날짜를 기준으로 옮겨질 기간이 갱신된다.
...
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 제목·설명·컬러·태그 연결·대표 태그·장소 연결·웹 연결·연락처 연결이 저장된 7월 14일부터 7월 16일까지의 메모 `회의`가 표시되어 있다.
- When: 사용자가 7월 15일 위치의 `회의` 제목에서 이동을 시작해 7월 17일로 드래그한 뒤 완료한다.
- Then: `회의`의 기간이 7월 16일부터 7월 18일까지로 바뀌고 수정 시각이 반영 시점으로 기록되며, 제목, 설명, 컬러, 완료 여부, 삭제 여부, 생성 시각, 태그 연결과 대표 태그, 장소 연결, 웹 연결, 연락처 연결은 바뀌지 않는다.
...
- 근거: `domain > 유지 경계`
- Given: 캘린더 홈에서 사용자가 7월 15일 위치에서 7월 14일부터 7월 16일까지의 메모 `회의`의 이동을 시작해 7월 22일을 가리키고 있다.
- When: 앱이 백그라운드로 이동해 화면 상태가 저장되고, 화면은 그대로 남은 채 메모를 누른 조작이 끊긴다.
...
### TC-CALENDAR-MEMO-MOVE-DOMAIN-014: 화면을 벗어나면 이동 중 표시가 남지 않고 그 시점의 기간이 반영된다

- 근거: `domain > 유지 경계`, `feature > 이동 중단`
- Given: 캘린더 홈에서 사용자가 7월 15일 위치에서 7월 14일부터 7월 16일까지의 메모 `회의`의 이동을 시작해 7월 22일을 가리키고 있다.
- When: 화면 상태를 저장하지 않은 채 캘린더 홈 화면이 사라진다.
- Then: `회의`의 기간이 7월 21일부터 7월 23일까지로 반영된다.

### TC-CALENDAR-MEMO-MOVE-DOMAIN-015: 캘린더 밖으로 드래그해 완료하면 가장 가까운 날짜 칸을 기준으로 반영된다

- 근거: `domain > 가리키는 날짜 해석`
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 7월 14일부터 7월 16일까지의 메모 `회의`가 표시되어 있다. 2026년 7월 화면의 마지막 주는 8월 2일부터 8월 8일까지다.
- When: 사용자가 7월 15일 수요일 위치의 `회의` 제목에서 이동을 시작해, 가리키는 위치를 같은 세로줄을 따라 캘린더 아래쪽 밖으로 옮긴 뒤 완료한다.
- Then: 캘린더 안에서 가장 가까운 날짜 칸인 마지막 주 수요일 8월 5일을 가리킨 것으로 보아, `회의`의 기간이 8월 4일부터 8월 6일까지로 반영된다.

### TC-CALENDAR-MEMO-MOVE-DOMAIN-016: 가장자리 영역 밖에서는 이동을 위한 드래그로 달이 이동하지 않는다

- 근거: `domain > 이동 중 달 이동 기준`
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 사용자가 7월 15일 위치에서 메모 `회의`의 이동을 시작했다.
- When: 사용자가 가장자리 영역에 들어가지 않은 채 가리키는 날짜를 7월 17일로 바꾸고 기다린다.
- Then: 캘린더는 2026년 7월을 그대로 유지하고 메모 이동도 중단되지 않는다.

### TC-CALENDAR-MEMO-MOVE-DOMAIN-017: 이동 중 왼쪽 가장자리 영역에 머무르면 이전 달로 이동하고 메모 이동이 유지된다

- 근거: `domain > 이동 중 달 이동 기준`
- Given: 캘린더 홈에 2026년 7월이 표시되어 있고, 사용자가 7월 15일 위치에서 7월 14일부터 7월 16일까지의 메모 `회의`의 이동을 시작했다.
- When: 사용자가 현재 가리키는 위치를 왼쪽 가장자리 영역으로 옮겨 머무른다.
- Then: 캘린더가 2026년 6월로 이동하고, 메모 이동은 중단되지 않으며 6월에서 가리키는 날짜를 기준으로 옮겨질 기간이 갱신된다.
...
- 고스트 아이템의 그림자, 원본 조각의 불투명도 같은 구체적인 시각 표현은 디자인 문서의 책임이며 실제 픽셀을 판정해야 하므로 블랙박스 유닛 테스트 케이스로 작성하지 않는다. 화면의 실제 표현을 비교할 수 있는 스크린샷 테스트 환경이 갖춰지면 검증한다. 옮겨질 기간 자체가 맞는지는 TC-CALENDAR-MEMO-MOVE-FEATURE-004로 확인한다.
