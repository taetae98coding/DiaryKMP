...
### TC-SETTING-DOWNLOAD-FEATURE-021: 다른 앱에 다녀와도 입력 중이던 주소를 유지한다

- 근거: `domain > 유지와 초기화 기준`
- Given: Android 또는 iOS에서 입력을 저장된 주소와 다른 새 주소로 바꾼 SettingDownload 화면이 표시되어 있다.
- When: 사용자가 다른 앱으로 떠났다가 돌아온다.
- Then: 입력에 바꾼 주소가 그대로 남아 있고 저장 동작이 계속 제공되며, 저장은 수행되지 않는다.
...
### TC-SETTING-DOWNLOAD-DATA-003: 저장된 값을 읽을 수 없으면 비어 있는 주소로 제공하지 않는다

- 근거: `data > 저장 시점과 유지`
- Given: 저장된 프록시 주소를 읽을 수 없도록 제어되어 있다.
- When: 저장된 주소를 읽는다.
- Then: 비어 있는 주소를 기본값으로 제공하지 않고 읽기 실패를 그대로 알린다.

## 작성하지 않는 이유

- 앱을 종료하고 다시 시작한 뒤에도 저장된 주소가 유지되는 규칙과, 앱을 종료한 뒤 새로 시작하면 편집 상태가 저장된 주소로 돌아가는 규칙은 실제 저장 영역의 파일을 다시 열고 앱을 다시 실행해야 관찰할 수 있으므로 유닛 테스트 케이스로 작성하지 않는다. 자동화하려면 저장 영역을 임시 폴더에 만들고 닫았다 다시 여는 저장소 계층 테스트 환경과 앱 재실행을 제어하는 통합 테스트 환경이 필요하다.
- `data > 기기별 보관`의 로그인, 로그아웃, 계정 전환 후에도 주소가 유지되고 동기화 대상이 아니라는 규칙은 계정 흐름과 동기화 전체를 거쳐야 관찰할 수 있으므로 유닛 테스트 케이스로 작성하지 않는다. 자동화하려면 로그인·로그아웃·계정 전환과 동기화를 제어된 응답으로 실행하면서 같은 기기 저장 영역을 이어서 조회할 수 있는 통합 테스트 환경이 필요하다.
- `data > 환경별 저장 영역`은 [SettingMap 테스트 케이스](./setting-map.md)가 같은 저장 영역을 대상으로 다루므로 여기에서 다시 다루지 않는다.
