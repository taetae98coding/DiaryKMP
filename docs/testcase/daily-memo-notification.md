...
### TC-DAILY-MEMO-NOTIFICATION-FEATURE-010: 앱이 실행 중일 때 알림을 선택하면 보던 화면이 그대로 앞으로 나온다

- 근거: `client > feature > 알림에서 앱 열기`
- Given: 앱이 실행 중이고 사용자가 시작 화면이 아닌 화면을 보던 중에 앱을 떠났으며, 알림이 표시되어 있다.
- When: 사용자가 알림을 선택한다.
- Then: 실행 중이던 앱이 앞으로 나오고 보고 있던 화면이 그대로 유지된다.
- 작성하지 않는 이유: 실행 중인 앱을 앞으로 가져오고 화면을 유지하는 것은 각 플랫폼이 앱을 여는 동작이어서, 현재 테스트 환경은 실행 중인 앱에 알림 선택을 전달해 그 결과를 관찰할 수 없다. 실행 중인 앱에 알림 선택을 전달하고 보이는 화면을 확인할 수 있는 기기 테스트 환경이 갖춰지면 자동화한다.

### TC-DAILY-MEMO-NOTIFICATION-FEATURE-011: 지우지 않은 이전 날의 알림은 새 알림이 대신한다

- 근거: `client > domain > 하루에 표시되는 알림`
- Given: 이전 날의 알림을 사용자가 지우지 않아 표시되어 있다.
- When: 다음 날 알림이 테스트 데이터의 경로로 도착한다.
- Then: 새 알림이 이전 날의 알림을 대신해 사용자에게는 이 알림이 하나만 남는다.
- 테스트 데이터:
...
- 작성하지 않는 이유: 시스템이 표시한 알림 경로만 자동화하지 않는다. 그 알림이 몇 개 남는지는 각 플랫폼의 알림 목록에서만 관찰할 수 있고, 현재 테스트 환경은 시스템이 표시한 알림 목록을 확인하지 않는다. 시스템 알림 목록을 제어할 수 있는 테스트 환경이 제공되면 자동화한다.
...
### TC-DAILY-MEMO-NOTIFICATION-DOMAIN-032: 실행되지 않은 회차의 알림은 나중에 몰아서 보내지 않는다

- 근거: `server > domain > 발생 시각`
- Given: 서버에 계정 A의 기기가 시간대 `Asia/Seoul`로 등록되어 있고, `Asia/Seoul`의 오전 8시에 해당하는 회차의 확인이 서버 사정으로 실행되지 않았다.
- When: 다음 회차의 확인이 실행된다.
- Then: 그 기기로 그날의 알림이 발송되지 않는다.
- 작성하지 않는 이유: 서버의 발송 규칙이어서 앱의 단위 테스트 환경에서 결정적으로 판정할 수 없다. 서버 정책 테스트 환경이 마련되면 작성한다.
...
### TC-DAILY-MEMO-NOTIFICATION-DATA-007: 한 회차를 시작하지 못하면 그 회차의 알림을 보내지 않고 다시 시도하지 않는다

- 근거: `server > data > 발송`
- Given: 서버에 같은 시간대로 등록된 기기가 있고, 테스트 데이터의 이유로 그 시간대의 오전 8시 회차를 시작하지 못한다.
- When: 그 회차와 그다음 회차의 확인이 실행된다.
- Then: 어느 회차에서도 그 기기로 그날의 알림이 발송되지 않고, 사용자에게 알리지 않는다.
- 테스트 데이터:
...
### TC-DAILY-MEMO-NOTIFICATION-DATA-008: 발송은 서버의 예약 작업만 시작할 수 있다

- 근거: `server > data > 발송 처리의 보호`
- Given: 서버에 등록된 기기가 있다.
- When: 테스트 데이터의 요청자가 발송을 요청한다.
- Then: 요청이 거절되고 어느 기기로도 알림이 발송되지 않는다.
- 테스트 데이터:
...
- 작성하지 않는 이유: 서버의 접근 제한 규칙이어서 서버 함수 실행 환경에서만 판정할 수 있고, 그 환경은 저장소의 자동 검증 절차에 포함되어 있지 않다. 서버 함수 테스트 러너가 검증 절차에 들어오면 자동화한다.

### TC-DAILY-MEMO-NOTIFICATION-DATA-009: 앱은 발송에 쓰는 기기 목록과 다른 계정의 메모를 읽을 수 없다

- 근거: `server > data > 발송 처리의 보호`
- Given: 서버에 계정 A와 계정 B의 기기가 등록되어 있고, 계정 B에 오늘의 메모가 저장되어 있다.
- When: 계정 A의 로그인 세션으로 발송할 기기 목록이나 계정 B의 오늘의 메모를 조회하려 한다.
- Then: 어느 기기 목록도, 계정 B의 메모도 돌려받지 못한다.
