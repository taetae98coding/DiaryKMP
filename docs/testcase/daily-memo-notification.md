...
flowchart TD
    ...
    Runnable -- 아니오 --> Unknown["확인 안내만 담음<br/>TC-DAILY-MEMO-NOTIFICATION-DOMAIN-015"]
    Runnable -- 예 --> Fetch{"오늘의 메모를<br/>가져왔는가"}
...
### TC-DAILY-MEMO-NOTIFICATION-DOMAIN-014: 알림 내용은 알림이 발생하는 시점의 메모로 정한다

- 근거: `domain > 알림 내용을 정하는 시점`
- Given: 오전 6시에 알림이 예약되었고, 그때 현재 사용자 계정에는 오늘의 메모가 하나 있었다.
- When: 오전 7시에 오늘의 메모 하나가 추가된 뒤 현지 시각이 오전 8시가 된다.
- Then: 알림에 두 메모가 모두 담긴다.

### TC-DAILY-MEMO-NOTIFICATION-DOMAIN-015: 발생 시점에 앱 코드가 실행되지 않는 환경에서는 확인 안내만 담는다

- 근거: `domain > 알림 내용을 정하는 시점`
- Given: 앱 코드가 실행되지 않은 채 시스템이 알림을 표시하는 환경에서 알림이 예약되어 있다.
- When: 현지 시각이 오전 8시가 된다.
- Then: 알림은 오늘의 메모를 확인하라는 안내만 담고 개수와 제목 목록을 두지 않는다.
- 작성하지 않는 이유: 미리 등록한 알림을 정해진 시각에 표시하는 것은 플랫폼의 알림 시스템이 수행하며, 현재 테스트 환경은 그 플랫폼에서 실행되지 않고 실제 알림 표시도 재현하지 않는다. 그 플랫폼에서 예약된 알림의 내용을 확인할 수 있는 테스트 환경이 제공되면 자동화한다.
