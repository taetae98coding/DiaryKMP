...
### TC-MUSIC-DOWNLOAD-FEATURE-020: 다운로드를 실행하기 전에는 내려받기 도구를 설치하지 않는다

- 근거: `feature > 내려받기 준비`
- Given: 데스크톱 앱에서 `yt-dlp`가 없고 Homebrew는 있으며, 영상 ID를 얻을 수 있는 곡이 있다.
- When: PlaylistHome 화면이 표시되고 사용자가 다운로드를 실행하지 않는다.
- Then: 내려받기 준비와 도구 설치가 일어나지 않는다.

### TC-MUSIC-DOWNLOAD-FEATURE-021: 링크가 가리키는 영상이 바뀐 곡에는 이전 다운로드 상태를 표시하지 않는다

- 근거: `domain > 상태의 유지 범위`
- Given: 다운로드를 실행해 영상 A를 가리키는 곡의 항목에 다운로드 완료가 표시된 PlaylistHome 화면이 표시되어 있다.
- When: 그 곡의 링크가 테스트 데이터처럼 바뀌어 목록에 반영된다.
- Then: 테스트 데이터의 결과대로 그 곡의 항목에 다운로드 상태가 표시되거나 표시되지 않는다.
- 테스트 데이터:

| 바뀐 링크 | 결과 |
| --- | --- |
| 영상 B의 링크 | 어떤 상태도 표시되지 않는다 |
| 비어 있음 | 어떤 상태도 표시되지 않는다 |
| 영상 A를 가리키는 다른 형태의 링크 | 다운로드 완료가 그대로 표시된다 |
| 영상 B의 링크로 바꿨다가 영상 A의 링크로 되돌린다 | 다운로드 완료가 다시 표시된다 |

### TC-MUSIC-DOWNLOAD-FEATURE-022: 상태가 표시되지 않는 곡의 진행 중 다운로드도 다운로드 버튼의 진행 표시에 포함된다

- 근거: `domain > 상태의 유지 범위`
- Given: 영상 A를 가리키는 곡만 다운로드 진행 중이고 다른 곡에는 대기나 진행 중 상태가 없는 PlaylistHome 화면이 표시되어 있다.
- When: 그 곡의 링크가 영상 B의 링크로 바뀌어 목록에 반영된다.
- Then: 그 곡의 항목에는 어떤 상태도 표시되지 않고, 다운로드 버튼은 진행 표시로 남는다.
...
### TC-MUSIC-DOWNLOAD-DOMAIN-022: 진행 중인 곡의 링크를 바꿔도 판정한 영상으로 이어지고 새 실행 요청을 처리하지 않는다

- 근거: `domain > 상태의 유지 범위`, `domain > 다시 실행 성립 조건`
- Given: 다운로드를 실행해 영상 A를 가리키던 곡이 진행 중인 채 멈춰 있고, 그 뒤 그 곡의 링크가 영상 B의 링크로 바뀌었다.
- When: 다운로드를 한 번 더 실행한 뒤 멈춰 있던 곡이 끝난다.
- Then: 대상 곡을 다시 판정하지 않고, 영상 A의 내려받기가 한 번만 일어나 이어지며, 영상 B의 내려받기는 일어나지 않는다.
...
### TC-MUSIC-DOWNLOAD-DOMAIN-021: 백분율이 생긴 뒤에는 백분율 없는 진행 중으로 돌아가지 않는다

- 근거: `domain > 곡의 다운로드 상태`
- Given: Android 또는 iOS에서 영상 ID를 얻을 수 있는 링크가 저장된 곡이 있고, 프록시가 파일 응답을 시작해 일부씩 차례로 전달한 뒤 끝나도록 설정되어 있다.
- When: 다운로드를 실행한다.
- Then: 그 곡이 백분율을 가진 뒤로는 완료가 될 때까지 백분율 없는 진행 중이 되지 않는다.
