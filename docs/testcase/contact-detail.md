...
- Then: 연락처가 수정되지 않고 전화번호 입력이 필요하다는 피드백이 제공되며, 입력 중이던 이름, 설명, 키, 신발 사이즈, 생일, 고향과 전화번호 항목이 모두 그대로 유지된다.
...
- 근거: `feature > 진행 상태 유지`
- Given: ContactDetail 화면에서 입력을 바꾸고 수정을 실행하지 않은 채 테스트 데이터의 경계를 지났다.
- When: 사용자가 같은 연락처의 ContactDetail 화면에 다시 들어온다.
...
### TC-CONTACT-DETAIL-FEATURE-043: 처리 중 상태가 되거나 저장 내용이 갱신되어도 보던 화면 위치를 유지한다

- 근거: `feature > 진행 상태 유지`
- Given: 한 화면에 다 들어가지 않을 만큼 전화번호가 많은 연락처가 내용 표시 상태로 표시되어 있고, 사용자가 연락처 디테일 탭을 내려 마지막 전화번호 항목을 보고 있어 이름 입력은 화면 밖에 있다.
- When: 테스트 데이터의 변화가 일어난다.
- Then: 마지막 전화번호 항목이 같은 자리에 계속 보이고, 이름 입력은 여전히 화면 밖에 있어 화면이 맨 위로 돌아가지 않는다.
- 테스트 데이터:

| 변화 |
| --- |
| 수정 처리가 시작되어 진행 상태가 됨 |
| 즐겨찾기 변경 처리가 시작되어 진행 상태가 됨 |
| 삭제 처리가 시작되어 진행 상태가 됨 |
| 다른 경로에서 저장된 이름과 즐겨찾기 여부가 바뀌어 화면이 갱신됨 |

### TC-CONTACT-DETAIL-FEATURE-044: 메모 카드 위에서 밀면 탭은 바뀌지 않고 카드 동작이 실행된다

- 근거: `feature > 탭 전환`
- Given: ContactDetail 화면의 메모 탭이 선택되어 있고 목록에 메모 카드가 표시되어 있다.
- When: 사용자가 메모 카드 위에서 오른쪽으로 민다.
- Then: 메모 탭이 선택된 채로 남고, 그 메모의 완료가 한 번 요청된다.

### TC-CONTACT-DETAIL-FEATURE-045: 전화번호 미입력으로 수정하면 보고 있던 입력에 초점을 그대로 둔다

- 근거: `feature > 전화번호 미입력`
- Given: 저장된 연락처가 조회되어 있고, 사용자가 번호가 비어 있는 전화번호 항목을 추가한 뒤 이름이 아닌 다른 입력에 초점을 두고 있다.
- When: 사용자가 수정을 실행해 전화번호 입력이 필요하다는 피드백이 제공된다.
- Then: 입력 초점이 실행 전에 보고 있던 입력에 그대로 있다.

### TC-CONTACT-DETAIL-FEATURE-046: 이름을 비운 채 수정해도 수정 동작이 계속 제공된다

- 근거: `domain > 수정 동작의 제공 기준`
- Given: 이름이 저장된 연락처가 조회되어 있고 연락처 수정이 성공하도록 설정되어 있다.
- When: 사용자가 이름을 비운 채 수정을 실행하고 수정이 끝난다.
- Then: 비어 있는 이름 입력이 저장된 이름과 계속 다르므로 수정 동작이 계속 제공된다.

### TC-CONTACT-DETAIL-FEATURE-047: 화면이 재생성되어도 즐겨찾기 표시는 저장된 값을 보여 준다

- 근거: `feature > 진행 상태 유지`
- Given: 즐겨찾기 여부가 테스트 데이터와 같은 연락처의 ContactDetail 화면이 표시되어 있다.
- When: 화면이 재생성된다.
- Then: 즐겨찾기 표시가 저장된 즐겨찾기 여부 그대로 표시된다.
- 테스트 데이터:
...
### TC-CONTACT-DETAIL-FEATURE-048: 탭을 전환해도 메모 탭에서 보던 목록 위치가 유지된다

- 근거: `feature > 탭 전환`
- Given: 한 화면에 다 들어가지 않을 만큼 메모가 연결된 연락처의 ContactDetail 화면에서 메모 탭을 내려 뒤쪽의 메모를 보고 있다.
- When: 사용자가 연락처 디테일 탭으로 전환한 뒤 다시 메모 탭으로 돌아온다.
- Then: 전환 전에 보던 메모가 그대로 보이고 목록의 맨 위로 돌아가지 않는다.
...
- 근거: `domain > 수정 성립 조건`
- Given: 저장된 연락처가 있고 이름과 설명과 키와 신발 사이즈와 생일과 고향을 저장된 값과 다르게 바꾼 상태다.
- When: 번호가 테스트 데이터의 상태인 전화번호 항목을 포함해 수정을 실행한다.
- Then: 이름, 설명, 키, 신발 사이즈, 생일, 고향, 전화번호 중 어느 것도 수정되지 않고 전화번호 입력이 필요하다는 결과가 전달된다.
- 테스트 데이터:
...
- Then: 저장된 기존 이름이 유지되고 설명, 키, 신발 사이즈, 생일, 고향, 전화번호는 입력한 대로 수정된다.
- 테스트 데이터:
...
- 근거: `domain > 요청 제한`
- Given: 테스트 데이터의 처리 중인 동작이 완료되지 않고 진행 중인 상태다.
- When: 사용자가 테스트 데이터의 동작을 실행한다.
- Then: 테스트 데이터의 기대 결과대로 요청이 처리되거나 처리되지 않는다.
- 테스트 데이터:

| 처리 중인 동작 | 실행한 동작 | 기대 결과 |
| --- | --- | --- |
| 수정 | 수정 | 처리되지 않음 |
| 수정 | 즐겨찾기 변경 | 처리됨 |
| 수정 | 삭제 | 처리됨 |
| 즐겨찾기 변경 | 즐겨찾기 변경 | 처리되지 않음 |
| 즐겨찾기 변경 | 수정 | 처리됨 |
| 즐겨찾기 변경 | 삭제 | 처리됨 |
| 삭제 | 삭제 | 처리되지 않음 |
| 삭제 | 수정 | 처리됨 |
...
- 근거: `domain > 선택한 탭`
- Given: ContactDetail 화면에서 메모 탭을 선택한 뒤 테스트 데이터의 경계를 지났다.
...
### TC-CONTACT-DETAIL-DOMAIN-017: 즐겨찾기 변경은 즐겨찾기 여부를 반대 값으로 바꾸고 수정 시각을 기록한다

- 근거: `domain > 즐겨찾기 변경`
- Given: 즐겨찾기 여부가 테스트 데이터의 현재 값인 연락처가 있다.
- When: 그 연락처의 즐겨찾기 변경을 실행한다.
- Then: 즐겨찾기 여부가 테스트 데이터의 바뀐 값으로, 수정 시각이 변경 시점으로 기록된다.
- 테스트 데이터:
...
### TC-CONTACT-DETAIL-DOMAIN-018: 삭제 상태인 연락처도 즐겨찾기를 바꿀 수 있고 삭제 여부는 되돌리지 않는다

- 근거: `domain > 즐겨찾기 변경`
- Given: 현재 계정과 연결되고 삭제 상태이며 즐겨찾기가 아닌 연락처가 저장되어 있다.
- When: 그 연락처의 즐겨찾기 변경을 실행한 뒤 그 연락처를 조회한다.
- Then: 즐겨찾기 여부가 즐겨찾기로 바뀌고 삭제 상태는 그대로 유지된다.

### TC-CONTACT-DETAIL-DOMAIN-019: 이름을 비우고 번호가 없는 전화번호 항목도 있으면 전화번호 입력을 알린다

- 근거: `domain > 수정 성립 조건`
- Given: 이름이 저장된 연락처가 있다.
- When: 이름을 비우고 번호가 비어 있는 전화번호 항목을 포함해 수정을 실행한다.
- Then: 어떤 내용도 수정되지 않고 전화번호 입력이 필요하다는 결과가 전달된다.

### TC-CONTACT-DETAIL-DOMAIN-020: 앱이 백그라운드에 다녀와도 선택한 탭이 유지된다
