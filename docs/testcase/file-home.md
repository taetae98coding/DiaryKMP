...
FileHome 화면의 진입, 화면 내용, 뒤로가기는 [더보기 준비 중 화면 공통 스펙](../spec/client/more-menu-placeholder.md)에 위임되어 있다.
...
### TC-FILE-HOME-FEATURE-004: 제목 외의 정보와 준비 중 안내를 표시하지 않는다

- 근거: `feature > 화면 내용과 행동`
- Given: 사용자 언어가 한국어다.
- When: 사용자가 FileHome 화면을 확인한다.
- Then: 화면에 보이는 글은 제목 `파일` 하나뿐이고, 기능이 아직 준비 중이라는 안내나 그 밖의 정보가 표시되지 않는다.
