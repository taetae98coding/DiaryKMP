...
- 작성하지 않는 이유: MemoHome의 필터는 메모 목록 위에 쌓이는 별도 화면으로 열린다. 열림 여부는 저장해 둔 화면 전환 이력으로 유지되므로, 결과를 관찰하려면 메모 목적지의 화면 전환 이력, 그 이력을 화면으로 그리는 배치, 필터 화면의 상태 보관을 한꺼번에 띄운 채 재생성과 복원을 거쳐야 한다. 지금의 화면 단위 테스트는 목록 화면이나 필터 내용 하나만 띄우므로 이 흐름을 결정적으로 재현할 수 없다. 저장·복원할 수 있는 화면 전환 이력으로 메모 목적지 전체를 띄우고, 필터 화면에 필요한 상태 보관 객체를 테스트용으로 넣을 수 있는 내비게이션 테스트 환경이 갖춰지면 자동화한다.
