...
기준 스펙: [MemoHome 목록 스펙](../spec/client/memo-home.md)

태그 필터 케이스의 `근거`가 가리키는 MemoHome 목록 스펙의 태그 필터 관련 절은 공통 규칙을 [태그 필터 스펙](../spec/client/tag-filter.md)에, 필터를 바꾼 뒤 목록을 어디서부터 보는지는 [목록 필터 반영 스펙](../spec/client/list-filter.md)에 위임한다. 자리 표시 케이스의 `근거`가 가리키는 절도 같은 방식으로 [페이지 조회 목록의 자리 표시 스펙](../spec/client/paged-list-placeholder.md)에, 빈 상태 케이스의 `근거`가 가리키는 절은 [목록 빈 상태 스펙](../spec/client/list-empty-state.md)에 위임한다.
...
### TC-MEMO-HOME-FEATURE-067: 안내가 보이는 동안 이어서 동작하면 마지막 동작의 안내만 남는다

- 근거: `feature > 안내와 실행 취소`
- Given: 한 메모를 완료해 완료 안내와 실행 취소 동작이 제공되어 있다.
- When: 안내가 사라지기 전에 사용자가 다른 메모를 삭제하고, 이어서 실행 취소를 선택한다.
- Then: 완료 안내는 바로 사라지고 삭제 안내만 표시된다. 실행 취소는 삭제한 메모에만 적용되어 그 메모가 목록에 다시 나타나고, 먼저 완료한 메모는 완료 상태로 남는다.

### TC-MEMO-HOME-FEATURE-068: 완료나 삭제가 저장되지 못하면 메모가 목록에 남고 안내를 표시하지 않는다

- 근거: `feature > 안내와 실행 취소`
- Given: 미완료·미삭제 메모가 목록에 표시되어 있고, 테스트 데이터의 동작이 저장되지 못하도록 설정되어 있다.
...
### TC-MEMO-HOME-FEATURE-069: 실행 취소를 선택하지 않으면 안내가 잠시 뒤 사라지고 되돌릴 수 없다

- 근거: `feature > 안내와 실행 취소`
- Given: 메모를 완료해 완료 안내와 실행 취소 동작이 제공되어 있다.
- When: 사용자가 실행 취소를 선택하지 않고 안내가 저절로 사라질 때까지 기다린다.
- Then: 완료 안내와 실행 취소 동작이 사라지고, 완료한 메모는 완료 상태로 남는다.
...
### TC-MEMO-HOME-FEATURE-077: 메모를 선택하면 그 메모의 MemoDetail 화면으로 이동한다

- 근거: `feature > 메모 상세 확인`
- Given: 한 영역만 표시하는 환경에서 MemoHome 목록에 메모가 표시되어 있다.
- When: 사용자가 그 메모를 선택한다.
- Then: 선택한 메모의 MemoDetail 화면으로 한 번 이동한다.
...
- Given: 사용자가 MemoHome 목록을 보고 있다.
- When: 사용자가 검색을 선택한다.
- Then: 메모 검색 결과부터 보여 주는 SearchHome 화면으로 한 번 이동한다.
...
flowchart TD
    ...
    TagAxis -- "아니오" --> Combine
    Combine -- "예" --> And["모든 조건을 함께 만족해야 표시한다<br/>TC-MEMO-HOME-FEATURE-038<br/>TC-MEMO-HOME-FEATURE-040<br/>TC-MEMO-HOME-DOMAIN-021"]
...
- When: 사용자가 태그 축을 `없음`으로 고른 뒤 선택된 첫 번째 태그의 칩, 선택되지 않은 두 번째 태그의 칩과 선택 전체 해제를 실행하려고 한다.
...
- 근거: `feature > 빈 상태`
- Given: 유무 필터의 세 축이 모두 `전체`이고 태그 선택이 테스트 데이터와 같으며, 표시할 메모가 없고 목록의 준비가 끝났다.
...
| --- |
| 선택한 태그가 하나도 없다 |
| 선택한 태그가 모두 선택할 수 없게 되어 판정에서 무시되고 있다 |
...
### TC-MEMO-HOME-FEATURE-066: 태그 필터에 태그 추가 칩을 항상 표시한다
...
### TC-MEMO-HOME-FEATURE-070: TagAdd 화면에서 태그를 추가하고 돌아와도 필터가 다시 열리지 않고 추가한 태그가 선택되지 않는다

- 근거: `feature > 태그 필터`
- Given: 사용자가 태그 필터의 `태그 추가` 칩을 눌러 필터가 닫히고 TagAdd 화면으로 이동해 있다.
- When: 사용자가 TagAdd 화면에서 태그를 추가하고 메모 목록으로 돌아온다.
- Then: 필터는 다시 열리지 않고 메모 목록이 표시되며, 추가한 태그는 태그 필터에 선택되지 않는다.

### TC-MEMO-HOME-FEATURE-071: 목록을 불러오지 못해도 오류 안내 없이 보이던 메모를 유지하고, 없으면 빈 상태를 보여 준다

- 근거: `feature > 불러오기 실패`
- Given: 메모 목록을 테스트 데이터의 상황에서 불러오지 못했다.
- When: 사용자가 MemoHome 목록을 확인한다.
...
| 상황 | 결과 |
| --- | --- |
| 처음 불러오기에 실패해 보이는 메모가 없음 | 빈 상태 안내 |
| 이어서 불러오기에 실패했고 앞서 불러온 메모가 보임 | 앞서 불러온 메모가 그대로 표시됨 |

### TC-MEMO-HOME-FEATURE-072: 필터가 적용되어 있을 때만 필터를 여는 동작이 적용 상태를 알린다

- 근거: `feature > 유무 필터`, `feature > 태그 필터`, [목록 필터 반영 스펙의 필터 적용 알림](../spec/client/list-filter.md#필터-적용-알림)
- Given: MemoHome 목록의 필터 선택이 테스트 데이터와 같다.
- When: MemoHome 목록이 표시된다.
- Then: 필터를 여는 동작이 테스트 데이터의 적용 알림 여부대로 필터가 적용되어 있음을 알리거나 알리지 않는다.
- 테스트 데이터:

| 필터 선택 | 적용 알림 |
| --- | --- |
| 유무 필터의 세 축이 모두 `전체`이고 선택한 태그가 없다 | 알리지 않는다 |
| 유무 필터의 한 축이 `있음` | 알린다 |
| 유무 필터의 한 축이 `없음` | 알린다 |
| 태그를 하나 이상 선택했다 | 알린다 |
| 세 축이 모두 `전체`이고, 선택한 태그가 모두 선택할 수 없게 되어 판정에서 무시되고 있다 | 알리지 않는다 |

### TC-MEMO-HOME-FEATURE-073: 필터를 연 채 화면이 재생성되거나 앱이 백그라운드에 다녀와도 필터는 열린 채로 남는다

- 근거: `feature > 유무 필터`, `feature > 태그 필터`, [목록 필터 반영 스펙의 필터 열림 유지](../spec/client/list-filter.md#필터-열림-유지)
- Given: 사용자가 MemoHome 목록에서 필터를 열어 유무 필터의 한 축을 `있음`으로 고르고 태그 하나를 선택한 상태다.
- When: 테스트 데이터의 경계를 지난다.
- Then: 필터가 열린 채로 표시되고, 고른 축 상태와 태그 선택이 그대로 보인다.
- 테스트 데이터:
...
### TC-MEMO-HOME-FEATURE-074: 안내가 보이는 동안 화면을 떠나면 안내가 닫히고 되돌릴 수 없다

- 근거: `feature > 안내와 실행 취소`
- Given: MemoHome 목록에서 메모를 완료해 완료 안내와 실행 취소 동작이 표시되어 있다.
- When: 사용자가 MemoHome 목록을 떠났다가 다시 돌아온다.
- Then: 안내와 실행 취소 동작이 보이지 않고, 완료를 되돌리는 요청은 일어나지 않는다.

### TC-MEMO-HOME-FEATURE-075: 안내가 보이는 동안 화면이 재생성되면 안내가 닫히고 되돌릴 수 없다
...
### TC-MEMO-HOME-FEATURE-076: 안내가 보이는 동안 앱이 백그라운드에 다녀와도 안내가 남고 실행 취소할 수 있다

- 근거: `feature > 안내와 실행 취소`
- Given: MemoHome 목록에서 메모를 완료해 완료 안내와 실행 취소 동작이 표시되어 있다.
- When: 안내가 표시되는 시간이 지나기 전에 앱이 종료되지 않은 채 백그라운드에 갔다가 돌아오고, 사용자가 실행 취소를 선택한다.
- Then: 돌아온 뒤에도 완료 안내와 실행 취소 동작이 표시되어 있고, 그 메모의 완료를 되돌리는 요청이 한 번 일어난다.

### TC-MEMO-HOME-FEATURE-078: 태그를 조회하지 못하면 오류 안내 없이 태그 추가 칩만 표시한다
...
### TC-MEMO-HOME-FEATURE-079: 태그의 이모지, 제목이나 컬러가 바뀌면 필터에 반영하고 선택 여부를 유지한다
...
### TC-MEMO-HOME-FEATURE-080: 필터를 연 채로 태그가 추가되면 필터에 바로 나타난다
...
- When: 사용자가 메모를 완료한다.
- Then: 메모는 기기에 남아 있고 완료 상태로 조회된다.
...
- When: 사용자가 메모를 삭제한다.
- Then: 메모는 기기에 남아 있고 삭제 상태로 조회된다.
...
### TC-MEMO-HOME-DOMAIN-021: 태그 축이 있음이면 태그 필터를 함께 적용한다

- 근거: `domain > 필터 조합 기준`
- Given: 선택한 태그와 연결된 메모, 선택하지 않은 다른 태그에만 연결된 메모, 어느 태그와도 연결되지 않은 메모가 있고 태그 필터에서 한 태그가 선택되어 있다.
- When: 태그 축을 `있음`으로 두고 메모 목록을 확인한다.
- Then: 선택한 태그와 연결된 메모만 표시된다.
...
### TC-MEMO-HOME-DOMAIN-018: 화면이 회전하거나 재생성되어도 필터 선택, 정렬 선택과 보던 자리가 그대로다

- 근거: `domain > 실행 경계`
- Given: 유무 필터의 한 축을 `있음`으로 고르고 정렬을 최근 수정순으로 고른 목록에 한 화면에 다 들어가지 않는 수의 메모가 표시되어 있고, 사용자가 목록을 내려 뒤쪽의 메모를 보고 있다.
- When: 화면 회전처럼 시스템이 화면을 재생성한다.
- Then: 필터를 여는 동작은 여전히 필터가 적용되었음을 알리고, 정렬 컨트롤은 최근 수정순을 알리며, 재생성 전에 보던 메모가 그대로 보이고 첫 메모 카드는 보이지 않는다.

### TC-MEMO-HOME-DOMAIN-019: 앱이 백그라운드에 다녀와도 보던 자리가 그대로다

- 근거: `domain > 실행 경계`
- Given: 한 화면에 다 들어가지 않는 수의 메모가 표시되어 있고, 사용자가 목록을 내려 뒤쪽의 메모를 보고 있다.
- When: 앱이 종료되지 않은 채 백그라운드에 갔다가 돌아온다.
- Then: 보던 메모가 그대로 보이고 첫 메모 카드는 보이지 않는다.

### TC-MEMO-HOME-DOMAIN-020: 다른 화면에 다녀와도 보던 자리가 그대로다

- 근거: `domain > 실행 경계`, `feature > 검색으로 이동`
- Given: 한 화면에 다 들어가지 않는 수의 메모가 표시되어 있고, 사용자가 목록을 내려 뒤쪽의 메모를 보고 있다.
- When: 사용자가 테스트 데이터의 화면으로 이동했다가 뒤로 돌아온다.
- Then: 보던 메모가 그대로 보이고 첫 메모 카드는 보이지 않는다.
- 테스트 데이터: 메모 상세, 검색, 완료된 메모 확인

### TC-MEMO-HOME-DOMAIN-022: 시스템이 앱을 정리한 뒤 다시 만들면 정렬은 처음으로 돌아가고 보던 위치는 다시 보인다

- 근거: `domain > 실행 경계`
- Given: 정렬을 제목순으로 고른 목록에 한 화면에 다 들어가지 않는 수의 메모가 표시되어 있고, 사용자가 목록을 내려 뒤쪽의 메모를 보고 있다.
- When: 앱이 백그라운드에 있는 동안 시스템이 앱을 정리했다가 다시 만든다.
- Then: 정렬 컨트롤은 기본순을 알리고, 처음 정렬로 놓인 목록에서 떠나기 전과 같은 위치의 메모가 보이며 첫 메모 카드는 보이지 않는다.

### TC-MEMO-HOME-DOMAIN-023: 필터를 연 채 시스템이 앱을 정리한 뒤 다시 만들면 필터가 열린 채로 다시 보인다

- 근거: `domain > 실행 경계`, [목록 필터 반영 스펙의 필터 열림 유지](../spec/client/list-filter.md#필터-열림-유지)
- Given: 사용자가 MemoHome 목록에서 필터를 열어 유무 필터의 한 축을 `있음`으로 고른 상태다.
- When: 앱이 백그라운드에 있는 동안 시스템이 앱을 정리했다가 다시 만든다.
- Then: 필터가 열린 채로 표시되고, 고른 축 상태가 그대로 보인다.

### TC-MEMO-HOME-DOMAIN-024: 무시되고 있는 선택만 남으면 선택 전체 해제를 실행할 수 없고 그 선택이 유지된다

- 근거: [태그 필터 스펙의 선택 전체 해제 기준](../spec/client/tag-filter.md#선택-전체-해제-기준)
- Given: 메모 목록 필터에 선택한 태그가 하나뿐이고, 그 태그가 삭제되어 판정에서 무시되고 있다.
...
### TC-MEMO-HOME-DATA-004: 목록은 기간, 종일 여부, 시작 시점, 종료 시점, 제목 순으로 정렬해 조회한다
...
### TC-MEMO-HOME-DATA-018: 목록의 완료와 삭제는 현재 계정의 메모에만 저장된다

- 근거: `domain > 대상 메모`, `data > 상태 변경의 저장`
- Given: 현재 계정과 연결된 미완료·미삭제 메모와 다른 계정에만 연결된 미완료·미삭제 메모가 저장되어 있다.
- When: 현재 계정으로 두 메모에 테스트 데이터의 동작을 실행한다.
- Then: 현재 계정과 연결된 메모의 상태만 바뀌어 저장되고, 다른 계정에만 연결된 메모는 바뀌지 않는다.
- 테스트 데이터:
...
### TC-MEMO-HOME-DATA-017: 저장된 메모 변경을 목록에 자동으로 반영한다

- 근거: `feature > 내용 갱신`, `data > 목록 조회`
- Given: 사용자가 메모 목록을 보고 있고, 변경할 메모가 준비되어 있다.
...
- 테스트 데이터:
  ...
  | 변경 | 기대 결과 |
  | --- | --- |
  | 목록 노출 대상 메모를 추가 | 메모가 정렬 위치에 나타남 |
  | 메모의 제목 수정 | 바뀐 제목으로 표시됨 |
  | 메모의 기간 변경 | 변경된 기간의 정렬 위치로 이동 |
  | 메모를 완료 | 메모가 목록에서 사라짐 |
  | 메모를 삭제 | 메모가 목록에서 사라짐 |
...
- When: 사용자가 태그 필터에서 선택 전체 해제를 실행한 뒤, 다른 기기에서 받은 내용으로 두 번째 태그의 삭제가 풀린다.
...
### TC-MEMO-HOME-DATA-019: 선택 전체 해제는 다른 화면의 태그 필터 선택을 지우지 않는다

- 근거: [태그 필터 스펙의 필터 선택 저장](../spec/client/tag-filter.md#필터-선택-저장)
- Given: 같은 계정의 메모 목록 필터와 캘린더 필터에 같은 태그가 각각 선택되어 있다.
- When: 사용자가 메모 목록 필터에서 선택 전체 해제를 실행한다.
- Then: 메모 목록 필터의 선택은 모두 지워지고, 캘린더 필터에는 그 태그가 선택된 상태로 남는다.

## 작성하지 않는 이유

시작 날짜, 종일 여부, 시작 시각, 종료 시점, 제목이 모두 같은 메모끼리의 순서는 스펙에서 정하지 않으므로 기대 결과를 판정할 수 없어 작성하지 않는다.
