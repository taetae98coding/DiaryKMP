...
### TC-WEB-DETAIL-FEATURE-069: 화면이 재생성되면 웹 페이지를 처음부터 다시 그린다

- 근거: `feature > 진행 상태 유지`
- Given: 웹 페이지의 문서를 아래로 스크롤하고 문서 안의 링크로 이동해 두었다.
- When: 화면이 재생성된다.
- Then: 웹 페이지가 처음부터 다시 그려져 스크롤 위치와 이동한 결과가 남지 않고, 웹 페이지 요청은 추가로 일어나지 않는다.
- 작성하지 않는 이유: 문서 안의 스크롤 위치와 링크 이동은 플랫폼 웹 표시 수단 안에서만 관찰할 수 있어 플랫폼 웹 표시 수단 없이 결정적으로 판정할 수 없다. 재생성 뒤 웹 페이지를 다시 불러오지 않는 결과는 TC-WEB-DETAIL-FEATURE-013에서 자동화한다. 웹 표시 수단의 스크롤 위치와 이동 이력을 읽을 수 있는 테스트 환경이 마련되면 작성한다.
...
### TC-WEB-DETAIL-FEATURE-064: 뒤로가면 WebDetail 화면에 들어온 화면으로 돌아간다

- 근거: `feature > 뒤로가기`
- Given: 사용자가 테스트 데이터의 경로로 WebDetail 화면에 들어왔다.
- When: 사용자가 뒤로가기를 실행한다.
- Then: 테스트 데이터의 돌아간 화면이 표시된다.
- 테스트 데이터:

| 진입 경로 | 돌아간 화면 |
| --- | --- |
| WebHome의 웹 목록 | WebHome |
| SearchHome의 웹 결과 | SearchHome |
| TagDetail 웹 탭 | 웹 탭이 선택된 같은 태그의 TagDetail |
| MemoAdd 화면의 웹 입력 | MemoAdd |
| MemoDetail 화면의 웹 입력 | MemoDetail |

- 작성하지 않는 이유: 뒤로가기를 실행하면 진입하기 전 화면으로 돌아가는 동작이 한 번 실행되는 것은 TC-WEB-DETAIL-FEATURE-015에서 자동화한다. 진입 경로별로 돌아간 화면을 판정하려면 진입 화면마다 다른 기능의 화면과 전환 이력을 함께 구성해야 하므로 현재 테스트 환경에서는 결정적으로 검증할 수 없다. 여러 기능의 화면과 전환 이력을 함께 구성할 수 있는 테스트 환경이 갖춰지면 자동화한다.
...
### TC-WEB-DETAIL-FEATURE-066: 응답 본문 방식에서는 가져오기가 시작되어도 표시가 바뀌지 않고 URL 방식으로 바꾸면 진행 상태를 거친다

- 근거: `domain > 화면 상태 전이`
- Given: 응답 본문 방식에서 불러오기 결과가 표시되어 있고 로그인 정보 가져오는 상태가 `가져오기 성공`이다.
- When: 가져오는 상태가 `가져오는 중`으로 바뀐 뒤, 가져오는 중인 채로 사용자가 표시 방식을 URL 방식으로 바꾼다.
- Then: 가져오는 상태가 바뀌어도 응답 본문 방식의 표시가 그대로 남고, URL 방식으로 바꾸면 웹 페이지 자리에 진행 상태가 표시되며 웹 표시 수단은 아직 주소를 열지 않는다.

### TC-WEB-DETAIL-FEATURE-056: 화면에 처음 진입하면 웹 페이지가 보이는 상태로 시작하고 메모 탭은 선택되어 있지 않다

- 근거: `feature > 메모 탭`
- Given: 웹 항목이 저장되어 있고 화면 배치가 테스트 데이터와 같다.
- When: 사용자가 그 웹 항목의 WebDetail 화면에 진입한다.
- Then: 테스트 데이터의 탭이 선택된 상태로 웹 페이지가 보이고, 메모 탭은 선택되어 있지 않다.
- 테스트 데이터:

| 화면 배치 | 선택된 탭 |
| --- | --- |
| 웹 페이지를 따로 선택하는 탭으로 두는 배치 | 웹 페이지 탭 |
| 웹 페이지를 늘 함께 표시하는 배치 | 웹 정보 수정 탭 |
...
### TC-WEB-DETAIL-FEATURE-068: 화면 재생성 후에도 선택한 웹 정보 수정 탭이나 웹 페이지 탭이 유지된다

- 근거: `feature > 진행 상태 유지`, `domain > 선택한 탭`
- Given: 웹 페이지를 따로 선택하는 탭으로 두는 배치에서 사용자가 테스트 데이터의 탭을 선택했다.
- When: 화면이 재생성된다.
- Then: 테스트 데이터의 탭이 선택된 상태가 유지된다.
- 테스트 데이터:
...
### TC-WEB-DETAIL-FEATURE-067: 웹 페이지가 화면에서 빠졌다가 다시 보이면 처음부터 다시 그린다

- 근거: `feature > 메모 탭`
- Given: 웹 페이지를 따로 선택하는 탭으로 두는 배치에서 웹 페이지 탭의 문서를 아래로 스크롤하고 문서 안의 링크로 이동해 두었다.
- When: 사용자가 다른 탭으로 바꿨다가 웹 페이지 탭으로 돌아온다.
- Then: 웹 페이지가 처음부터 다시 그려져 스크롤 위치와 이동한 결과가 남지 않고, 웹 페이지 요청은 추가로 일어나지 않는다.
- 작성하지 않는 이유: 문서 안의 스크롤 위치와 링크 이동은 플랫폼 웹 표시 수단 안에서만 관찰할 수 있어 플랫폼 웹 표시 수단 없이 결정적으로 판정할 수 없다. 탭을 오가도 웹 페이지를 다시 불러오지 않는 결과는 TC-WEB-DETAIL-FEATURE-059에서 자동화한다. 웹 표시 수단의 스크롤 위치와 이동 이력을 읽을 수 있는 테스트 환경이 마련되면 작성한다.
...
- 근거: `domain > 상세 대상`
- Given: 대상 웹 항목이 테스트 데이터의 상태로 저장되어 있다.
...
### TC-WEB-DETAIL-DOMAIN-050: 태그 연결이 다른 경로에서 바뀌면 태그 입력에 최신 연결을 반영한다

- 근거: `domain > 외부 변경`
- Given: 대상 웹 항목이 조회되어 태그 입력에 저장된 연결이 표시되어 있다.
- When: 같은 웹 항목에 태그 하나가 다른 경로에서 연결된다.
- Then: 사용자가 아무것도 하지 않아도 태그 입력에 새로 연결된 태그가 표시된다.

### TC-WEB-DETAIL-DOMAIN-051: 백그라운드에 다녀와도 선택한 탭과 고른 표시 방식을 유지한다

- 근거: `domain > 선택한 탭`, `domain > 표시 방식의 기본값과 유지`
- Given: 사용자가 메모 탭을 선택하고 표시 방식을 응답 본문 방식으로 바꿔 두었다.
- When: 앱이 백그라운드에 갔다가 같은 WebDetail 화면으로 돌아온다.
- Then: 메모 탭이 선택된 상태와 응답 본문 방식이 그대로 유지되고 웹 페이지를 다시 불러오지 않는다.
...
### TC-WEB-DETAIL-DOMAIN-007: 저장 내용이 다른 경로에서 바뀌면 URL 방식은 새 주소를 열고 응답 본문 방식은 다시 불러오지 않는다

- 근거: `domain > 외부 변경`
- Given: 테스트 데이터의 표시 방식으로 웹 페이지가 표시되어 있다.
...
| 표시 방식 | 결과 |
| --- | --- |
| 응답 본문 방식에서 웹 페이지를 한 번 불러와 본문이 표시되어 있다 | 표시하던 본문이 그대로 남고 웹 페이지 요청이 추가로 일어나지 않는다 |
| URL 방식 | 웹 표시 수단이 다른 경로에서 저장된 새 URL을 연다 |

### TC-WEB-DETAIL-DOMAIN-048: 다른 경로에서 URL이 바뀐 뒤 제목만 고쳐 수정해도 최신 저장 URL로 다시 불러온다

- 근거: `domain > 외부 변경`, `domain > 불러오기 시점`
- Given: 응답 본문 방식에서 웹 페이지를 한 번 불러와 본문이 표시된 뒤, 대상 웹 항목의 URL이 다른 경로에서 다른 값으로 저장되었고, 수정이 성공하도록 설정되어 있다.
- When: 사용자가 제목만 다른 값으로 바꿔 수정을 실행한다.
- Then: 다른 경로에서 저장된 최신 URL로 웹 페이지 요청이 한 번 일어난다.
...
### TC-WEB-DETAIL-DOMAIN-049: 다른 주소로 옮겨 가라는 응답을 따라가 마지막 응답을 결과로 삼는다

- 근거: `domain > 불러오기 대상`
- Given: 저장된 URL로 요청하면 서버가 다른 주소로 옮겨 가라고 응답하고, 옮겨 간 주소는 본문을 담은 응답을 내도록 설정되어 있다.
- When: 저장된 URL로 웹 페이지를 불러온다.
- Then: 옮겨 간 주소로 요청이 이어지고, 그 주소에서 받은 본문이 결과가 되며 응답을 받은 주소는 옮겨 간 주소다.
...
### TC-WEB-DETAIL-DOMAIN-009: 서버가 오류를 알린 응답도 불러오기 성공으로 다룬다
...
### TC-WEB-DETAIL-DOMAIN-013: 응답 본문의 상대 경로는 응답을 받은 주소를 기준 주소로 해석한다

- 근거: `domain > 표시하는 웹 페이지`
- Given: 상대 경로 리소스를 포함한 응답 본문을 받았다.
- When: 웹 페이지 자리에 그 본문이 표시된다.
- Then: 상대 경로가 응답을 받은 주소를 기준 주소로 삼아 해석되어 요청되고, 그 요청에는 대상 웹 항목의 요청 헤더가 담기지 않는다.
...
### TC-WEB-DETAIL-DOMAIN-040: URL 방식에서 수정으로 URL이 바뀌면 응답 본문 방식을 연 적이 있을 때만 곧바로 다시 불러온다

- 근거: `domain > 불러오기 시점`, `feature > 수정 뒤의 웹 페이지`
- Given: URL 방식으로 표시되어 있고, 응답 본문 방식을 연 이력이 테스트 데이터와 같으며, 수정이 성공하도록 설정되어 있다.
- When: 사용자가 URL을 다른 값으로 바꿔 수정을 실행한다.
- Then: 테스트 데이터의 결과대로 웹 페이지 요청이 일어난다.
- 테스트 데이터:

| 응답 본문 방식을 연 이력 | 결과 |
| --- | --- |
| 화면에 들어온 뒤 한 번도 열지 않았다 | 웹 페이지 요청이 일어나지 않는다 |
| 응답 본문 방식에서 한 번 불러온 뒤 URL 방식으로 되돌렸다 | URL 방식에 있는 채로 새로 저장된 URL로 웹 페이지 요청이 곧바로 한 번 일어나고, 응답 본문 방식으로 돌아오면 다시 불러온 결과가 표시된다 |
...
- Then: 메모 탭이 선택되지 않고 TC-WEB-DETAIL-FEATURE-056의 처음 선택으로 시작한다.
...
### TC-WEB-DETAIL-DATA-001: 현재 계정과 고른 웹 항목을 기준으로 기기에 저장된 웹 항목에서 찾는다

- 근거: `data > 대상 웹 항목 조회`
- Given: 현재 계정과 연결된 대상 웹 항목이 요청 헤더 여러 개와 함께 기기에 저장되어 있다.
- When: 상세 대상 웹 항목을 조회한다.
...
### TC-WEB-DETAIL-DATA-014: 수정은 제목, 설명, URL, 요청 헤더와 수정 시각을 기기에 저장한다

- 근거: `data > 수정의 저장`
- Given: 현재 계정과 연결된 웹 항목이 요청 헤더와 함께 기기에 저장되어 있다.
- When: 그 웹 항목의 제목, 설명, URL과 요청 헤더를 다른 값으로 수정한다.
- Then: 네 값과 수정 시각이 기기에 덮어써지고, 계정 연결과 생성 시각은 바뀌지 않는다.

### TC-WEB-DETAIL-DATA-015: 헤더를 모두 지운 수정을 반영하면 요청 헤더가 남지 않는다

- 근거: `data > 수정의 저장`
- Given: 요청 헤더 두 개를 가진 웹 항목이 기기에 저장되어 있다.
- When: 요청 헤더를 하나도 두지 않고 수정을 반영한다.
- Then: 그 웹 항목을 다시 조회하면 요청 헤더가 하나도 없다.

### TC-WEB-DETAIL-DATA-016: 현재 계정에 연결된 대상 웹 항목에만 수정을 반영한다

- 근거: `data > 수정의 저장`
- Given: 현재 계정의 웹 항목, 다른 계정의 웹 항목이 기기에 저장되어 있다.
...
| 저장되지 않은 웹 항목 | 어떤 웹 항목도 바뀌지 않는다 |

### TC-WEB-DETAIL-DATA-004: 저장된 URL과 요청 헤더로 그 주소에 직접 요청한다
...
- 근거: `data > 웹 페이지 요청`
- Given: 응답 본문 방식에서 웹 페이지를 한 번 불러와 본문을 받은 뒤 WebDetail 화면을 떠났다.
- When: 같은 웹 항목의 WebDetail 화면에 다시 진입해 응답 본문 방식을 고른다.
- Then: 다시 진입한 화면은 이전에 받은 본문 없이 시작하고, 응답 본문 방식을 고르면 웹 페이지 요청이 새로 일어난다.
...
### TC-WEB-DETAIL-DATA-008: 삭제는 삭제 여부와 수정 시각을 기기에 저장한다

- 근거: `data > 삭제의 저장`
- Given: 현재 계정과 연결된 미삭제 웹 항목이 요청 헤더와 함께 기기에 저장되어 있다.
- When: 그 웹 항목의 삭제를 실행한다.
- Then: 그 웹 항목의 삭제 여부와 수정 시각이 기기에 저장되고, 웹 항목 자체와 계정 연결, 요청 헤더는 기기에서 지워지지 않는다.

### TC-WEB-DETAIL-DATA-009: 현재 계정에 연결된 대상 웹 항목에만 삭제를 반영한다
...
### TC-WEB-DETAIL-DATA-010: 저장에 실패한 동작은 성공으로 다루지 않는다

- 근거: `data > 저장 실패`
- Given: 기기 저장이 실패하도록 설정되어 있다.
- When: 테스트 데이터의 동작을 실행한다.
- Then: 그 동작이 성공한 것으로 다뤄지지 않고, 기기에 저장된 대상 웹 항목은 그 동작이 반영되지 않은 채로 남는다.
- 테스트 데이터:
...
- When: 테스트 데이터의 동작을 실행한다.
- Then: 그 웹 항목이 계정의 업로드 대기 상태로 기록되고 화면 뒤에서 동기화가 한 번 요청된다.
- 테스트 데이터:
...
### TC-WEB-DETAIL-DATA-012: 성공 여부는 기기에 저장한 결과로만 판단한다
...
- Then: 연결 대상 표시에는 연결된 완료 태그가 나누지 않고 한 번에 나타나고, 태그 선택 목록에는 미완료·미삭제 태그와 연결된 완료 태그가 나누어 불러와지며 함께 나타난다.
...
### TC-WEB-DETAIL-DATA-022: URL 방식에서는 앱이 웹 페이지를 직접 요청하지 않는다

- 근거: `data > 웹 페이지 요청`
- Given: URL과 요청 헤더를 가진 대상 웹 항목이 조회된다.
- When: WebDetail 화면이 URL 방식으로 표시된 채 유지된다.
- Then: 앱이 직접 보내는 웹 페이지 요청이 한 번도 나가지 않는다.
