...
이 문서에서 `domain > 추가 항목의 동작 판정`, `domain > 추가한 태그의 자동 선택`, `domain > 태그 검색어`, `domain > 태그 선택 목록의 열림 상태`, `feature > 선택 상태 표시`, `feature > 태그 검색`, `feature > 태그 상세 이동`, `feature > 태그 선택 목록`, `feature > 태그 선택과 해제`, `feature > 태그 추가 이동` 절은 [태그 선택 입력 공통 스펙](../spec/tag-select-input.md)이 소유한다. 나머지 케이스의 절은 기준 스펙이 소유한다.
...
- 근거: `feature > 태그 선택과 해제`, `feature > 연결 상태 표시`
...
### TC-TAG-LINK-INPUT-FEATURE-019: 검색어를 입력하면 목록이 즉시 좁혀진다
...
- 작성하지 않는 이유: 확정 동작 없이 입력한 검색어가 목록 조회에 반영되는 것까지는 화면 테스트로 검증하지만, 좁혀진 목록이 화면에 나타나는 것은 검증하지 않는다. 화면 테스트 환경에서 선택 목록이 두 번째 조회 결과를 받지 못해 같은 조건에서 결과가 일정하지 않다. 검색어를 만족하는 태그만 조회된다는 결과는 `TC-TAG-LINK-INPUT-DOMAIN-007`과 `TC-TAG-LINK-INPUT-DATA-004`로, 검색어가 바뀌면 다시 조회한다는 결과는 `TC-TAG-LINK-INPUT-DATA-005`로 검증한다. 화면 테스트에서 이어지는 조회 결과를 받을 수 있게 되면 작성한다.
...
- 작성하지 않는 이유: 검색어를 지운 상태가 목록 조회에 반영되는 것까지는 화면 테스트로 검증하고, 다시 나타난 목록은 `TC-TAG-LINK-INPUT-FEATURE-019`와 같은 이유로 검증하지 않는다. 빈 검색어가 목록을 좁히지 않는다는 결과는 `TC-TAG-LINK-INPUT-DOMAIN-008`로 검증한다.
...
### TC-TAG-LINK-INPUT-DOMAIN-010: 화면이 재생성되어도 열려 있는 목록의 검색어가 유지된다
