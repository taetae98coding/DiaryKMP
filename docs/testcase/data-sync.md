...
- 근거: `domain > 동기화 대상`
- Given: 계정이 게스트 상태다.
- When: 메모, 태그, 장소, 웹 항목, 곡 또는 여섯 연결 종류에 변경이 발생한다.
...
### TC-DATA-SYNC-DOMAIN-068: JVM 데스크톱과 웹에서 앱이 포커스를 다시 얻으면 동기화를 다시 요청한다

- 근거: `domain > 동기화 계기`
- Given: JVM 데스크톱 또는 웹에서 인증된 계정이 확인되어 동기화가 한 번 요청된 뒤, 앱 창이나 브라우저 탭은 보이는 채로 포커스를 잃었다.
- When: 사용자가 앱으로 돌아와 앱 창이나 브라우저 탭이 포커스를 다시 얻는다.
- Then: 같은 계정으로 동기화가 한 번 더 요청된다.

### TC-DATA-SYNC-DOMAIN-069: JVM 데스크톱과 웹에서 앱이 보이기만 하고 포커스가 없는 동안에는 동기화를 요청하지 않는다

- 근거: `domain > 동기화 계기`
- Given: JVM 데스크톱 또는 웹에서 앱 창이나 브라우저 탭이 보이지만 포커스는 다른 창이나 탭에 있다.
- When: 인증된 계정이 확인되거나 확인된 계정이 다른 계정으로 바뀐다.
...
### TC-DATA-SYNC-DOMAIN-065: JVM 데스크톱과 웹은 앱을 다시 실행하면 4시간을 새로 센다

- 근거: `domain > 주기 동기화`
- Given: JVM 데스크톱 또는 웹에서 인증된 계정으로 주기 동기화가 예약된 뒤 앱이 종료되었다.
...
### TC-DATA-SYNC-DOMAIN-070: 마지막 동기화에서 30일이 지난 계정만 강제 전체 재동기화를 시작한다

- 근거: `domain > 강제 전체 재동기화`
...
### TC-DATA-SYNC-DOMAIN-071: 마지막 동기화 시각은 계정마다 따로 판단된다
...
### TC-DATA-SYNC-DOMAIN-072: 업로드와 내려받기가 모두 끝났을 때만 마지막 동기화 시각을 갱신한다
...
- 근거: `domain > 강제 전체 재동기화`, `domain > 동기화 계기`
- Given: 인증된 사용자 계정이 있고 그 계정의 마지막 동기화 시각이 31일 전으로 기록되어 있다.
...
- 근거: `domain > 강제 전체 재동기화`
- Given: 인증된 사용자 계정의 마지막 실행 시각이 31일 전으로 기록되어 있고, 그 계정에 테스트 데이터가 기기에 저장되어 있다.
...
- 근거: `domain > 강제 전체 재동기화`
- Given: 인증된 사용자 계정의 마지막 실행 시각이 31일 전으로 기록되어 있고, 테스트 데이터가 기기에 함께 저장되어 있다.
- When: 앱이 시작되어 그 계정이 확인되고 강제 전체 재동기화의 삭제 단계가 끝난다.
...
- 근거: `domain > 강제 전체 재동기화`, `domain > 동기화 방향`
- Given: 인증된 사용자 계정의 마지막 실행 시각이 31일 전으로 기록되어 있고, 그 계정의 열세 종류에 내려받기 위치가 이미 기록되어 있다.
- When: 앱이 시작되어 그 계정이 확인되고 강제 전체 재동기화가 진행된다.
- Then: 열세 종류의 첫 내려받기 요청이 모두 기록되어 있던 위치가 아니라 기본 커서로 전달된다.
...
- 서버가 삭제된 상태로 60일이 지난 항목을 영구히 제거하는 동작, 항목이 영구히 제거될 때 그 항목을 가리키는 연결을 함께 제거하는 동작, 태그가 영구히 제거될 때 대표 태그 지정을 해제하고 서버의 변경으로 기록하는 동작은 모두 서버 저장소의 정리 규칙이어서 앱의 단위 테스트 환경에서 결정적으로 판정할 수 없다. 서버 정책 테스트 환경이 마련되면 작성한다.
- 이미 영구히 제거된 태그를 대표 태그로 지정한 메모의 업로드를 서버가 해제로 반영하는 동작도 서버 검증 규칙이어서 같은 이유로 작성하지 않는다. 기기가 해제된 대표 태그를 내려받아 반영하는 결과는 [메모 대표 태그 테스트 케이스](./memo-primary-tag.md)의 내려받기 반영 케이스에서 검증한다.
- 앱을 종료하지 않고 30일 넘게 켜 둔 기기가 판정에 걸리지 않는다는 규칙은 앱을 다시 실행하지 않는 상태 자체가 조건이어서, 앱 실행 계기에서만 판정한다는 케이스와 구분되는 별도의 관찰 결과를 만들 수 없다.
