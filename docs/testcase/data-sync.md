...
- Given: 로그인한 사용자 계정에 업로드할 열두 종류가 모두 여러 묶음씩 있다.
...
| --- | --- |
| 태그만 | 태그만 요청, 나머지 열한 종류 요청 없음 |
| 장소만 | 장소만 요청, 나머지 열한 종류 요청 없음 |
| 웹 항목만 | 웹 항목만 요청, 나머지 열한 종류 요청 없음 |
| 연락처만 | 연락처만 요청, 나머지 열한 종류 요청 없음 |
| 곡만 | 곡만 요청, 나머지 열한 종류 요청 없음 |
| 메모만 | 메모만 요청, 나머지 열한 종류 요청 없음 |
| 메모와 태그의 연결만 | 그 연결만 요청, 나머지 열한 종류 요청 없음 |
| 메모와 장소의 연결만 | 그 연결만 요청, 나머지 열한 종류 요청 없음 |
| 메모와 웹 항목의 연결만 | 그 연결만 요청, 나머지 열한 종류 요청 없음 |
| 태그와 태그의 연결만 | 그 연결만 요청, 나머지 열한 종류 요청 없음 |
| 웹 항목과 태그의 연결만 | 그 연결만 요청, 나머지 열한 종류 요청 없음 |
| 장소와 태그의 연결만 | 그 연결만 요청, 나머지 열한 종류 요청 없음 |
| 없음 | 열두 종류 요청 모두 없음 |
| 열두 종류 모두 | 열두 종류 모두 요청 |
...
### TC-DATA-SYNC-DOMAIN-038: 업로드가 끝난 뒤 열두 종류를 동시에 내려받는다

- 근거: `domain > 내려받기 병렬 처리`
- Given: 로그인한 사용자 계정의 업로드가 모두 성공했고 서버에 내려받을 열두 종류가 모두 있으며, 한 종류의 첫 내려받기 응답이 지연되도록 제어되어 있다.
- When: 동기화가 진행된다.
- Then: 모든 업로드 요청이 끝난 뒤, 지연된 종류의 내려받기가 완료되기 전에 나머지 열한 종류의 첫 내려받기 요청이 시작된다.
- 테스트 데이터:
...
| 곡 | 나머지 열한 종류 |
| 메모 | 나머지 열한 종류 |
| 메모와 태그의 연결 | 나머지 열한 종류 |
| 메모와 장소의 연결 | 나머지 열한 종류 |
| 메모와 웹 항목의 연결 | 나머지 열한 종류 |
| 태그와 태그의 연결 | 나머지 열한 종류 |
| 웹 항목과 태그의 연결 | 나머지 열한 종류 |
| 장소와 태그의 연결 | 나머지 열한 종류 |

### TC-DATA-SYNC-DOMAIN-039: 한 종류의 내려받기가 실패해도 다른 종류는 끝까지 진행한다

- 근거: `domain > 내려받기 병렬 처리`
- Given: 업로드가 모두 성공했고, 한 종류의 내려받기 요청이 실패하며 다른 열한 종류는 여러 묶음을 응답하도록 제어되어 있다.
- When: 동기화가 진행된다.
- Then: 실패하지 않은 열한 종류는 빈 응답을 받을 때까지 모든 묶음을 내려받아 기기에 반영하고 그 종류의 내려받기 위치가 전진한다.
- 테스트 데이터:
...
- 근거: `data > 내려받기 흐름`
- Given: 같은 계정의 열두 종류에 서로 다른 내려받기 위치가 기록되어 있고, 서버가 각 종류에 서로 다른 순번의 항목을 응답하도록 제어되어 있다.
- When: 열두 종류의 내려받기가 동시에 진행되어 모두 끝난다.
...
- Given: 인증된 사용자 계정의 마지막 실행 시각이 31일 전으로 기록되어 있고, 그 계정의 열두 종류에 내려받기 위치가 이미 기록되어 있다.
- When: 앱이 시작되어 그 계정이 확인되고 강제 전체 재동기화가 진행된다.
- Then: 열두 종류의 첫 내려받기 요청이 모두 기록되어 있던 위치가 아니라 기본 커서로 전달된다.
