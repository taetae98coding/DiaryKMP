...
- 작성하지 않는 이유: 아직 준비되지 않은 구간이 있는 목록 상태를 만들려면 기기에 저장된 장소를 나누어 이어서 불러오는 환경이 필요하고, 현재 호스트 테스트 환경은 화면에 전달할 목록을 직접 구성하므로 이 상태를 결정적으로 만들 수 없다. 이어서 불러오는 시점을 제어할 수 있는 테스트 환경이 제공되면 자동화한다.
