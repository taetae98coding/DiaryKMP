...
### TC-MEMO-PLACE-CARD-FEATURE-042: 목록을 연 적이 없어도 화면에 들어온 뒤 대상이 없다고 확인되면 첫 누름에 PlaceAdd 이동을 요청한다

- 근거: `domain > 추가 항목의 동작 판정`
- Given: 계정과 연결되고 삭제되지 않은 장소가 하나도 저장되어 있지 않은 상태로 화면에 들어왔고, 장소 선택 목록을 한 번도 열지 않은 채 선택할 수 있는 장소가 없다는 확인이 끝났다.
- When: 사용자가 추가 항목을 처음 누른다.
- Then: 장소 선택 목록이 열리지 않고 PlaceAdd 화면으로의 이동이 한 번 요청된다.
...
### TC-MEMO-PLACE-CARD-FEATURE-038: 목록을 닫았다가 다시 열면 앞부분부터 나타난다

- 근거: `feature > 장소 선택 목록`
- Given: 한 화면에 모두 담기지 않는 수의 선택할 수 있는 장소가 있고, 사용자가 장소 선택 목록을 열어 뒷부분까지 내려 본 상태다.
...
- Then: 그 구간의 장소가 정렬 순서에 이어서 나타난다.
- 작성하지 않는 이유: 목록을 끝으로 옮긴 뒤 다음 장소가 준비되는 과정은 화면 테스트 환경에서 배경 조회가 끝나는 시점을 제어할 수 없어 같은 조건에서 결과가 일정하지 않다. 조회가 정렬 순서 앞쪽부터 나뉘어 이루어지고 이어지는 조회가 그다음 장소를 가져온다는 결과는 `TC-MEMO-PLACE-CARD-DATA-005`로 검증한다. 화면 테스트에서 배경 조회 완료 시점을 제어할 수 있게 되면 작성한다.

### TC-MEMO-PLACE-CARD-FEATURE-039: 다음 장소를 불러오는 동안에도 이미 나타난 장소를 조작할 수 있다

- 근거: `feature > 장소 선택 목록`
- Given: 장소 선택 목록이 열려 있고 다음 구간의 조회가 끝나지 않도록 제어되어 있다.
- When: 사용자가 이미 나타난 장소를 누른다.
- Then: 그 장소의 선택 여부가 바뀐다.

### TC-MEMO-PLACE-CARD-FEATURE-040: 목록의 장소를 눌러도 장소 상세로 이동하지 않는다

- 근거: `feature > 장소 선택 목록`
- Given: 장소 선택 목록이 열려 있고 장소가 나타나 있다.
- When: 사용자가 목록의 장소를 누른다.
- Then: 장소 상세 이동이 요청되지 않고 그 장소의 선택 여부만 바뀐다.
...
- Then: 그 핀이 가리키는 장소로 장소 상세 이동이 한 번 요청되고, 지도가 보고 있는 위치와 확대 수준, 지도 제공자와 카드의 선택 상태는 바뀌지 않는다.
...
### TC-MEMO-PLACE-CARD-FEATURE-037: 목록의 장소를 눌러도 장소 상세로 이동하지 않고 선택이 바뀌지 않는다

- 근거: `feature > 장소 위치 보기`, `feature > 장소 상세 이동`
- Given: 카드가 장소 여러 개를 노출하고 있다.
- When: 사용자가 카드의 장소 목록에서 장소 하나를 누른다.
- Then: 장소 상세 이동이 요청되지 않고 장소 선택 목록도 열리지 않으며, 카드가 노출하는 장소는 누르기 전과 같다.

### TC-MEMO-PLACE-CARD-FEATURE-041: 선택한 장소의 제목이나 컬러가 바뀌면 카드의 장소 목록에 반영된다

- 근거: `feature > 장소 상세 이동`, `data > 내용 갱신`
- Given: 장소 하나가 선택되어 카드의 장소 목록에 표시되어 있다.
- When: 그 장소의 저장된 값이 테스트 데이터처럼 다른 경로로 바뀐다.
- Then: 별도의 새로고침 없이 카드의 장소 목록에 바뀐 값이 표시된다.
- 테스트 데이터:
...
### TC-MEMO-PLACE-CARD-DOMAIN-029: 카드에 노출하는 장소는 제목 오름차순으로 놓인다

- 근거: `domain > 카드가 노출하는 장소`
- Given: 제목이 서로 다른 장소 여러 개가 선택되어 있거나 메모에 연결되어 있고, 선택하거나 연결한 순서가 제목 순서와 다르다.
- When: 카드에 노출하는 장소를 확인한다.
- Then: 장소가 선택하거나 연결한 순서와 관계없이 제목 오름차순으로 놓인다.
- 테스트 데이터:

| 화면 | 노출하는 장소 |
| --- | --- |
| MemoAdd | 추가할 메모에 연결할 장소로 선택한 장소 |
| MemoDetail | 상세 대상 메모에 연결된 장소 |
...
### TC-MEMO-PLACE-CARD-DOMAIN-013: 노출하는 장소와 현재 위치를 모두 확인하기 전에는 지도를 표시하지 않는다

- 근거: `domain > 초기 지도 위치`
- Given: 기본 지도는 확인되었고, 노출하는 장소와 현재 위치의 확인 상태가 테스트 데이터와 같다.
- When: 사용자가 장소 카드를 확인한다.
- Then: 지도가 표시되지 않는다. 확인 중이던 쪽의 확인이 끝나 두 확인이 모두 끝나면 지도가 표시된다.
- 테스트 데이터:

| 노출하는 장소 | 현재 위치 |
| --- | --- |
| 아직 확인하지 못했다 | 확인했다 |
| 확인했고 노출하는 장소가 있다 | 아직 확인 중이다 |

- 작성하지 않는 이유: 지도가 표시되지 않는 절반만 자동화했다. 두 확인이 끝난 뒤 지도가 표시되는 것을 관찰하려면 지도 제공자의 실제 표시 요소가 필요하고, 현재 호스트 테스트 환경은 이를 제공하지 않는다. 지도 표시를 대체할 수 있는 테스트 환경이 제공되면 나머지 절반을 자동화한다.
...
### TC-MEMO-PLACE-CARD-DOMAIN-022: 검색어는 카드에 노출하는 장소를 좁히지 않는다

- 근거: `domain > 장소 검색어`
- Given: 선택한 장소 두 개가 카드의 장소 목록에 표시된 상태로 장소 선택 목록이 열려 있다.
- When: 사용자가 그중 한 장소만 만족하는 검색어를 입력한다.
- Then: 목록에는 그 한 장소만 남고, 카드의 장소 목록에는 선택한 두 장소가 모두 그대로 표시된다.

### TC-MEMO-PLACE-CARD-DOMAIN-031: 검색어는 지도 핀과 지도가 보고 있는 위치를 바꾸지 않는다

- 근거: `feature > 장소 검색`, `domain > 장소 검색어`
- Given: 기본 지도가 확인되어 선택한 장소 두 개가 지도 핀으로 표시된 상태로 장소 선택 목록이 열려 있다.
- When: 사용자가 그중 한 장소만 만족하는 검색어를 입력한다.
- Then: 지도 핀에는 선택한 두 장소가 모두 그대로 표시되고, 지도가 보고 있는 위치와 확대 수준도 바뀌지 않는다.
- 작성하지 않는 이유: 핀이 지도 기능에 지정되는 것과 지도가 보고 있는 위치는 지도가 표시된 화면을 구성해야 관찰할 수 있는데, 현재 호스트 테스트 환경은 실제 지도의 플랫폼 표시 요소를 제공하지 않는다. 핀이 카드에 노출하는 장소와 같다는 것은 `TC-MEMO-PLACE-CARD-DOMAIN-022`와 `TC-MEMO-PLACE-CARD-FEATURE-018`의 변환 검증으로 나누어 덮는다. 지도 표시를 대체할 수 있는 테스트 환경이 제공되면 자동화한다.
...
### TC-MEMO-PLACE-CARD-DOMAIN-030: 메모리 정리 뒤 복원하면 검색어가 다시 나타나고 기다리지 않고 그 검색어로 좁힌 목록을 보여 준다

- 근거: [검색어 일치 판정 스펙의 검색어 반영 시점](../spec/client/search-match.md#검색어-반영-시점), `domain > 장소 검색어`, `domain > 진행 상태 유지`
- Given: 장소 선택 목록이 열려 있고 일부 장소만 남도록 검색어가 입력되어 있다.
...
- When: 기기에 저장된 장소가 테스트 데이터처럼 바뀐다.
...
- 근거: `data > 장소 목록 조회`
- Given: 로그인한 계정에 선택할 수 있는 장소가 한 번에 불러오는 분량보다 많이 저장되어 있고, 검색어를 만족하는 장소는 정렬 순서 뒤쪽에 하나만 있다.
- When: 장소 선택 목록에 그 검색어가 적용된다.
- Then: 검색어를 만족하는 그 장소가 처음 불러온 목록에 나타난다.

### TC-MEMO-PLACE-CARD-DATA-004: 검색어가 바뀌면 목록의 처음부터 다시 불러온다

- 근거: `data > 장소 목록 조회`
- Given: 로그인한 계정에 검색어를 만족하는 장소가 한 번에 불러오는 분량보다 많이 저장되어 있고, 사용자가 목록의 뒤쪽 구간까지 확인한 상태다.
- When: 사용자가 검색어를 다른 값으로 바꾼다.
- Then: 새 검색어를 만족하는 장소가 정렬 순서 앞쪽부터 조회되어 목록의 처음부터 나타난다.

### TC-MEMO-PLACE-CARD-DATA-005: 장소 선택 목록을 나누어 불러온다

- 근거: `data > 장소 목록 조회`
- Given: 한 번에 준비되는 수보다 많은 장소가 계정과 연결되어 저장되어 있다.
- When: 장소 선택 목록을 열고 끝까지 내린다.
- Then: 처음에는 정렬 순서의 첫 구간만 조회되고, 목록을 내릴 때 다음 구간이 이어서 조회된다.
