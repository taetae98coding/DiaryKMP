...
- 근거: `feature > 태그 선택과 해제`, `feature > 태그 연결과 해제`, `feature > 선택 상태 표시`
...
- 근거: `feature > 태그 선택과 해제`, `feature > 태그 연결과 해제`
...
- 근거: [항목 태그 연결 공통 스펙의 대표 태그](../spec/common/entity-tag.md#대표-태그)
- Given: 목록에 나타나는 태그가 여러 개인 상태로 태그 선택 목록이 열려 있다.
- When: 사용자가 목록을 확인한다.
...
### TC-ENTITY-TAG-INPUT-DOMAIN-017: 메모리 정리 뒤 복원하면 검색어가 다시 나타나고 기다리지 않고 그 검색어로 좁힌 목록을 보여 준다
