...
JVM 데스크톱의 앱 창은 두 영역을 좌우로 함께 표시하는 1080×920dp 크기로 연다. 사용자는 창을 최소 360×784dp까지 줄일 수 있으며, compact 너비로 줄이면 한 영역만 표시하는 배치가 된다.
...
상세 영역의 화면에서 다른 상세로 이어 이동한 화면이 기준 스펙의 `함께 사용할 때의 행동`에 따라 이전 상세로 돌아가는 동작을 제공하면, 그 화면의 상단 바에는 단독 표시할 때와 같은 자리에 같은 뒤로가기 버튼을 표시한다. 어느 화면이 이 버튼을 표시하는지는 각 화면 디자인이 정한다.
