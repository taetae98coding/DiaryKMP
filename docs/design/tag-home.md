...
화면 오른쪽 아래에 플러스 아이콘을 표시하는 떠 있는 태그 추가 버튼을 둔다. 버튼은 아이콘만 표시하고, 이름을 알려 주는 설명 표시는 [아이콘 버튼 설명 디자인](./icon-button-tooltip.md)을 따른다. 누르면 TagAdd 화면을 표시한다.

물리 키보드 환경에서는 태그 목록 화면에서 `Command + A`를 눌러 태그 추가 버튼과 같은 동작을 실행한다.

목록과 상세를 함께 표시하는 동안 버튼과 단축키를 제공하는 기준은 [목록·상세 배치 공통 디자인](./list-detail-pane.md)의 `버튼 노출`을 따른다. 목록이 비어 있어도, 필터를 켜 두어도 버튼은 같은 자리에 같은 모양으로 표시한다.
...
행 전체를 하나의 조작 영역으로 제공해 라벨을 눌러도 켜짐과 꺼짐이 바뀌게 하고, 행은 최소 터치 크기인 `48dp` 이상의 높이를 유지한다. 스위치는 행과 따로 누르는 대상으로 두지 않는다. 행 안의 내용을 맞추는 좌우 자리와 누르는 동안 드러나는 배경의 범위는 [필터 Bottom Sheet 디자인](./filter-bottom-sheet.md)의 `항목 줄의 누름 표현`을 따른다. 스위치가 바뀌면 표시를 즉시 갱신하고 Bottom Sheet는 열린 상태를 유지한다. 스위치 전환의 시간과 곡선은 Material 기본 모션을 따르고 별도 수치를 정하지 않는다.
...
각 태그는 카드로 표시한다. 카드에는 태그 이모지와 제목을 표시하고 그 왼쪽에는 저장된 컬러로 채운 작은 원형 표시를 둔다. 카드 가장자리와 내용 사이의 여백은 [공통 스타일](./styles.md)의 `카드 내용`을 따른다.
...
카드를 누르면 [TagDetail 화면 디자인](./tag-detail.md)으로 이동한다. 카드를 옆으로 미는 조작은 `완료와 삭제 제스처`를 따른다.

## 완료와 삭제 제스처

사용자는 태그 카드를 왼쪽에서 오른쪽으로 스와이프해 완료하고, 오른쪽에서 왼쪽으로 스와이프해 삭제한다. 2열 격자에서도 카드마다 따로 스와이프하며, 스와이프하는 카드는 자기 칸 안에서만 움직인다.

스와이프 중 상태 아이콘, 완료·삭제 기준, 취소와 비활성 표현은 [SwipeToFinishAndDelete 컴포넌트 디자인](./swipe-to-finish-and-delete.md)을 따른다. 자리 표시 카드는 눌러도 반응하지 않고 스와이프도 시작되지 않는다.

완료 또는 삭제가 성립하면 카드를 격자에서 없애고, 결과 안내와 실행 취소 동작을 스낵바로 표시한다. 실행 취소 동작이 있는 스낵바는 Material 기본의 긴 표시 시간을 쓴다. 이미 보이는 스낵바가 있으면 표시 시간이 지나기를 기다리지 않고 즉시 새 안내로 바꾼다. 스낵바는 떠 있는 태그 추가 버튼에 가리지 않는 자리에 둔다. 표현은 [MemoHome 목록 디자인](./memo-home.md)의 `완료와 삭제 제스처`와 같다.

완료나 삭제가 저장되지 못하면 스낵바를 표시하지 않는다.

실행 취소로 돌아온 카드는 스와이프 전 모양으로 정렬 기준에 맞는 자리에 다시 나타난다. 목록이 갱신될 때 카드가 자리를 옮기면 기본 항목 이동 애니메이션으로 옮긴다.

목록과 상세를 함께 표시하는 동안에도 스낵바는 목록 영역 기준으로 표시하며, 상세 영역에 놓인 화면은 스와이프와 실행 취소로 바뀌지 않는다.
...
| 완료 안내 | `태그가 완료되었습니다.` | `Tag finished.` |
| 삭제 안내 | `태그가 삭제되었습니다.` | `Tag deleted.` |
| 실행 취소 동작 | `실행 취소` | `Undo` |
| 완료 스와이프 아이콘 접근성 이름 | `태그 완료` | `Finish tag` |
