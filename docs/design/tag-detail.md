...
예외로 메모 탭의 메모 카드, 웹 탭의 웹 카드와 장소 탭의 장소 카드 위에서 시작한 좌우 스와이프는 카드의 스와이프가 가져간다. 장소 탭에서는 두 보기 모드의 장소 카드가 모두 여기에 해당한다. 카드는 [SwipeToFinishAndDelete 컴포넌트 디자인](./swipe-to-finish-and-delete.md)에 따라 움직이고 페이지는 전환되지 않는다. 카드 사이나 목록 가장자리 같은 카드 밖 여백에서 시작한 스와이프와 탭 행 선택은 그대로 페이지를 전환한다. 장소 탭 지도 모드의 지도 영역에서 시작한 끌기는 이 예외와 관계없이 지금처럼 지도를 옮긴다.

## 태그 디테일 탭

본문에는 이모지 입력과 단일 줄 제목 입력을 한 행에 배치한 제목 행, 설명 입력, 컬러 입력과 태그 연결 입력을 순서대로 배치하고 각각 [공통 구성요소 간격](./dimens.md)을 둔다. 제목 행의 배치는 [TagAdd 화면 디자인](./tag-add.md)과 같고, 제목 입력의 구성과 문구는 [제목 입력 디자인](./title-input.md)을, 태그 연결 입력의 구성과 조작은 [태그 연결 입력 컴포넌트 디자인](./tag-link-input.md)을 따른다.
...
완료 또는 다시 시작 버튼은 완료 여부와 관계없이 방패에 체크가 들어간 같은 완료 아이콘을 쓰는 토글 버튼이다. 완료되지 않은 태그에서는 버튼을 켜지지 않은 상태로, 완료된 태그에서는 켜진 상태로 강조해 표시한다.

수정, 완료·다시 시작 또는 삭제를 처리하는 동안에는 해당 버튼의 아이콘을 진행 표시로 바꾼다.

삭제에 성공하면 뒤로가기와 같은 결과로 진입하기 전 화면으로 돌아간다. 목록과 상세를 함께 표시하던 경우에도 같아서, 목록에서 고른 상세였으면 상세 영역이 태그 상세를 고르기 전 상태로, 연결한 태그를 눌러 연 상세였으면 이전 태그의 상세로 돌아간다.

수정 성공 피드백은 스낵바로 표시한다. 스낵바는 네 탭이 함께 사용하므로 메모·웹·장소 탭에서 발생한 완료·삭제 피드백도 같은 자리에 표시한다.
