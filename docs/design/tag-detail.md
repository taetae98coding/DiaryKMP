...
예외로 메모 탭의 메모 카드와 웹 탭의 웹 카드 위에서 시작한 좌우 스와이프는 카드의 스와이프가 가져간다. 카드는 [SwipeToFinishAndDelete 컴포넌트 디자인](./swipe-to-finish-and-delete.md)에 따라 움직이고 페이지는 전환되지 않는다. 카드 사이나 목록 가장자리 같은 카드 밖 여백에서 시작한 스와이프와 탭 행 선택은 그대로 페이지를 전환한다.
