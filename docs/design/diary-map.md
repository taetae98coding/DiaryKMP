...
앱 마커는 장소를 나타내는 물방울 모양을 쓰고 가운데를 둥글게 비워 지도 바탕이 비쳐 보이게 한다. 앱 마커를 쓰는 모든 제공자와 환경은 같은 모양과 같은 크기로 그린다. 앱 마커의 크기는 가로세로 `32dp`다.
...
앱이 라벨 글씨 크기를 정하는 환경에서는 `12sp` 글씨 한 줄로 쓰고, 라벨이 마커 아래에 차지하는 높이는 `16dp`다. 앱이 라벨을 직접 그리는 환경에서는 검은 글씨 뒤에 불투명도 `80%`의 흰 바탕을 두어 지도 바탕 위에서도 읽히게 하고, 바탕에는 좌우 `2dp` 여백과 `2dp` 둥근 모서리를 둔다.
