...
스크롤로 키우거나 줄일 때는 휠 한 칸마다 약 `10%`씩 배율을 바꾼다.
...
버튼을 누르면 기기의 사진 선택 도구가 열린다. Android와 iOS는 시스템의 사진 선택 화면을 이미지만 고를 수 있게 열고, 데스크톱은 시스템의 파일 열기 창을 이미지 파일만 보이게 열며, 웹은 브라우저의 파일 선택 창을 이미지 파일만 고를 수 있게 연다. 어느 도구든 사진 하나만 고른다. 사진을 고르면 편집 영역에 그 사진이 처음 상태의 틀과 함께 나타나고, 이미 사진이 있었으면 새 사진으로 바뀌며 틀은 처음 상태로 돌아간다. 사진이 나타나거나 바뀔 때 전환 모션은 두지 않는다.
