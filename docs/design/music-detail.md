...
기준 스펙: [MusicDetail 화면 스펙](../spec/client/music-detail.md), [Playlist 목록·상세 배치 스펙](../spec/client/playlist-list-detail.md)
...
본문에는 단일 줄 제목 입력, 단일 줄 가수 입력, 링크 입력을 위에서부터 이 순서로 둔다. 입력의 순서와 그 이유, 링크 카드의 구성과 조작, 링크 입력의 이름, 소프트 키보드 요청, 입력 사이 간격은 모두 [MusicAdd 화면 디자인](./music-add.md)의 `화면 구조`, `링크 입력`, `불러오기 버튼`, `썸네일 미리보기`를 따른다. 두 화면이 같은 자리에 같은 입력을 두어 사용자가 곡을 추가할 때와 고칠 때 같은 방식으로 조작한다. 이 화면에도 태그 입력을 두지 않는다.

본문 바깥에는 [공통 화면 가로 여백과 세로 여백](./dimens.md)을 둔다. 본문의 내용이 화면 높이를 넘으면 본문 안에서 세로로 스크롤한다.

## 진입 시 표시 내용

조회가 완료되면 제목, 가수, 링크 입력에 저장된 값을 채운다. 링크 카드의 썸네일 미리보기는 [MusicAdd 화면 디자인](./music-add.md)의 `썸네일 미리보기`와 같게 링크 입력의 값에서 정해지므로, 진입 직후에는 저장된 링크의 썸네일이 보인다.

저장된 가수나 링크가 비어 있으면 그 입력을 비운 채로 두고, 링크가 비어 있거나 영상을 가리키지 않으면 미리보기 자리를 카드에서 완전히 비운다.

저장된 값이 이미 채워져 있어도 입력을 읽기 전용으로 바꾸지 않으며, 링크 입력의 지우기 버튼과 불러오기 버튼도 그대로 제공한다.

입력이 비어 있는 상태에 오류 색이나 필수 표시를 상시로 두지 않는다. 제목을 비운 채 수정하면 저장된 기존 제목이 쓰이고, 가수와 링크는 비워 둘 수 있는 값이므로 어느 쪽도 사용자가 고쳐야 하는 상태가 아니다.
...
불러오기에 성공하면 제목 입력과 가수 입력의 값이 불러온 영상의 값으로 바뀐다. 값이 바뀌는 것이 결과를 충분히 알리므로 성공 스낵바를 표시하지 않는다. 썸네일 미리보기는 링크에서 정해지므로 불러오기로 바뀌지 않는다.

사용자가 링크를 고쳐 미리보기가 나타나거나 사라질 때의 카드 높이 전환은 [MusicAdd 화면 디자인](./music-add.md)의 `썸네일 미리보기`를 따른다. 링크 카드가 본문의 마지막 입력이므로 다른 입력은 자리를 옮기지 않는다.
...
수정 버튼은 제목, 가수, 링크 입력 내용이 저장 내용과 다를 때만 표시하며, 나타나고 사라지는 표현은 [나타나고 사라지는 버튼 디자인](./button-visibility.md)을 따른다. 링크가 YouTube 영상 링크가 아니어도 저장 내용과 다르므로 버튼을 표시하고, 링크 형식 안내는 수정을 실행한 시점에만 제공한다.
...
수정 성공, 링크 미입력 안내, 링크 형식 안내와 불러오기 실패 안내는 모두 스낵바로 표시한다. 링크 미입력 안내는 링크를 비운 채 불러오기를 실행했을 때만 쓰고, 입력 초점을 링크 입력으로 옮긴다. 목록과 함께 표시할 때 스낵바를 두는 자리는 [목록·상세 배치 공통 디자인](./list-detail-pane.md)의 `진행과 피드백`을 따른다.
...
제목을 비운 채 수정해 저장된 기존 제목이 유지되는 경우와 가수나 링크를 비워 그 값이 지워지는 경우에는 별도의 안내를 표시하지 않고 수정 성공 피드백만 표시한다.
...
링크 입력과 가수 입력의 이름, 불러오기 버튼의 라벨과 접근성 이름, 썸네일 미리보기의 접근성 이름, 링크 미입력 안내와 링크 형식 안내와 불러오기 실패 안내 문구는 [MusicAdd 화면 디자인](./music-add.md)의 문구를 그대로 쓴다. 같은 입력과 같은 판단 기준을 두 화면에서 다른 문구로 안내하지 않는다.
