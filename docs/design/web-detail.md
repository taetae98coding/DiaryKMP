...
수정으로 저장된 URL이나 요청 헤더가 바뀌어 다시 불러올 때도 같은 로딩 진행 표시를 쓴다. 창 너비가 compact여서 사용자가 수정 폼 탭이나 메모 탭을 보고 있으면 웹 페이지 탭으로 자동으로 옮기지 않고, 사용자가 웹 페이지 탭을 고를 때 그 시점의 상태를 보여 준다.
