...
다만 웹 페이지 영역이 화면에서 빠졌다가 돌아오면 문서 안에서 보고 있던 자리와 문서 안에서 이동한 결과는 유지하지 않고 처음부터 다시 그린다. 웹 표시 수단은 화면에서 빼지 않고 감춰 두기만 하면 수정 폼 위에 그대로 남아 보이기 때문이다. 이는 탭을 바꿀 때, `적응형 배치`가 바뀔 때, 화면이 재생성될 때 모두 적용한다.
...
표시 방식 줄과 표시 방식 선택 Bottom Sheet는 앱이 직접 그리므로 모든 플랫폼에서 같게 표시한다.
