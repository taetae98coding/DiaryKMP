...
기본 지도를 확인하기 전이거나 읽지 못해 지도를 표시하지 않는 동안에는 본문을 나누지 않고 입력 영역이 본문 전체를 채운다. 별도의 로딩 표시나 오류 안내도 배치하지 않는다. 기본 지도를 확인하면 위의 기준대로 본문을 나눠 지도 영역을 표시하고, 입력 영역은 나뉜 자리로 옮겨 간다.
