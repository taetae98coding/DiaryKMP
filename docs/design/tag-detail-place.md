...
장소 카드는 [장소 보기 모드 디자인](./place-view-mode.md)의 `삭제 스와이프`에 따라 두 보기 모드에서 오른쪽에서 왼쪽으로 스와이프해 삭제하고, 결과 안내와 실행 취소를 같은 스낵바로 표시한다. 완료 스와이프는 두지 않는다. 스낵바는 TagDetail 화면의 떠 있는 버튼에 가리지 않는 자리에 두며, 메모 탭과 웹 탭의 스낵바와 같은 자리다.
...
보기 모드 전환 버튼의 접근성 이름은 [장소 보기 모드 디자인](./place-view-mode.md)의 문구 표가 소유한다. 장소 카드와 핀의 접근성 이름, 삭제 안내와 실행 취소 동작, 삭제 스와이프 아이콘 접근성 이름도 그 문서를 따른다. 빈 상태 문구는 [목록 빈 상태 디자인](./list-empty-state.md)의 문구 표가 소유한다.
