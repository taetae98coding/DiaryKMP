...
아이콘만 표시하고 글자 라벨을 두지 않는 공통 버튼에 설명을 둔다. 지우기, 복사, 삭제, 완료, 뒤로가기, 외부로 열기, 검색, 설정, 다운로드 버튼과 떠 있는 추가 버튼, 떠 있는 확인 버튼이 대상이다. 공통 버튼이 아닌 버튼 가운데 [ContactDetail 화면](./contact-detail.md)의 즐겨찾기 버튼과 [메모 Gemini 작성 도우미](./memo-gemini.md)의 여는 버튼도 같은 설명을 둔다.

그 밖에 화면마다 따로 둔 아이콘 버튼에는 지금 설명을 두지 않는다. 각 화면 디자인이 설명을 둔다고 정한 버튼만 이 문서를 따른다.
