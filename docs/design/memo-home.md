...
버튼의 아이콘과 Bottom Sheet의 표시, 닫는 조작, 적응형 배치는 [필터 Bottom Sheet 디자인](./filter-bottom-sheet.md)을 따른다. 다만 MemoHome의 Bottom Sheet는 태그 필터에 더해 유무 필터를 함께 담으므로, 아래의 `필터 Bottom Sheet 구성`이 이 화면의 내용과 배치를 소유한다.
...
태그 칩과 선택 전체 해제 버튼은 Material 기본 비활성 표현으로 표시한다. 태그 칩은 선택한 태그의 컬러 채움과 선택하지 않은 태그의 원형 표시를 그대로 유지한 채 영역 전체를 흐리게 표시해, 유지된 선택은 그대로 읽히면서 지금 조작할 수 없다는 것만 드러낸다.
...
메모는 카드 형태로 표시하고 카드 사이에는 세로로 [공통 항목 간격](./dimens.md)을 둔다.

각 카드에는 메모 제목을 표시하고, 제목 왼쪽에는 메모에 저장된 컬러로 채운 작은 원형 표시를 둔다.
...
기간이 있는 메모 카드는 시작 날짜가 같은 것끼리 모으고, 각 묶음 위에 시작 날짜를 나타내는 날짜 헤더를 표시한다.
