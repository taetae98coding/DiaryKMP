...
MemoHome 목록은 `메모` 주요 목적지의 첫 화면이므로 앱의 공통 내비게이션 영역과 함께 표시한다.

화면 상단에는 `메모` 제목을 표시하는 상단 바를 둔다. 상단 바에는 뒤로가기 버튼을 두지 않는다.

본문은 상단 바 아래에 정렬 줄과 메모 목록을 이 순서로 세로로 두고, 메모 목록이 남은 높이를 모두 채운다. 오른쪽 아래에는 떠 있는 메모 추가 버튼을 목록 위에 겹쳐 표시한다.

창 너비가 compact보다 넓으면 메모 목록과 상세 영역을 함께 표시하며, 그 배치는 [Memo 목록·상세 배치 디자인](./memo-list-detail.md)을 따른다.
...
떠 있는 메모 추가 버튼에는 더하기 아이콘을 표시한다. 목록이 비어 있거나 필터를 적용해 두었어도 같은 자리에 같은 모양으로 표시하고, 비활성 표현을 사용하지 않는다.

버튼을 누르면 [MemoAdd 화면](./memo-add.md)으로 이동한다. 목록과 상세를 함께 표시하는 동안 상세 영역에 이미 MemoAdd가 놓여 있으면 버튼을 표시하지 않으며, 노출 기준은 [목록·상세 배치 공통 디자인](./list-detail-pane.md)의 `버튼 노출`을 따른다.

물리 키보드 환경에서는 `Cmd` 또는 `Meta`와 `A`를 함께 눌러 같은 추가를 실행한다. 같은 조합을 쓰는 [TagHome 목록 디자인](./tag-home.md), [RoutineHome 화면 디자인](./routine-home.md)과 같으므로 사용자는 화면이 달라도 같은 조합으로 항목을 추가한다.
...
필터 아이콘 버튼은 필터가 적용되지 않은 상태이면 기본 상단 바 아이콘 색을 사용하고, 적용된 상태이면 아이콘을 주요 색으로 표시한다. 적용 여부의 기준은 [목록 필터 반영 스펙](../spec/client/list-filter.md)의 `필터 적용 상태`를 따른다. 어떤 필터가 적용되었는지는 아이콘으로 구분하지 않고 배지도 표시하지 않는다.
...
각 행에는 왼쪽에 축 이름을 본문 크기의 글씨로 표시하고, 오른쪽 남은 폭에 `있음`, `없음`, `전체`를 이 순서로 담은 3분할 세그먼트 버튼을 배치한다. 세 버튼은 같은 폭으로 나누고, 세그먼트 버튼 행은 행의 남은 폭을 모두 사용한다. 축 이름과 세그먼트 버튼 행 사이에는 [공통 구성요소 간격](./dimens.md)을 둔다.
...
기간이 있는 메모는 제목 아래에 제목보다 작은 글씨와 보조 색으로 기간을 표시한다. 기간이 없는 메모는 제목만 표시한다.

카드를 누르면 그 메모의 MemoDetail 화면을 연다. compact 창에서는 MemoDetail 화면을 단독으로 표시하고, 목록과 상세를 함께 표시하는 동안에는 [Memo 목록·상세 배치 디자인](./memo-list-detail.md)의 `상세 영역의 전환`을 따른다.
...
아직 준비되지 않은 자리에는 [페이지 조회 목록의 자리 표시 디자인](./paged-list-placeholder.md)에 따라 자리 표시 카드를 표시한다. 자리 표시 카드는 눌러도 반응하지 않고 스와이프도 시작되지 않는다.

## 새로고침

정렬 줄 아래의 메모 목록 영역을 [새로고침 디자인](./sync-refresh.md)의 당김 새로고침 컨테이너로 감싼다. 정렬 줄은 감싸지 않아 정렬 줄을 세로로 끌어도 새로고침이 시작되지 않는다. 목록 대신 빈 상태를 표시하는 동안에도 같은 영역에서 당겨 새로고침할 수 있다.

진행 표시의 모양과 자리, 진행 중 조작 가능 여부는 [새로고침 디자인](./sync-refresh.md)을 그대로 따르며 이 화면에서 따로 정하지 않는다.
...
두 빈 상태 사이의 전환과 목록과의 전환, 보조 문구를 포함한 정확한 문구는 [목록 빈 상태 디자인](./list-empty-state.md)을 따른다.

목록을 불러오지 못했을 때는 별도 오류 안내나 다시 시도 버튼을 표시하지 않는다. 표시할 메모가 하나도 없으면 같은 빈 상태를 표시한다.

빈 상태에서도 상단 바의 검색 버튼과 필터 버튼, 정렬 줄의 완료된 메모 진입 버튼, 떠 있는 메모 추가 버튼은 같은 자리에 그대로 표시한다.

## 날짜 헤더

기간이 있는 메모 카드는 시작 날짜가 같은 것끼리 모으고, 각 묶음 위에 시작 날짜를 나타내는 날짜 헤더를 표시한다. 날짜 헤더 글은 주요 색의 강조된 작은 제목 글씨로 표시하고, 글의 위아래에는 `4dp` 여백을 둔다. 헤더는 기본 표면 색으로 채워, 목록 위쪽에 고정되었을 때 아래로 지나가는 카드가 비치지 않게 한다.
...
완료 또는 삭제가 성립하면 카드를 목록에서 없애고, 결과 안내와 실행 취소 동작을 스낵바로 표시한다. 실행 취소 동작이 있는 스낵바는 Material 기본의 긴 표시 시간을 쓴다. 이미 보이는 스낵바가 있으면 표시 시간이 지나기를 기다리지 않고 즉시 새 안내로 바꾼다.

완료나 삭제가 저장되지 못하면 스낵바를 표시하지 않는다.
...
| --- | --- | --- |
| 상단 바 제목 | `메모` | `Memo` |
| 메모 추가 버튼 접근성 이름 | `메모 추가` | `Add memo` |
