...
기준 스펙: [장소 보기 모드 스펙](../spec/client/place-view-mode.md)

이 문서는 장소 목록을 목록 모드와 지도 모드로 함께 보여 주는 자리의 영역 분할과 적응형 배치, 두 모드를 오가는 전환 표현, 장소 격자와 장소 카드, 지도 핀, 정렬 줄과 빈 상태와 새로고침이 놓이는 자리를 소유한다.

전환 컨트롤을 어느 자리에 두는지, 그 자리에서 어떤 모양으로 두는지와 보기 모드 밖에서 함께 두는 컨트롤은 각 적용 대상 디자인이 소유한다. 적용 대상은 [장소 보기 모드 스펙](../spec/client/place-view-mode.md)의 `적용 대상`을 따른다.
...
격자의 카드는 누르는 조작과 `삭제 스와이프`의 미는 조작을 받는다.
...
원형 표시는 제목 줄이 아니라 제목과 주소를 합친 글 묶음 전체의 세로 가운데에 둔다. 주소가 없어 제목 한 줄만 있는 카드에서는 결과적으로 제목 줄의 세로 가운데가 된다. 원형 표시와 글 묶음 사이에는 [공통 여백과 간격](./dimens.md)의 `컬러 원형 표시 간격`, 제목 줄과 주소 줄 사이에는 [공통 여백과 간격](./dimens.md)의 `카드 줄 간격`을 둔다.
...
사용자는 두 보기 모드의 장소 카드를 오른쪽에서 왼쪽으로 스와이프해 삭제한다. 장소에는 완료 여부가 없으므로 왼쪽에서 오른쪽으로 미는 조작에는 반응하지 않는다.

스와이프 중 상태 아이콘, 삭제 기준, 취소와 비활성 표현은 [SwipeToFinishAndDelete 컴포넌트 디자인](./swipe-to-finish-and-delete.md)을 따른다. 목록 모드의 자리 표시 카드는 스와이프가 시작되지 않는다. 지도 영역의 핀은 스와이프로 삭제하지 않으며, 지도 위에서 끄는 조작은 지금처럼 지도를 옮긴다.

삭제가 성립하면 카드를 격자에서 없애고, 지도 모드에서는 그 장소의 핀도 지도에서 함께 없앤다. 결과 안내와 실행 취소 동작은 스낵바로 표시한다. 실행 취소 동작이 있는 스낵바는 Material 기본의 긴 표시 시간을 쓴다. 이미 보이는 스낵바가 있으면 표시 시간이 지나기를 기다리지 않고 즉시 새 안내로 바꾼다. 표현은 [MemoHome 목록 디자인](./memo-home.md)의 완료·삭제 스낵바와 같고, 스낵바를 두는 자리는 각 적용 대상 디자인이 소유한다.

보기 모드를 바꾸는 교차 페이드는 스낵바에 영향을 주지 않는다. 안내가 보이는 동안 보기 모드를 바꿔도 스낵바는 같은 자리에 그대로 남는다.

삭제가 저장되지 못하면 스낵바를 표시하지 않는다.

실행 취소로 돌아온 카드는 스와이프 전 모양으로 정렬 기준에 맞는 자리에 다시 나타나고, 지도 모드에서는 핀도 다시 나타난다.
...
지도 모드에서 지도와 목록 영역을 모두 표시하지 않는 동안, 그리고 지도가 보이는 영역을 아직 확인하지 못한 동안에는 빈 상태도 표시하지 않는다. 보이는 영역을 확인하기 전의 장소 목록 영역은 카드와 자리 표시 카드, 진행 표시 없이 비워 둔다.
...
진행 표시가 나타나 있는 동안에도 지도를 옮기고 확대·축소하는 조작, 핀과 장소 카드를 누르는 조작, 장소 카드를 스와이프해 삭제하는 조작, 장소 추가 버튼과 보기 모드 전환 컨트롤을 누르는 조작을 모두 그대로 할 수 있으며 어떤 요소도 비활성으로 표시하지 않는다.
...
장소 삭제의 문구는 한국어 환경과 그 외 기본 환경에서 다음을 사용한다. 어느 적용 대상에서도 같은 문구를 쓴다.

| 항목 | 한국어 | 그 외 기본 |
| --- | --- | --- |
| 삭제 안내 | `장소가 삭제되었습니다.` | `Place deleted.` |
| 실행 취소 동작 | `실행 취소` | `Undo` |
| 삭제 스와이프 아이콘 접근성 이름 | `장소 삭제` | `Delete place` |
