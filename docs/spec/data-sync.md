...
메모나 태그에 생성, 수정, 완료, 삭제, 실행 취소가 발생하거나, 장소나 웹 항목이나 연락처에 추가, 수정, 삭제가 발생하거나, 곡이 추가되거나, 메모와 태그의 연결, 메모와 장소의 연결, 메모와 웹 항목의 연결, 메모와 연락처의 연결, 태그와 태그의 연결, 웹 항목과 태그의 연결, 장소와 태그의 연결이 만들어지거나 해제되면 해당 항목은 업로드 대기 상태가 된다. 이미 업로드 대기 상태였다면 별도의 항목이 추가로 쌓이지 않는다.
