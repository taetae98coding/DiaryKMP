...
삭제한 항목이 다시 삭제되지 않은 상태가 되면 그 항목을 가리키는 연결이 다시 조회 결과에 나타난다. 메모 삭제의 실행 취소처럼 화면에서 삭제를 되돌린 경우와, 다른 기기에서 받은 내용으로 삭제가 풀린 경우가 모두 같다. 화면에서 삭제를 되돌리는 동작을 언제 제공하는지는 각 화면 스펙이 정한다.
