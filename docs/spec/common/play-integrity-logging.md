...
앱은 요청할 때 요청 확인 값을 함께 알리고, Google은 그 값을 판정 결과에 그대로 담아 돌려준다. 앱이 요청 확인 값을 만드는 기준은 [Play Integrity 판정 기록 스펙(client)](../client/play-integrity-logging.md)의 `요청 확인 값`을 따른다.
