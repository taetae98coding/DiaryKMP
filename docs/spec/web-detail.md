...
사용자가 수정을 실행해 저장된 URL이 바뀌면 URL 방식에서는 새로 저장된 URL을 웹 표시 수단이 다시 연다. 저장된 요청 헤더만 바뀌었으면 URL 방식의 표시는 바뀌지 않는다.
...
화면이 회전하거나 시스템에 의해 재생성되어도 고른 표시 방식을 유지하고, 이미 표시하고 있던 웹 페이지와 불러오기 상태도 유지하며 웹 페이지를 다시 불러오지 않는다.

화면을 떠난 뒤 다시 들어오거나 앱을 종료한 뒤 새로 진입하면 수정 중이던 내용을 복원하지 않고 저장된 내용으로 다시 시작하고, 표시 방식도 복원하지 않고 `웹 페이지 표시 방식`이 정한 처음 방식으로 다시 시작하며, 이전에 받은 웹 페이지도 복원하지 않는다.
