...
이번 범위는 웹 목록과 웹 결과에서 웹 항목을 선택해 상세로 이동하고, 저장된 제목·설명·URL·요청 헤더를 확인하고 수정하고, 연결한 태그를 확인하고 바꾸고, 저장된 URL의 웹 페이지를 두 가지 표시 방식 중 하나로 보고, 표시 방식을 바꾸고, 응답 본문 방식에서 불러오지 못했을 때 다시 시도하고, 웹 항목을 앱 밖에서 열거나 삭제하는 것까지다.
...
[Chrome 로그인 이어받기 스펙](./chrome-session-import.md)의 `제공 환경`에서 사용자가 Chrome 프로필을 골라 두었으면, 웹 표시 수단이 저장된 URL을 열기 전에 그 주소의 로그인 정보를 가져온다. 무엇을 가져오고 어떤 경우가 실패인지는 그 스펙을 따른다.

가져오는 동안에는 웹 페이지 자리에 진행 상태를 표시하고 웹 표시 수단은 아직 주소를 열지 않는다. 가져오기가 끝나면 웹 표시 수단이 주소를 연다.

가져오지 못하면 가져오지 못했다는 피드백을 제공하고, 웹 표시 수단은 로그인 정보 없이 주소를 그대로 연다. 다시 시도는 따로 제공하지 않는다.

가져오는 동안에도 사용자는 제목, 설명, URL과 요청 헤더를 확인하고 수정하고, 태그를 바꾸고, 표시 방식을 바꾸고, 외부로 열고, 삭제하고, 뒤로갈 수 있다.

프로필을 고르지 않았거나 제공하지 않는 환경이면 아무것도 가져오지 않고 웹 표시 수단이 곧바로 주소를 연다.
...
사용자가 수정을 실행해 저장된 URL이 바뀌면 URL 방식에서는 새로 저장된 URL을 웹 표시 수단이 다시 연다. 로그인 정보를 가져오는 환경이면 `로그인 정보 가져오기`와 같은 규칙으로 새 주소의 로그인 정보를 먼저 가져온 뒤 연다. 저장된 요청 헤더만 바뀌었으면 URL 방식의 표시는 바뀌지 않는다.
...
화면이 회전하거나 시스템에 의해 재생성되어도 입력 중이던 제목, 설명, URL과 헤더 항목의 이름·값·순서를 유지한다.

화면이 회전하거나 시스템에 의해 재생성되어도 고른 표시 방식을 유지하고, 이미 표시하고 있던 웹 페이지와 불러오기 상태도 유지하며 웹 페이지를 다시 불러오지 않는다. 이미 가져온 로그인 정보도 다시 가져오지 않는다.

화면을 떠난 뒤 다시 들어오거나 앱을 종료한 뒤 새로 진입하면 수정 중이던 내용을 복원하지 않고 저장된 내용으로 다시 시작하고, 표시 방식도 복원하지 않고 `웹 페이지 표시 방식`이 정한 처음 방식으로 다시 시작하며, 이전에 받은 웹 페이지도 복원하지 않는다. 로그인 정보를 가져오는 환경이면 다시 진입할 때 로그인 정보를 다시 가져온다.

### 뒤로가기

사용자가 뒤로가면 WebDetail 화면에 진입하기 전 화면으로 돌아간다. WebHome에서 들어왔으면 WebHome으로, SearchHome에서 들어왔으면 SearchHome으로, TagDetail 웹 탭에서 들어왔으면 웹 탭이 선택된 같은 태그의 TagDetail 화면으로 돌아간다. 반영하지 않은 수정 내용은 저장되지 않으며, 이미 반영된 태그 연결과 해제는 그대로 남는다.
...
stateDiagram-v2
    ...
    Loading --> Preparing: 대상 웹 항목 조회 성공, 로그인 정보를 가져오는 환경
    Loading --> Address: 대상 웹 항목 조회 성공, 로그인 정보를 가져오지 않는 환경
    Preparing --> Address: 가져오기 성공 또는 실패
    Address --> Preparing: 수정으로 저장된 URL이 바뀜, 로그인 정보를 가져오는 환경
...
로그인 정보 가져오는 중 상태는 [Chrome 로그인 이어받기 스펙](./chrome-session-import.md)의 `제공 환경`에서 사용자가 Chrome 프로필을 골라 두었을 때만 거친다. 이 상태에서는 웹 표시 수단이 주소를 열지 않으며, 가져오기가 성공하든 실패하든 URL 방식 표시로 넘어간다. 응답 본문 방식에 있는 동안 저장된 URL이 바뀌어도 새 주소의 로그인 정보를 가져오며, 그 뒤 사용자가 URL 방식으로 바꿀 때 아직 가져오는 중이면 이 상태를 거친 뒤 URL 방식 표시로 넘어간다.
...
화면 재생성과 백그라운드 복귀 뒤에도 입력 중이던 제목, 설명, URL과 요청 헤더를 유지하고, 고른 표시 방식과 받은 웹 페이지와 화면 상태도 유지하며 웹 페이지를 다시 불러오지 않는다.
