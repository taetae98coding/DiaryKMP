...
사용자가 당겼을 때와 앱이 인증된 사용자 계정을 확인해 서버 동기화를 시작했을 때는 진행 중임을 표시한다. 메모나 태그의 생성·수정·완료·삭제·실행 취소, 장소의 추가·수정·삭제, 웹 항목의 추가, 메모·태그 연결과 메모·장소 연결의 변경을 계기로 시작된 서버 동기화는 진행을 표시하지 않는다.

계정 확인을 계기로 하는 서버 동기화의 판단 기준은 [데이터 동기화 스펙](./data-sync.md)의 `동기화 계기`를 따른다.
