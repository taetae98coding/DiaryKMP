...
이 문서는 양력 날짜마다 대응하는 음력 날짜를 원격에서 받아 로컬 캐시에 동기화하고 조회하는 규칙을 다룬다. 사용자에게 직접 노출되는 화면이나 사용자 행동은 없으므로 `feature` 영역은 생략한다. 음력 자료로 음력 생일을 양력 날짜에 놓는 기준은 [캘린더 연락처 생일 표시 스펙](./calendar-contact-birthday.md)이, 동기화를 요청하는 계기와 대상 연도는 [CalendarHome 스펙](./calendar-home.md)이 소유한다.
...
음력 자료 동기화 실패는 오류 보고 대상이다. 보고 방식은 [UseCase 실패 로깅](./usecase-failure-logging.md)의 오류 보고를 따른다.
...
flowchart TD
    ...
    remote -- 성공 --> replace["받은 음력 날짜 전체로 해당 연도 캐시 교체<br/>제공된 자료가 없으면 기존 캐시 제거"]
    replace -- 실패 --> fail
...
연도가 이미 동기화 이력에 있으면 원격 조회와 로컬 캐시 교체 없이 성공으로 끝낸다. 이때 동기화 결과로는 그 연도의 로컬 캐시에 저장된 음력 날짜를 알린다.

### 원격 자료 저장

동기화 이력에 없는 연도는 그 연도의 음력 자료를 원격에서 조회한다.

원격 조회가 성공하면 받은 음력 날짜 전체로 그 연도의 로컬 캐시를 교체한다. 원격이 해당 연도의 자료를 제공하지 않으면 그 연도의 기존 로컬 캐시를 제거한다.

로컬 캐시 교체까지 성공하면 해당 연도를 동기화 이력에 기록하고, 받은 음력 날짜를 동기화 결과로 알린다.

한 연도의 동기화는 다른 연도의 로컬 캐시에 영향을 주지 않는다.

### 실패 처리

원격 조회에 실패하면 그 연도의 로컬 캐시를 변경하지 않고 실패로 끝낸다.

원격 조회 후 로컬 캐시 교체에 실패하면 실패로 끝내고, 해당 연도의 기존 로컬 캐시는 유지한다.

실패한 연도는 동기화 이력에 기록하지 않으므로, 다음 동기화 요청에서 다시 원격 조회한다.

### 기간별 음력 날짜 조회

시작일과 종료일로 기간을 정해 음력 날짜를 조회하면, 그 기간에 드는 양력 날짜의 음력 날짜를 로컬 캐시에서 합쳐 제공한다.

기간에 드는 양력 날짜 중 캐시에 없는 날짜는 결과에 들지 않는다. 기간이 연도 경계를 넘으면 캐시된 연도의 날짜만 담기고, 캐시되지 않은 연도의 날짜는 담기지 않는다. 캐시된 날짜가 하나도 없으면 빈 목록을 제공한다.

조회 중인 기간에 드는 연도의 캐시가 교체되면 변경된 목록을 이어서 제공한다.
