...
선택은 즉시 메모 목록에 반영되며 별도의 확정 동작은 필요하지 않다. 필터를 닫아도 선택은 유지되고, 사용자는 필터를 다시 열어 현재 선택을 확인하고 바꿀 수 있다. 반영된 목록을 사용자가 어디서부터 확인하는지는 [목록 필터 반영 스펙](./list-filter.md)의 `필터 반영`을 따른다.

유무 필터와 태그 필터는 같은 필터 안에서 함께 제공되며, 두 필터의 선택은 함께 적용된다.

사용자가 태그 축을 `없음`으로 고르면 태그 필터는 목록에 적용되지 않는다. 이때 사용자는 태그 필터에서 선택과 해제, 선택 전체 해제를 실행할 수 없고, 태그 필터가 지금 적용되지 않는다는 것을 확인할 수 있다. 태그 축을 `있음`이나 `전체`로 되돌리면 사용자는 이전 선택을 그대로 확인하고 다시 바꿀 수 있다.
...
선택과 해제, 전체 해제는 즉시 메모 목록에 반영된다. 반영된 목록을 사용자가 어디서부터 확인하는지는 [목록 필터 반영 스펙](./list-filter.md)의 `필터 반영`을 따른다.
...
메모 목록에는 완료되지 않았고 삭제되지 않은 메모만 노출한다. 완료되었거나 삭제된 메모는 저장소에 남아 있지만 목록에는 나타나지 않는다.
...
태그 축이 세는 태그는 현재 사용자 계정과 연결되고 삭제되지 않은 태그와의 해제되지 않은 연결이다. 완료된 태그와의 연결도 태그가 있는 것으로 센다. 연결의 해제 규칙은 [메모 태그 스펙](./memo-tag.md)을 따른다.

장소 축이 세는 장소는 현재 사용자 계정과 연결되고 삭제되지 않은 장소와의 해제되지 않은 연결이다. 연결의 해제 규칙은 [메모 장소 스펙](./memo-place.md)을 따른다.
...
태그 축이 `없음`인 동안에는 태그 필터의 선택과 해제, 선택 전체 해제를 실행할 수 없다. 판정에 쓰이지 않는 선택을 바꾸게 두면 사용자가 목록에 반영되지 않는 조작을 하게 되기 때문이다. 사용자가 태그 축을 `있음`이나 `전체`로 되돌리면 유지된 선택이 다시 판정에 쓰이고 다시 바꿀 수 있다.
...
메모 목록의 태그 필터 선택은 [태그 필터 스펙](./tag-filter.md)의 `필터 선택 유지`를 따라 현재 사용자 계정별로 유지한다.

유무 필터의 각 축 상태는 사용자 계정이 아니라 기기별로 유지한다. 필터를 닫거나 화면이 재생성되거나 앱이 백그라운드에 다녀와도 유지하며, 앱을 종료하고 다시 실행해도 유지한다. 로그인이나 로그아웃으로 현재 사용자 계정이 바뀌어도 고른 상태를 그대로 이어서 적용한다.

메모 목록의 필터 선택은 캘린더의 필터 선택과 독립이다. 유무 필터는 메모 목록에서만 제공하며 다른 화면의 메모 표시에는 적용하지 않는다.

### 완료와 삭제의 상태 전이

완료는 메모의 완료 여부를 완료 상태로 바꾸는 것이고, 삭제는 삭제 여부를 삭제 상태로 바꾸는 것이다. 두 동작 모두 메모 자체를 저장소에서 제거하지 않는다.
...
메모 목록은 기간이 없는 메모를 먼저 놓고, 그 뒤에 기간이 있는 메모를 시작 시점 오름차순으로 놓는다. 시작 시점이 같으면 종료 시점 오름차순으로 놓고, 시작 시점과 종료 시점이 모두 같으면 제목 오름차순으로 놓는다. 기간이 없는 메모끼리도 제목 오름차순으로 놓는다.

시작 시점 비교는 날짜뿐 아니라 시각까지 포함한다. 같은 날짜를 시작으로 하는 메모끼리는 이른 시각이 먼저 오고, 종일 메모는 그날 자정을 시작 시각으로 보아 같은 날짜의 다른 메모보다 먼저 온다.

시작 시점, 종료 시점, 제목이 모두 같은 메모끼리의 순서는 정하지 않는다.
...
목록에서 메모를 완료하거나 삭제하면 현재 사용자 계정과 연결된 해당 메모의 공유 상태가 저장소에 즉시 반영된다.

실행 취소를 선택하면 되돌린 상태가 같은 방식으로 즉시 반영된다.

### 목록 조회

메모 목록은 저장소에서 완료되지 않았고 삭제되지 않은 메모만 `domain`의 정렬 순서로 조회해 표시한다. 선택한 태그가 있고 태그 축이 `없음`이 아니면 그중 하나 이상과 해제되지 않은 연결을 가진 메모만 함께 조회하고, `전체`가 아닌 유무 필터 축이 있으면 그 축의 기준을 함께 만족하는 메모만 조회한다. 태그 축이 `없음`이면 `domain`의 `필터 조합 기준`에 따라 선택한 태그를 조회 기준에 넣지 않는다. 상태 변경이나 기간 변경, 태그·장소 연결 또는 필터 선택 변경이 반영되면 목록은 조회 기준과 정렬 순서에 따라 자동으로 갱신된다.
...
필터를 열면 기기에 유지된 세 축의 상태를 함께 조회해 표시한다. 유지된 상태가 없으면 세 축을 모두 `전체`로 본다.

### 필터 선택 저장

메모 목록의 필터 선택 저장은 [태그 필터 스펙](./tag-filter.md)의 `필터 선택 저장`을 따른다.

유무 필터의 축 상태도 기기에 즉시 저장한다. 사용자가 어느 축의 상태를 바꾸면 그 축만 저장하며 다른 축의 상태는 바꾸지 않는다. 이 상태는 사용자 계정과 무관하게 기기 하나에 하나만 보관하며 서버와 동기화하지 않는다.
