...
황금연휴 계산에 사용하는 공휴일은 로컬 캐시에서 판단 대상 년도별로 적용 국가의 공휴일을 조회해 합쳐 사용한다. 조회 규칙은 [공휴일 로컬 캐시 스펙](./holiday-database.md)의 연도별 캐시 조회를 따른다.

원격 동기화 결과로 대상 년도의 캐시가 바뀌거나 국가 설정이 바뀌면 사용자가 별도 조작을 하지 않아도 바뀐 공휴일 기준으로 황금연휴 목록이 다시 표시된다.
