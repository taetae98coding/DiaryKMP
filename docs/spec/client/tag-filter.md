...
선택 대상이 되는 태그의 노출과 정렬 기준은 [TagHome 목록 스펙](./tag-home.md)을, 메모와 태그의 연결 규칙은 [메모 태그 스펙](../common/memo-tag.md)을 따른다.
...
사용자는 태그를 여러 개 선택하거나 선택을 해제할 수 있다. 선택과 해제가 반영되는 시점과 필터를 열고 닫는 결과는 [목록 필터 반영 스펙](./list-filter.md)의 `필터 열기와 선택`을 따르고, 반영된 결과를 사용자가 어디서부터 확인하는지는 그 화면의 스펙이 정한다.
...
선택한 태그가 하나 이상이면 필터를 여는 동작이 필터가 적용되었음을 알린다. 선택한 태그를 세는 기준은 [목록 필터 반영 스펙](./list-filter.md)의 `필터 적용 상태`를 따른다.
...
선택 자체는 유지되어, 태그를 다시 시작하거나 다른 기기에서 받은 내용으로 태그를 다시 선택할 수 있게 되면 이전 선택이 다시 적용된다. 다만 사용자가 그 사이에 선택을 전체 해제하면 이 선택도 함께 지워지며, 그 기준은 `선택 전체 해제 기준`을 따른다.
...
태그 필터 선택은 현재 사용자 계정별로, 필터를 사용하는 화면별로 독립적으로 유지한다. 유지되는 경계, 계정이 바뀔 때의 결과와 선택이 처음 상태로 돌아가는 경우는 [목록 필터 반영 스펙](./list-filter.md)의 `필터 선택 유지`와 `필터 선택 초기화`를 따른다.

## data

### 태그 필터 조회

태그 필터는 기기에 저장된 자료에서 현재 사용자 계정의 선택할 수 있는 태그와 현재 계정에 유지된 그 화면의 필터 선택을 함께 조회해 표시한다.

태그 조회는 [TagHome 목록 스펙](./tag-home.md)의 `목록 조회`와 같은 기준으로 태그를 나누어 불러온다. 다만 필터는 태그를 한 영역에 모아 보여 주므로 사용자의 이동을 기다리지 않고 남은 태그를 이어서 불러와 선택할 수 있는 태그 전체를 채운다.

태그가 추가되거나 저장된 태그의 이모지·제목·컬러·상태가 바뀌면 필터를 연 채로도 새로고침 없이 필터의 태그 목록과 선택 여부에 바로 반영한다. 다른 기기에서 바뀐 태그는 동기화로 기기에 내려받아진 뒤 같은 기준으로 반영된다.

태그를 조회하지 못하면 별도 오류 안내 없이 태그 추가 동작만 나타난다.

선택할 수 없게 된 태그의 선택은 조회 결과에서 제외할 뿐 저장된 선택에서 제거하지 않는다. 이 선택을 저장에서 지우는 것은 `필터 선택 저장`의 전체 해제뿐이다.

### 필터 선택 저장

사용자가 태그를 선택하거나 선택을 해제하면 현재 사용자 계정의 그 화면 필터 선택을 기기에 즉시 저장한다. 저장 위치, 서버와 동기화하지 않는 것과 저장 실패의 결과는 [목록 필터 반영 스펙](./list-filter.md)의 `필터 선택 저장`을 따른다.
