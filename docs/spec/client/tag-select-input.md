...
목록이 열려 있는 동안 시스템이 앱을 메모리에서 정리한 뒤 화면을 복원하면 목록이 다시 열리고 입력하던 검색어도 다시 나타난다. 이때 목록은 좁히지 않은 대상 전체로 먼저 나타나고, 사용자가 입력을 멈춘 것과 같이 [입력 정지 대기 시간](./input-idle-delay.md)이 지나면 그 검색어로 다시 좁혀진다. 목록에 나타나는 선택 상태는 그 화면이 복원한 선택을 따른다.
