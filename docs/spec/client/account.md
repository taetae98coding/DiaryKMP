...
flowchart TD
    ...
    session -- 아니오 --> checking{"앱 시작 후 세션을 갱신하는 중이거나<br/>네트워크 문제나 서버의 일시적인 오류로<br/>갱신을 마치지 못했는가?"}
    checking -- 예 --> confirming["세션 갱신되지 않음<br/>세션 확인 중"]
    checking -- 아니오 --> stale["세션 갱신되지 않음<br/>인증되지 않은 것으로 확인됨"]
```
...
로그인 세션이 인증되지 않은 상태이면 세션이 갱신되지 않은 것으로 본다. 세션이 갱신되지 않은 사용자는 다음 둘로 나뉜다.

- 세션 확인 중: 앱이 시작되어 저장된 로그인 세션을 갱신하고 있는 동안이나, 오프라인 같은 네트워크 문제나 서버의 일시적인 오류로 세션 갱신을 마치지 못한 동안이다. 세션이 인증되지 않은 것으로 결론 난 상태가 아니며, 갱신이 끝나면 세션 갱신됨이나 인증되지 않은 것으로 확인됨으로 바뀐다.
- 인증되지 않은 것으로 확인됨: 세션 갱신이 끝났고 로그인 세션이 인증되지 않은 것으로 결론 난 상태다. 세션을 갱신하는 데 쓰는 정보가 서버에서 폐기되어 인증 제공자가 로그인 세션을 지운 경우가 여기에 해당하며, 세션 확인 중으로 보지 않는다.

세션 갱신 여부에 따라 무엇을 하거나 하지 않는지는 그 값을 쓰는 각 기능의 스펙이 정한다.
