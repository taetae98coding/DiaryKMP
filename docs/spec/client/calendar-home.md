...
이 문서는 캘린더 홈 화면의 달 표시와 이동, 오늘 강조, 공휴일 동기화와 표시, 공휴일 이름 선택, 메모 표시와 선택, 메모 태그 필터, 연락처 생일 표시와 선택, 날씨 동기화와 표시, 날씨 선택, 위치 권한 요청, 날짜 기간 선택을 통한 메모 추가 시작, 시간표 열기를 다룬다. 시간표 화면의 내용과 행동은 [CalendarTimetable 화면 스펙](./calendar-timetable.md)에서 다룬다. 캘린더에 표시된 메모를 길게 누른 뒤 드래그해 기간을 옮기는 행동은 [캘린더 메모 이동 스펙](./calendar-memo-move.md)에서 다룬다. 캘린더에 표시할 생일을 정하는 기준은 [캘린더 연락처 생일 표시 스펙](./calendar-contact-birthday.md)에서 다룬다. 사용자가 화면을 당겨 새로고침하는 행동과 동기화 진행 표시는 [새로고침 스펙](./sync-refresh.md)에서 다룬다. 태그 필터의 공통 행동과 정책은 [태그 필터 스펙](./tag-filter.md)을 따르며, 이 문서는 캘린더에 적용되는 부분만 다룬다. 위치 권한을 요청하는 시점과 기준은 [위치 권한 요청 스펙](./location-permission.md)이 소유하고, 이 문서는 캘린더 홈 화면에서 요청이 시작되는 것과 그 결과가 날씨에 반영되는 부분만 다룬다.
...
사용자는 캘린더 홈 화면에서 태그 필터를 열어 태그를 선택하거나 선택을 해제할 수 있고, 선택한 태그를 한 번에 모두 해제하거나 필터에서 바로 새 태그를 추가하러 갈 수 있다. 필터를 열고 선택하고 전체 해제하고 태그를 추가하고 닫는 행동은 [태그 필터 스펙](./tag-filter.md)의 `feature`를 따른다. 캘린더 홈 화면의 필터에는 태그 필터만 있으므로 태그 추가 동작을 막는 조건은 없다.

필터가 적용되어 있는지 알리는 기준은 [목록 필터 반영 스펙](./list-filter.md)의 `필터 적용 알림`을 따른다. 선택과 해제, 전체 해제는 같은 문서의 `필터 열기와 선택`에 따라 곧바로 캘린더의 메모 표시에 반영된다.
...
생일은 해마다 반복되므로 사용자가 다른 연도의 달로 이동해도 같은 연락처의 생일을 다시 본다. 음력 생일은 그 해의 음력에서 저장된 월·일에 해당하는 양력 날짜에 표시되며, 차지하는 날짜의 기준은 [캘린더 연락처 생일 표시 스펙](./calendar-contact-birthday.md)의 `음력 생일`을 따른다.
...
날짜별 날씨는 해당 날짜에 속한 모든 날씨의 대표 상태 아이콘을 보여 주고, 온도 표시가 있으면 온도도 함께 보여 준다. 아이콘은 시각 순서대로 한 줄에 표시하되 같은 아이콘이 연속되면 한 번만 표시하며, 그 기준은 `domain`의 날씨 아이콘 순서를 따른다. 한 번에 모두 보이지 않으면 사용자가 가로로 스크롤해 나머지를 확인할 수 있다.
...
캘린더 홈 화면에 진입하면 위치 권한 요청이 시작된다. 요청이 표시되는 조건과 요청에 대한 응답은 [위치 권한 요청 스펙](./location-permission.md)을 따른다.

사용자가 요청에서 새로 허용하면 사용자가 별도 조작을 하지 않아도 현재 위치의 날씨를 다시 동기화해 캘린더에 반영한다. 이미 허용되어 있어 요청이 표시되지 않았으면 다시 동기화하지 않는다.

사용자가 요청을 거부하면 날씨는 [현재 위치 확인 스펙](./current-location.md)을 따라 공인 IP 기준 위치로 계속 제공된다.
...
사용자가 날짜를 선택하는 도중 앱이 백그라운드로 이동하는 것처럼 시스템이 선택 조작을 중단해도 [Calendar 날짜 선택 스펙](./calendar-select.md)의 `선택 중단`에 따라 그 시점의 기간으로 선택이 완료되므로, 손을 뗐을 때와 같이 MemoAdd 화면으로 이동한다. 사용자가 앱으로 돌아오면 그 기간이 입력된 MemoAdd 화면을 본다.

선택하는 도중 화면이 회전하거나 시스템에 의해 재생성되면 [Calendar 날짜 선택 스펙](./calendar-select.md)의 `유지 경계`에 따라 진행 중이던 선택이 취소되므로 MemoAdd 화면으로 이동하지 않는다. 사용자는 선택된 날짜 없이 보던 달을 그대로 보여 주는 캘린더 홈 화면을 본다.

### 시간표 열기

사용자는 캘린더에 표시된 날짜 하나를 눌러 그 날짜의 하루 시간표를 열 수 있다. 이웃 달에 속한 날짜도 실제 날짜 그대로 열린다.

사용자는 캘린더에 표시된 한 주를 눌러 그 주의 주간 시간표를 열 수 있다. 주는 일요일부터 토요일까지이며, 두 달에 걸친 주도 그 주의 일곱 날짜가 그대로 열린다. 날짜를 누르는 자리와 주를 누르는 자리는 [CalendarHome 디자인](../../design/calendar-home.md)의 `시간표 열기`를 따른다.

메모 제목, 공휴일 이름, 생일, 날씨를 누르면 시간표를 열지 않고 각 아이템의 선택 결과가 실행된다. 길게 누르면 시간표를 열지 않고 `날짜 기간 선택으로 메모 추가 시작` 또는 [캘린더 메모 이동 스펙](./calendar-memo-move.md)의 메모 이동이 시작된다.

캘린더 홈 화면에서 연 시간표는 화면 너비와 관계없이 단독으로 표시한다. 시간표에서 뒤로 갔을 때의 결과는 [CalendarTimetable 화면 스펙](./calendar-timetable.md)의 `시간표 닫기`를 따른다.
...
공통 내비게이션에서 `캘린더` 목적지를 다시 선택하는 것도 이 이동을 요청하는 수단이며, 그 조건은 [TopLevelNavigation 스펙](./top-level-navigation.md)의 `다시 선택으로 돌아가는 자리`를 따른다.
...
오늘은 캘린더 홈 화면에 들어오거나, 다른 화면이나 다른 앱에서 캘린더 홈 화면으로 돌아올 때마다 다시 정한다. 예를 들어 자정 이전에 화면을 벗어났다가 자정 이후에 복귀하면 바뀐 오늘을 표시하고 이동 목적지도 바뀐 오늘이 속한 달이 된다.
...
화면 회전처럼 화면이 재생성되거나, 다른 화면이나 다른 앱에 다녀와도 사용자는 보던 달을 그대로 본다. 다른 앱에 다녀오는 동안 시스템이 앱을 정리했다가 다시 만들어도 보던 달을 그대로 본다. 사용자가 앱을 종료하고 다시 실행하면 보던 달은 유지하지 않고 오늘이 속한 달부터 다시 본다.

### 달 선택 진행 유지

달 선택이 열려 있는 동안 화면이 회전하거나 시스템에 의해 화면이 재생성되어도 달 선택은 열린 상태를 유지한다. 재생성 전에 선택을 확인하지 않았다면 표시 중인 달은 아직 이동하지 않은 상태다.

달 선택이 열려 있는 동안 다른 앱에 다녀와도 달 선택은 열린 상태를 유지하고, 표시 중인 달도 이동하지 않은 상태로 남는다. 다른 앱에 다녀오는 동안 시스템이 앱을 정리했다가 다시 만들어도 달 선택은 열린 채로 다시 보이고, 표시 중인 달도 이동하지 않은 상태로 남는다. 사용자가 앱을 종료하고 다시 실행하면 달 선택은 닫힌 상태로 시작하고 `보던 달 유지`에 따라 오늘이 속한 달을 본다.
...
표시 대상 연도의 공휴일은 [공휴일 국가 설정 스펙](./holiday-country.md)의 적용 국가 공휴일이다. 적용 국가가 둘 이상이면 모든 적용 국가의 공휴일을 함께 표시한다.
...
캘린더 홈 화면에서 구분은 표현에서만 이루어진다. 캘린더 홈 화면에서 두 갈래의 메모는 노출 기준, 차지하는 기간, 표시 색상, 아이템 줄 배치, 선택했을 때의 이동이 모두 같다.
...
생일 조회 기간은 표시 중인 달을 기준으로 두 달 전의 1일부터 두 달 후의 마지막 날까지다. 예를 들어 2026년 7월을 표시 중이면 2026년 5월 1일부터 2026년 9월 30일까지가 조회 기간이다. 메모 조회 기간과 같은 범위를 쓴다.

이 기간은 캘린더가 사용자에게 보여 주는 날짜 범위를 항상 포함하므로 [캘린더 연락처 생일 표시 스펙](./calendar-contact-birthday.md)의 표시 대상 기간에 드는 생일은 모두 조회 기간에 들어온다. 달을 이동하는 동안 함께 보이는 이전 달과 다음 달의 날짜도 조회 기간이 감싼다.

조회 기간에 들더라도 캘린더가 보여 주는 여섯 주에 들지 않는 생일은 화면에 나타나지 않는다.

### 음력 자료 동기화 대상

음력 생일을 양력 날짜에 놓으려면 생일 조회 기간에 드는 양력 날짜의 음력 자료가 필요하다. 생일 조회 기간의 시작일이 속한 양력 연도부터 종료일이 속한 양력 연도까지를 동기화 대상으로 정한다. 조회 기간이 연도 경계를 넘으면 두 연도가 모두 대상이다. 예를 들어 2026년 12월을 표시 중이면 조회 기간이 2026년 10월 1일부터 2027년 2월 28일까지이므로 2026년과 2027년이 대상이다.

달을 이동하면 이동한 달을 기준으로 조회 기간을 다시 정한다. 조회 기간이 연도 경계를 넘으면 넘어간 각 연도의 반복 생일을 함께 조회한다.

화면이 재생성되거나 화면을 벗어났다가 복귀하면 그 시점의 표시 중인 달을 기준으로 조회 기간을 다시 정한다. 보던 달은 `보던 달 유지`를 따르므로, 사용자는 재생성 뒤에도 같은 달의 생일을 이어서 본다.

### 생일 표시 대상

표시 대상 생일은 캘린더가 보여 주는 여섯 주를 표시 대상 기간으로 삼아 [캘린더 연락처 생일 표시 스펙](./calendar-contact-birthday.md)의 노출 기준, 반복 연도와 달력에 없는 날짜를 적용한 결과다. 조회는 `생일 조회 기간`으로 더 넓게 하며, 그중 표시 대상 기간에 드는 생일만 캘린더에 나타난다.
...
캘린더의 태그 필터 선택은 [태그 필터 스펙](./tag-filter.md)의 `필터 선택 유지`를 따라 현재 사용자 계정별로 유지하며, 메모 목록의 필터 선택과 공유하지 않는다.
...
화면이 재생성되면 같은 기준을 처음부터 다시 적용한다. 다른 화면에 다녀와 캘린더 홈 화면으로 돌아와도 화면에 새로 들어온 것으로 보아 같은 기준을 처음부터 다시 적용한다. 다른 앱에 다녀오는 것은 화면에 새로 들어온 것이 아니므로 다시 적용하지 않는다.
...
한 그룹으로 다루면 그룹 안의 줄 위치는 [CalendarWeekOfMonth 컴포넌트 스펙](./calendar-week-of-month.md)의 `아이템 줄 배치`를 따라 기간이 겹치지 않는 아이템끼리 같은 줄에 놓이므로, 그 주의 아이템이 모두 같은 한 줄을 공유한다.
...
진행 중에 사용자가 위치 권한 요청에서 새로 허용하면, 진행 중인 동기화가 끝난 뒤 위치 권한 허용 계기의 동기화를 한 번 더 시작한다. 진행 중이던 동기화는 허용 전의 위치로 날씨를 받았을 수 있으므로, 사용자는 끝난 뒤 새로 허용한 위치의 날씨를 본다. 진행 중에 허용이 여러 번 일어나도 끝난 뒤 한 번만 더 받는다.

진행 중인 동기화가 없어 시작한 동기화도 [현재 날씨 동기화 스펙](./weather-fetch.md)의 `동기화 간격`에 따라 조회 없이 끝날 수 있다.

### 날씨 표시 실패

날씨를 동기화하지 못하면 사용자에게 별도로 알리지 않는다. 이전에 표시하던 날씨가 있으면 그대로 유지하고, 없으면 날씨 없이 캘린더를 표시한다. 기기에 저장된 날씨를 읽지 못하면 날씨 없이 캘린더를 표시하고 사용자에게 별도로 알리지 않는다. 날씨 실패는 공휴일이나 메모 표시 같은 캘린더의 다른 표시를 막지 않는다.
...
캘린더 홈 화면에 들어오거나(다른 화면에서 돌아올 때 포함) 표시 중인 달이 바뀌면 공휴일 동기화 대상에 해당하는 각 연도의 최신 공휴일을 원격에서 받아 기기에 저장한다. 다른 앱에 다녀오는 것만으로는 공휴일을 다시 동기화하지 않는다. 사용자가 화면을 당겨 새로고침할 때도 같은 대상으로 동기화하며, 그 계기는 [새로고침 스펙](./sync-refresh.md)을 따른다.

각 연도의 동기화, 이미 성공한 연도의 재조회 생략, 실패 시 기기에 저장해 둔 공휴일 유지 규칙은 [공휴일 동기화 스펙](./holiday-fetch.md)을 따른다. 앱 실행 중 이미 동기화에 성공한 연도는 다시 표시되거나 다시 이동해도 원격에서 받지 않고 이미 저장된 공휴일을 그대로 표시한다.
...
캘린더에 표시하는 공휴일은 기기에 저장해 둔 공휴일에서 표시 대상 연도별로 조회한 공휴일에 노출 설정을 반영한 결과를 합쳐 사용한다. 조회 규칙은 [공휴일 노출 설정 스펙](./holiday-visibility.md)의 캘린더 공휴일 조회를 따른다.

원격 동기화 결과로 대상 연도의 저장된 공휴일이 바뀌거나 사용자가 노출 설정이나 국가 설정을 바꾸면, 사용자가 별도 조작을 하지 않아도 바뀐 공휴일을 캘린더에 반영한다.

기기 지역이 바뀐 것만으로는 캘린더의 공휴일을 다시 표시하지 않는다. 바뀐 지역이 캘린더에 보이는 시점은 [공휴일 국가 설정 스펙](./holiday-country.md)의 `적용 국가`를 따른다. 다른 주요 목적지로 옮겨 가는 것도 다른 화면에 다녀오는 것과 같아, 캘린더가 보이지 않은 지 5초가 지나기 전에 돌아오면 이전 지역 기준의 공휴일을 그대로 보고, 5초가 지난 뒤 돌아오면 바뀐 지역 기준으로 다시 본다.

대상 연도에 저장된 공휴일이 없거나 조회에 실패하면 공휴일 없이 표시하고 사용자에게 별도로 알리지 않는다. 한 연도의 조회 실패는 다른 대상 연도의 공휴일 표시를 막지 않는다.

### 현재 위치의 날씨 동기화

캘린더 홈 화면에 들어오거나, 다른 화면이나 다른 앱에서 캘린더 홈 화면으로 돌아올 때마다 현재 위치의 날씨와 지역명을 동기화한다. 사용자가 화면을 당겨 새로고침할 때도 동기화하며, 그 계기는 [새로고침 스펙](./sync-refresh.md)을 따른다. 위치 확인, 조회, 저장, 실패 시 기존 날씨 유지 규칙은 [현재 날씨 동기화 스펙](./weather-fetch.md)을 따른다. 화면 진입·복귀와 당겨서 새로고침을 계기로 하는 동기화는 그 스펙의 `동기화 간격`을 따르므로, 마지막 동기화 성공 후 1시간이 지나지 않았으면 조회 없이 끝나고 이미 표시하던 날씨가 그대로 유지된다.

사용자가 위치 권한 요청에서 새로 허용하면 현재 위치의 날씨를 다시 동기화한다. 이미 허용되어 있어 요청이 표시되지 않았으면 이 계기의 동기화를 시작하지 않는다. 이 계기의 동기화는 `동기화 간격`의 예외이므로 마지막 동기화로부터 지난 시간과 관계없이 조회한다. 진행 중인 동기화가 있으면 바로 시작하지 않고, `domain`의 날씨 중복 동기화 방지에 따라 그 동기화가 끝난 뒤 한 번 더 시작한다.
...
캘린더 홈 화면에 진입하는 것만으로는 서버에서 연락처를 다시 가져오지 않는다. 서버와 맞추는 조회는 [데이터 동기화 스펙](../common/data-sync.md)이 정한 계기에서만 일어나며, 사용자가 화면을 당겨 새로고침하면 그 동기화로 내려받은 연락처의 생일이 별도 조작 없이 캘린더에 반영된다.
...
캘린더 홈 화면에 들어오거나(다른 화면에서 돌아올 때 포함) 표시 중인 달이 바뀌면 음력 자료 동기화 대상에 해당하는 각 연도의 음력 자료를 원격에서 받아 기기에 저장한다. 당겨서 새로고침은 음력 자료를 다시 동기화하지 않는다.

각 연도의 동기화, 이미 성공한 연도의 재조회 생략, 실패 시 기기에 저장해 둔 음력 자료 유지 규칙은 [음력 자료 동기화 스펙](./lunar-fetch.md)을 따른다. 음력 자료 동기화는 진행 중인 동기화가 있어도 새로 요청하며, 이미 성공한 연도는 재조회 생략으로 원격 조회 없이 끝난다. 앱 실행 중 이미 동기화에 성공한 연도는 다시 표시되거나 다시 이동해도 원격에서 받지 않는다.

동기화 실패는 화면에 별도로 알리지 않으며 다른 대상 연도의 동기화와 캘린더의 다른 표시를 막지 않는다. 음력 자료 동기화는 [새로고침 스펙](./sync-refresh.md)의 동기화 진행 표시에 포함하지 않는다.

음력 자료가 기기에 저장되면 사용자가 별도 조작을 하지 않아도 그 자료로 놓인 음력 생일이 캘린더에 반영된다.

### 필터 선택 저장

캘린더의 필터 선택 저장은 [태그 필터 스펙](./tag-filter.md)의 `필터 선택 저장`을 따른다.
