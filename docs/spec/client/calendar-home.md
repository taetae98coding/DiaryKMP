...
화면 회전처럼 화면이 재생성되거나, 다른 화면이나 다른 앱에 다녀와도 사용자는 보던 달을 그대로 본다. 앱을 다시 실행하면 보던 달은 유지하지 않고 오늘이 속한 달부터 다시 본다.

### 달 선택 진행 유지

달 선택이 열려 있는 동안 화면이 회전하거나 시스템에 의해 화면이 재생성되어도 달 선택은 열린 상태를 유지한다. 재생성 전에 선택을 확인하지 않았다면 표시 중인 달은 아직 이동하지 않은 상태다.

달 선택이 열려 있는 동안 다른 앱에 다녀와도 달 선택은 열린 상태를 유지하고, 표시 중인 달도 이동하지 않은 상태로 남는다. 앱을 다시 실행하면 달 선택은 닫힌 상태로 시작하고 `보던 달 유지`에 따라 오늘이 속한 달을 본다.
...
사용자가 위치 권한 요청에서 새로 허용하면 현재 위치의 날씨를 다시 동기화한다. 이미 허용되어 있어 요청이 표시되지 않았으면 이 계기의 동기화를 시작하지 않는다. 이 계기의 동기화는 `동기화 간격`의 예외이므로 마지막 동기화로부터 지난 시간과 관계없이 조회한다. 다만 날씨 중복 동기화 방지는 그대로 따르므로, 진행 중인 동기화가 있으면 새 동기화를 시작하지 않는다.
