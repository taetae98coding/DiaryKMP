...
이 문서는 사용자가 `더보기`의 `연락처` 메뉴에서 진입하는 ContactHome 화면의 내용과 행동을 다룬다. `더보기` 메뉴 영역의 표시와 선택은 [MoreHome 화면 스펙](./more-home.md)에서, 세부 화면에서의 공통 내비게이션 노출 규칙은 [TopLevelNavigation 스펙](./top-level-navigation.md)에서, 연락처를 작성해 추가하는 흐름과 연락처가 갖는 정보는 [ContactAdd 화면 스펙](./contact-add.md)에서, 선택한 연락처를 확인하고 수정하고 상세에서 삭제하는 흐름은 [ContactDetail 화면 스펙](./contact-detail.md)에서, 목록 카드에서 삭제를 실행하는 상호작용의 공통 규칙은 [SwipeToFinishAndDelete 컴포넌트 스펙](./swipe-to-finish-and-delete.md)에서, 연락처 목록과 상세를 함께 사용하는 배치는 [Contact 목록·상세 배치 스펙](./contact-list-detail.md)에서, 나누어 불러오는 목록이 나타나는 순서와 아직 준비되지 않은 자리의 처리는 [페이지 조회 목록의 자리 표시 스펙](./paged-list-placeholder.md)에서, 목록이 비어 있을 때의 안내는 [목록 빈 상태 스펙](./list-empty-state.md)에서, 정렬 기준의 선택과 반영은 [목록 정렬 스펙](./list-sort.md)에서, 목록을 당겨 서버와 맞추는 조작은 [새로고침 스펙](./sync-refresh.md)에서, 연락처를 서버와 맞추는 규칙은 [데이터 동기화 스펙](../common/data-sync.md)에서 다룬다.

디자인: [ContactHome 화면 디자인](../../design/contact-home.md)

이번 범위는 `더보기`에서 ContactHome으로 이동하고, 계정에 저장된 연락처를 기기에서 불러와 즐겨찾기를 앞세운 목록으로 표시하고, 목록의 정렬 기준을 바꾸고, 목록을 당겨 서버와 다시 맞추고, ContactHome에서 연락처 추가와 연락처 상세로 이동하고, 목록에서 연락처를 삭제하고 되돌리는 것까지다. 연락처의 즐겨찾기 여부와 생일이 무엇인지는 [ContactAdd 화면 스펙](./contact-add.md)이, 즐겨찾기를 바꾸는 조작은 [ContactDetail 화면 스펙](./contact-detail.md)이 소유한다.
...
사용자는 목록의 연락처를 삭제할 수 있다. 목록 카드에서 삭제를 실행하는 상호작용은 [SwipeToFinishAndDelete 컴포넌트 스펙](./swipe-to-finish-and-delete.md)을 따르며, 연락처에는 완료 여부가 없으므로 삭제 방향만 제공한다. 삭제를 실행하기 전에 확인을 되묻지 않는다.

삭제하면 연락처가 목록에서 사라지고 삭제되었음을 안내하며, 실행 취소 동작을 제공한다.

사용자가 실행 취소를 선택하면 연락처는 삭제되지 않은 상태로 되돌아가 즐겨찾기 묶음과 목록의 정렬 기준에 맞는 위치에 다시 나타난다. 그 위치는 `domain`의 `목록 정렬`을 따른다.

아직 준비되지 않은 자리는 삭제할 수 없다.

목록과 상세를 함께 보는 동안 상세에 열려 있는 연락처를 목록에서 삭제해도 상세 영역에 놓인 화면은 바뀌지 않는다.

ContactDetail 화면에서 삭제한 연락처도 노출 기준에 따라 목록에서 사라진다.

### 안내와 실행 취소

삭제의 안내와 실행 취소는 [MemoHome 목록 스펙](./memo-home.md)의 `안내와 실행 취소` 절 전체를 메모를 연락처로 바꿔 그대로 따른다. 마지막 삭제에만 실행 취소가 적용되고, 저장되지 못한 삭제에는 안내가 보이지 않으며, 안내가 보이는 동안 화면을 떠나면 안내가 닫히고 그 삭제는 목록에서 되돌릴 수 없다.

[ContactDetail 화면 스펙](./contact-detail.md)의 `삭제`는 화면을 떠나는 동작이므로 실행 취소를 제공하지 않으며, 이 절은 그 규칙을 바꾸지 않는다.
...
목록의 삭제는 [ContactDetail 화면 스펙](./contact-detail.md)의 `domain > 삭제`와 같은 상태 전이다. 삭제는 연락처의 삭제 여부를 삭제로 바꾸며 연락처 자체를 지우지 않는다. 이름, 설명, 키, 신발 사이즈, 생일, 고향, 전화번호와 즐겨찾기 여부는 바꾸지 않는다.

실행 취소는 삭제 여부를 미삭제로 되돌린다. 실행 취소도 즐겨찾기 여부를 바꾸지 않으므로, 즐겨찾기였던 연락처는 즐겨찾기 묶음으로, 즐겨찾기가 아니었던 연락처는 즐겨찾기가 아닌 묶음으로 돌아온다.

삭제와 실행 취소는 모두 연락처의 수정 시각을 동작 시점으로 갱신한다. 따라서 최근 수정순에서는 실행 취소로 돌아온 연락처가 자기 묶음의 앞쪽에 놓인다.

삭제와 실행 취소는 메모와 연락처의 연결을 해제하지 않는다. 따라서 실행 취소한 연락처는 삭제 전의 연결을 그대로 가진 채 돌아온다. 그 기준은 [항목 연결 공통 스펙](../common/entity-link.md)의 `항목의 상태 변화와 연결`과 [메모 연락처 스펙](../common/memo-contact.md)을 따른다.

목록에서 실행하는 삭제와 실행 취소는 현재 사용자 계정과 연결된 연락처에만 적용된다.

### 화면 유지

화면이 회전하거나 다른 앱에 다녀와도, 연락처 추가·연락처 상세에서 돌아와도 사용자가 보던 목록 위치를 그대로 유지한다. 앱이 백그라운드에 있는 동안 시스템이 앱을 정리했다가 다시 만들면 목록 위치는 [페이지 조회 목록의 자리 표시 스펙](./paged-list-placeholder.md)의 `보던 자리 유지`에 따라 떠나기 전 위치로 다시 보인다. `더보기`로 나갔다가 다시 들어오거나 앱을 다시 실행하면 목록의 맨 위부터 보여 준다. 정렬 선택의 유지 기준은 [목록 정렬 스펙](./list-sort.md)의 `정렬 선택 유지`를 따른다.

## data

### 삭제와 실행 취소의 저장

목록에서 연락처를 삭제하거나 실행 취소하면 현재 사용자 계정과 연결된 그 연락처의 삭제 여부와 수정 시각이 기기에 즉시 저장된다. 연락처 자체와 계정 연결, 전화번호, 고향, 즐겨찾기 여부와 메모와의 연결은 기기에서 지우거나 바꾸지 않는다.

저장하면 그 연락처가 업로드 대기로 기록되고 화면 뒤에서 동기화가 시작되어, 같은 계정의 다른 기기에도 같은 상태로 보인다. 성공 여부는 기기에 저장한 결과로만 판단하며, 업로드 단위와 실패 시 재시도는 [데이터 동기화 스펙](../common/data-sync.md)을 따른다. 로그인하지 않은 상태에서 삭제하거나 되돌린 연락처는 서버에 반영되지 않고 기기 안에만 유지된다.

실행 취소를 저장하지 못하면, 현재 계정을 확인할 수 없는 경우를 포함해 연락처는 삭제된 채로 남아 목록에 다시 나타나지 않으며 별도의 안내는 제공하지 않는다.
