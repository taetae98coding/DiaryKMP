...
사용자가 [QrHome 화면](./qr-home.md)에서 QR 스캔 시작을 선택하면 카메라 권한이 아직 허용되어 있지 않은 경우 시스템 카메라 권한 요청을 시작한다. 요청을 사용자에게 실제로 표시할지는 시스템이 정하며, 그 기준은 [권한 요청 공통 스펙](./permission.md)의 요청 기준을 따른다.

요청에 응답한 뒤 사용자가 보는 결과는 [QrHome 화면 스펙](./qr-home.md)의 `QR 스캔 시작`이 정한다. 이는 [권한 요청 공통 스펙](./permission.md)의 권한 요청 응답에 대한 예외다.
...
카메라 권한 요청은 Android와 iOS에서 제공한다. 웹과 데스크톱 앱에서는 요청하지 않는다. 이는 [QrHome 화면 스펙](./qr-home.md)의 `QR 스캔 제공 범위`에 맞춘 것이며, 그 밖의 기준은 [권한 요청 공통 스펙](./permission.md)의 요청 제공 범위를 따른다.
