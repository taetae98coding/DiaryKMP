...
이 문서는 사용자가 검색어로 장소를 찾아 그 장소의 이름, 주소와 좌표를 입력 중인 화면으로 가져오는 공용 컴포넌트 장소 검색 다이얼로그의 동작과 정책을 다룬다. 이 컴포넌트는 [PlaceAdd 화면](./place-add.md)과 [PlaceDetail 화면](./place-detail.md)이 배치하며, 두 화면에서의 동작은 같다.

검색으로 장소를 찾아오는 조회 규칙은 [네이버 장소 검색 스펙](./naver-place-search.md)과 [Google 장소 검색 스펙](./google-place-search.md)이, 지도의 표시·조작·제공자 전환·지점 표시·핀 표시 자체의 동작은 [DiaryMap 컴포넌트 스펙](./diary-map.md)이 다룬다.

이번 범위는 검색어로 장소를 찾아 목록과 지도에서 확인하고, 그중 하나를 골라 배치한 화면의 제목, 주소와 좌표에 반영하는 것까지다. 찾은 장소를 저장하는 것은 배치한 화면이 정하며 이 문서에서는 다루지 않는다. 컴포넌트는 사용자 데이터를 만들거나 저장하지 않고 조회 계약은 위 두 검색 스펙이 소유하므로 `data` 영역은 생략한다.
...
배치한 화면이 기본 지도를 확인하지 못해 지도를 표시할 수 없는 동안에는 장소 검색을 제공하지 않는다. 기본 지도를 확인해 지도를 표시할 수 있으면, 배치한 화면의 다른 영역을 보고 있어 지도가 화면에 보이지 않는 동안에도 장소 검색을 제공한다.
...
검색하는 동안에는 직전에 받은 결과를 그대로 두어 목록이 비었다가 다시 차지 않으며, 검색이 진행 중임을 따로 알리지 않는다. 다만 검색 제공자를 바꾸면 직전 결과를 곧바로 비운 뒤 새 제공자의 결과를 기다린다.

검색어와 일치하는 장소가 없으면 결과가 없다는 안내를 제공한다. 검색에 실패하면 실패했다는 안내를 제공하며, 사용자는 검색어를 고치거나 검색 제공자를 바꿔 다시 시도할 수 있다.

### 장소 선택

사용자는 목록의 장소를 눌러 그 장소를 고를 수 있다. 지도에 표시된 결과를 눌러서도 같은 장소를 고른다.

장소를 고르면 다이얼로그가 닫히고, 고른 장소의 좌표와 주소가 배치한 화면의 위도, 경도와 주소에 채워진다. 배치한 화면의 제목이 비어 있으면 고른 장소의 이름도 함께 채운다.
...
사용자는 장소를 고르지 않고 다이얼로그를 닫을 수 있다. 이때 배치한 화면의 제목, 설명, 컬러, 주소와 좌표는 무엇도 바뀌지 않는다.

닫은 뒤 다시 열면 검색어와 결과가 없는 상태로 다시 시작한다.

### 진행 상태 유지

다이얼로그가 열려 있는 동안 화면이 회전하거나 시스템에 의해 재생성되어도 다이얼로그는 열린 상태를 유지하고, 입력 중이던 검색어와 사용자가 보고 있던 지도 위치·확대 수준·제공자도 유지한다. 결과 목록은 유지한 검색어로 기다리지 않고 곧바로 다시 검색해 채운다. 사용자가 이미 입력을 마친 검색어를 되살린 것이므로 [검색어 일치 판정 스펙](./search-match.md)의 `검색어 반영 시점`이 정한 복원 기준을 따른다.

다이얼로그가 열려 있는 동안 앱이 백그라운드에 갔다가 돌아오면 다이얼로그, 검색어, 지도와 결과 목록을 그대로 유지하고 다시 검색하지 않는다. 돌아온 순간 목록이 비었다가 다시 차지 않는다.
...
각 제공자가 한 번에 돌려주는 장소의 개수와 정렬은 [네이버 장소 검색 스펙](./naver-place-search.md)과 [Google 장소 검색 스펙](./google-place-search.md)의 `결과 개수와 정렬`, [장소 검색 공통 스펙](./place-search.md)의 `결과 개수`를 따른다.
...
배치한 화면이 지도를 표시할 수 없는 조건에서는 이어받을 제공자와 위치가 없으므로 검색 자체를 제공하지 않는다. 그 조건은 [PlaceAdd 화면 스펙](./place-add.md)의 `PlaceAdd의 지도`와 [PlaceDetail 화면 스펙](./place-detail.md)의 `초기 지도 위치`를 따른다.
...
지도가 준비되지 않아 보이는 영역을 확인할 수 없거나, 보이는 영역의 크기가 없는 등 그 안에 원을 만들 수 없으면 기준 위치를 넘기지 않고 검색한다.
...
화면을 복원해 검색어가 다시 나타나면 그 검색어로는 기다리지 않고 곧바로 검색한다. 복원한 뒤 사용자가 검색어를 고치거나 지도를 옮기면 그때부터는 위 기준대로 기다린다.
...
stateDiagram-v2
    ...
    Loading --> Loading: 검색 다시 실행
    Loading --> Idle: 검색어를 비움
    Loading --> Idle: 검색 제공자 변경
    ...
    Empty --> Idle: 검색어를 비움
    Failed --> Idle: 검색어를 비움
    Loaded --> Idle: 검색 제공자 변경
    Empty --> Idle: 검색 제공자 변경
    Failed --> Idle: 검색 제공자 변경
```

검색 중에는 직전에 받은 결과를 그대로 두고, 새 결과를 받으면 그때 목록과 지도 표시를 교체한다. 검색이 진행 중인 것은 사용자에게 알리지 않으므로 검색 중 상태와 그 직전 상태는 사용자에게 같게 보인다.

예외로 검색 제공자가 바뀌어 다시 검색할 때는 직전 결과를 곧바로 비워 결과 없음 상태로 두고, 그 상태에서 새 제공자로 검색을 실행해 새 결과를 기다린다. 한 제공자에서 받은 결과를 다른 제공자의 지도 위에 남기지 않기 위한 것이며, 그 제약은 [Google 장소 검색 스펙](./google-place-search.md)의 `결과 사용 제약`을 따른다.
