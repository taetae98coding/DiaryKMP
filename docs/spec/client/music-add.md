...
시스템이 앱을 메모리에서 정리한 뒤 화면을 복원해도 이 대상을 모두 복원한다. 이 화면은 태그 입력을 제공하지 않으므로 복원에서 제외하는 입력이 없다.
...
불러오기를 처리하는 동안 사용자가 링크를 바꿔, 결과가 도착했을 때의 링크 입력이 불러오기를 실행한 때의 링크와 다르면 그 결과를 채우지 않는다. 채우는 값은 지금 입력된 링크의 영상 정보여야 하기 때문이다. 이때 별도의 안내는 제공하지 않으며, 사용자는 바꾼 링크로 다시 불러올 수 있다. 링크를 바꿨다가 실행한 때와 같은 값으로 되돌렸으면 다르지 않은 것으로 본다.
...
flowchart TD
    ...
    Cleared -- "아니오" --> Changed{"지금 링크가 실행한 때의<br/>링크와 다른가"}
    Changed -- "예" --> Discard
    Changed -- "아니오" --> Fill["제목, 가수를<br/>영상 제목, 채널 이름으로 덮어쓴다"]
```
