...
진행 표시는 사용자가 당겨서 요청했을 때와, 앱이 사용자 계정을 확인해 동기화를 시작했을 때 나타난다. CalendarHome에서는 화면이 표시되거나 표시 중인 달이 바뀌어 공휴일과 날씨를 가져오는 동안에도 나타나며, 그 기준은 `domain > 새로고침 계기`를 따른다.
...
계정 데이터의 서버 동기화는 앱에 하나뿐이므로 그 진행 표시는 화면마다 따로 두지 않는다. 한 화면에서 당겨 시작한 동기화가 진행 중이면, 다른 새로고침 대상 화면으로 옮겨 가도 그 화면에 진행이 이어서 표시된다. 동기화가 성공했는지 실패했는지는 표시 해제 기준에 영향을 주지 않는다.

### 진행 표시 유지

표시 대상 서버 동기화가 진행 중일 때 계정 데이터를 직접 바꾸는 조작으로 새 서버 동기화가 시작되어도 진행 표시를 해제하지 않는다.

새 동기화는 진행 중이던 동기화를 취소하고 그 몫까지 이어서 처리하므로, 이어진 동기화가 끝날 때까지 진행 표시를 유지한다.

이어진 동기화가 끝난 뒤 다시 계정 데이터를 직접 바꾸는 조작만으로 시작되는 동기화는 표시 대상이 아니므로 진행을 표시하지 않는다.
