...
이 문서를 따르는 권한은 [알림 권한 요청](./notification-permission.md), [위치 권한 요청](./location-permission.md)과 [카메라 권한 요청](./camera-permission.md)이다.
...
사용자가 기능을 쓰겠다고 직접 선택해 요청이 시작되고, 그 기능이 권한 없이는 성립하지 않는 경우에는 각 권한 스펙이 이 기준의 예외를 둘 수 있다. 이때 응답 뒤의 화면 이동과 안내는 그 권한을 사용하는 기능의 스펙이 정한다.
...
사용자의 선택으로 요청을 시작하는 권한은 그 선택이 요청 지점이며, 선택할 때마다 요청 조건을 확인한다. 이 경우 아래의 다시 확인하지 않는 기준은 적용하지 않는다.
...
권한이 이미 허용되어 있는지 확인할 수 없는 환경에서는 요청하지 않고 `거부`로 처리한다. 웹에서 브라우저가 허용 여부를 알려 주지 않을 때 그 권한에 해당하는 기능을 요청해 응답으로 결과를 정하는 방법은 각 권한 스펙이 정할 수 있으며, 그 권한 스펙이 정한 방법이 이 기준보다 앞선다.
...
권한 요청은 Android, iOS와 웹에서 제공한다. 데스크톱 앱에서는 시스템이 요청을 제공하는 카메라 권한만 요청하고, 그 밖의 권한은 요청하지 않는다.
...
허용 여부를 화면에 반영하는 기능은 그 화면이 앞에 다시 나올 때마다 허용 여부를 다시 확인한다. 사용자가 앱을 떠나 시스템 설정에서 권한을 바꾸고 돌아오면, 그 화면은 다시 들어가지 않아도 바뀐 허용 여부를 반영한다. 앱 안에서 권한 요청에 응답한 경우에도 응답 직후 바뀐 허용 여부가 반영된다.
...
- [웹 브라우저 권한 확인 안내(MDN)](https://developer.mozilla.org/en-US/docs/Web/API/Permissions_API)
