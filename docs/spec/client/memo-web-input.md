...
메모와 웹 항목의 연결 규칙은 [메모 웹 스펙](../common/memo-web.md)을, 웹 항목이 갖는 정보는 [WebAdd 화면 스펙](./web-add.md)의 `웹 항목의 정보`를, 선택 대상이 되는 웹 항목의 노출 기준은 [WebHome 화면 스펙](./web-home.md)을 따른다. 웹 항목을 눌러 이동하는 웹 상세의 내용과 행동은 [WebDetail 화면 스펙](./web-detail.md)을 따르고, 웹 선택 목록을 검색어로 좁힐 때의 판정 기준은 [검색어 일치 판정 스펙](./search-match.md)을 따른다.
...
선택할 수 있는 웹 항목의 확인은 웹 입력이 있는 화면에 들어올 때 시작하며, 사용자가 웹 선택 목록을 연 적이 있는지와 관계없다. 따라서 화면에 들어온 뒤 선택할 수 있는 웹 항목이 하나도 없는 것으로 확인되었으면 추가 항목을 처음 누를 때부터 목록을 열지 않고 WebAdd 화면으로 이동한다.
...
검색어의 정규화와 웹 항목의 일치 판정, 검색어에 맞는 웹 항목이 없다고 알리는 조건은 [검색어 일치 판정 스펙](./search-match.md)을 따른다. 다만 이 목록의 일치 판정에는 웹 항목의 제목과 설명만 쓰고, 그 문서가 웹 항목의 판정 대상으로 정한 URL은 쓰지 않는다. 요청 헤더도 쓰지 않는다.
...
선택되어 있던 웹 항목이 삭제되면 그 웹 항목은 선택에서 제외된 것으로 표시한다. 이는 표시 기준이며, MemoDetail 화면에서 이미 저장된 메모와 웹 항목의 연결을 해제하지는 않는다. 저장된 연결이 유지되는 규칙은 [항목 연결 공통 스펙](../common/entity-link.md)의 `항목의 상태 변화와 연결`를 따른다.
...
웹 선택 목록은 기기에 저장된 현재 사용자 계정의 웹 항목을 `domain`의 기준과 정렬 순서로 불러와 표시한다.

목록은 한 번에 전부 불러오지 않고 사용자가 목록을 내려 보는 만큼 이어서 불러온다. 아직 확인하지 않은 구간의 웹 항목은 그 구간에 다가갈 때 불러온다.

검색어가 비어 있지 않으면 `domain`의 `웹 검색어` 기준을 함께 적용해 검색어를 만족하는 웹 항목만 불러온다. 검색어가 바뀌면 새 검색어 기준으로 목록의 처음부터 다시 불러온다.

처음 불러오기나 이어서 불러오기에 실패하면 이미 표시된 웹 항목을 유지하며 별도의 오류 안내나 재시도 동작을 제공하지 않는다.

현재 선택 상태는 선택 목록과 따로 불러오므로, 사용자가 목록을 얼마나 확인했는지와 목록에 입력한 검색어에 영향을 받지 않는다. MemoAdd 화면은 사용자가 선택한 웹 항목만 따로 불러오고, MemoDetail 화면은 [MemoDetail 화면 스펙](./memo-detail.md)의 `웹 입력의 조회`를 따른다.

### 내용 갱신

웹 항목이 추가되거나 저장된 웹 항목의 제목·상태가 바뀌면, 다른 화면에서 바꾸었든 동기화로 다른 기기의 변경이 기기에 반영되었든 사용자가 아무것도 하지 않아도 웹 입력과 열려 있는 웹 선택 목록에 바로 반영된다.

웹 입력과 웹 선택 목록에는 새로고침 동작이 없다. 서버와 맞추는 시점은 [새로고침 스펙](./sync-refresh.md)과 [데이터 동기화 스펙](../common/data-sync.md)을 따른다.
