...
인증 정보가 비어 있거나 공백만 있는 상태에서 조회를 실행하면 모델을 받아 오지 않고 인증 정보 입력이 필요하다는 피드백을 제공한다. 입력 중이던 다른 값은 그대로 유지한다.

받아 둔 모델 목록이 있으면 인증 정보가 비어 있거나 공백만 있어도 사용자는 모델 선택을 열어 받아 둔 목록에서 모델을 고를 수 있다. 받아 둔 목록을 여는 것은 조회가 아니므로 인증 정보 입력이 필요하다는 피드백은 사용자가 다시 조회를 실행할 때만 제공한다. 받아 둔 목록이 없으면 모델 선택을 여는 것이 곧 첫 조회이므로, 인증 정보가 비어 있으면 모델 선택을 열지 않고 같은 피드백을 제공한다.
...
받아 둔 모델 목록은 사용자가 다시 조회를 실행할 때만 바뀐다. 다른 앱에 다녀오거나 모델 선택을 닫았다 다시 열어도 스스로 다시 조회하지 않는다.
...
화면이 회전하거나 시스템에 의해 재생성되어도 입력 중이던 세 값을 유지한다. 받아 둔 모델 목록은 화면이 회전해도 유지되지만, 앱이 백그라운드에 있는 동안 시스템이 앱을 종료했다가 되살리면 남지 않으며 사용자가 조회를 다시 실행해 받아 온다.

모델 선택이 열려 있던 상태는 이어지지 않는다. 모델 선택을 연 채로 화면이 회전하거나 재생성되거나, 시스템이 앱을 종료했다가 되살리면 모델 선택은 닫힌 상태로 돌아오고 모델을 저절로 조회하지 않는다. 사용자는 모델 선택을 다시 열어 받아 둔 목록에서 고르거나, 받아 둔 목록이 없으면 다시 여는 것으로 조회를 실행한다.
...
저장을 처리하는 동안에도 같은 기준을 적용한다. 저장이 끝나기 전이라도 저장된 설정이 편집 중인 세 값과 같아지거나 사용자가 입력을 저장된 값으로 되돌리면 저장 동작은 사라지고, 저장 동작에 함께 보이던 진행 상태도 같이 사라진다. 저장이 끝나 저장된 설정이 편집 중인 값과 같아졌을 때도 마찬가지다.
...
stateDiagram-v2
    ...
    HasList --> HasList: 인증 정보가 빈 상태로 조회 실행<br/>(입력 필요 안내, 받아 둔 목록에서 계속 고를 수 있음)
...
편집 상태는 화면이 전환 이력에 남아 있는 동안 화면 재생성과 백그라운드 복귀 후에도 유지한다. 받아 둔 모델 목록은 화면 회전과 백그라운드 복귀 후에는 유지하지만, 시스템이 앱을 종료했다가 되살리면 버린다. 모델 선택의 열림 여부는 유지하지 않으므로 화면 재생성과 되살림 뒤에는 닫힌 상태로 시작하며, 이때 조회를 새로 실행하지 않는다.
...
Gemini 설정은 [데이터 동기화 스펙](../common/data-sync.md)의 동기화 대상이 아니다.
