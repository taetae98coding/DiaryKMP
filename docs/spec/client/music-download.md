...
다운로드를 제공하는 플랫폼은 데스크톱 앱과 Android와 iOS다. 데스크톱 앱은 기기에 설치된 도구로 직접 받고, Android와 iOS는 같은 네트워크의 데스크톱 앱이 제공하는 프록시를 통해 받는다. 웹에서 다운로드를 실행하면 아무 일도 일어나지 않으며, 그 기준은 `feature`의 `다운로드를 제공하지 않는 환경`을 따른다.
...
다운로드를 실행하면 먼저 `내려받기 준비`가 일어나고, 준비가 끝난 뒤에 대상 곡이 대기 상태가 되어 `domain`의 `진행 순서`에 따라 한 곡씩 차례로 진행한다. 이미 완료나 실패 상태인 곡은 대기로 바뀌지 않고 그 상태로 차례를 기다린다.
...
| 플랫폼 | 확인하는 것 |
| --- | --- |
| 데스크톱 앱 | 기기에 `yt-dlp`와 `ffmpeg`이 있는지 |
| Android, iOS | 프록시 주소가 저장되어 있고 그 프록시에 연결할 수 있는지 |

준비하는 동안에는 아직 어떤 곡에도 상태가 없으므로 진행 중임을 따로 알리지 않는다. 준비에 성공하면 곧바로 곡을 받기 시작하고, 사용자에게 아무것도 알리지 않는다. 준비에 실패하면 무엇이 문제인지 알리고 어떤 곡도 받지 않는다. 이때 어떤 곡에도 상태가 생기지 않는다.

#### 데스크톱 앱의 도구 준비

이 앱은 영상을 내려받고 합치는 일을 기기에 설치된 `yt-dlp`와 `ffmpeg`에 맡긴다. 두 도구는 사용자의 기기에 있어야 하며, 어떤 도구가 왜 필요한지는 `domain`의 `내려받기 도구`를 따른다.

없는 도구가 있으면 앱이 Homebrew로 그 도구를 설치한다. 설치하는 동안에는 `내려받기 준비`와 같이 진행 중임을 따로 알리지 않고, 설치가 끝나면 이어서 곡을 받는다. 설치는 사용자가 다운로드를 실행했을 때만 일어나며, 앱을 켜거나 화면에 들어오는 것만으로는 일어나지 않는다.
...
프록시 주소가 저장되어 있지 않으면 설정에서 프록시 주소를 입력해야 함을 알리고 어떤 곡도 받지 않는다. 저장된 주소의 프록시에 연결할 수 없으면 프록시에 연결할 수 없음을 알리고 어떤 곡도 받지 않는다. 사용자는 주소를 고치거나 데스크톱 앱을 실행한 뒤 다운로드를 다시 실행할 수 있다.
...
모든 곡이 완료나 실패로 끝난 뒤 다시 실행하면 대상 곡을 다시 판정한다. 이미 내려받아 둔 곡은 차례가 되면 다시 받지 않고 곧바로 완료가 되며, 파일이 없는 곡만 실제로 받는다. 이전에 실패한 곡과 다운로드를 실행한 뒤에 추가된 곡이 여기에 해당한다. 파일이 있는지는 대상을 판정할 때가 아니라 그 곡의 차례가 되었을 때 확인하므로, 파일이 있는 곡도 차례가 오기 전까지는 다른 대상 곡과 같이 대기로 보이고, 이전에 실패한 곡은 실패로 보인다.

### 다운로드를 제공하지 않는 환경

웹에서는 다운로드를 실행해도 아무 일도 일어나지 않는다. 어떤 곡의 상태도 바뀌지 않고, 내려받기를 준비하지 않으며, 파일도 남지 않는다. 브라우저에는 받은 파일을 둘 앱 전용 보관 공간이 없기 때문이다.
...
| 도구 | 하는 일 |
| --- | --- |
| `yt-dlp` | 링크가 가리키는 영상의 화면과 소리를 각각 내려받는다 |
| `ffmpeg` | 따로 받은 영상과 소리를 하나의 파일로 합친다 |

`yt-dlp`는 YouTube가 재생 방식을 바꿀 때마다 스스로 갱신되므로, 이 앱은 재생 방식을 직접 해석하지 않고 이 도구에 맡긴다.

`ffmpeg`이 필요한 것은 YouTube가 가장 좋은 화질의 화면과 소리를 따로 나누어 제공하기 때문이다. 합치지 않으면 화면과 소리가 처음부터 함께 들어 있는, 화질이 크게 떨어지는 영상만 받을 수 있다.

두 도구 중 하나라도 없으면 내려받기를 시작하지 않는다. 두 도구를 실행하는 곳은 데스크톱 앱이며, Android와 iOS는 프록시가 실행하는 도구에 맡긴다.
...
flowchart TD
    ...
    Platform -- "데스크톱 앱" --> Tool{"yt-dlp와 ffmpeg이<br/>모두 있는가"}
...
준비가 되었는지는 다운로드를 실행할 때마다 확인한다. 앞선 실행에서 준비에 성공했더라도 그 결과를 기억해 두지 않는다. 사용자가 도구를 지웠거나 데스크톱 앱을 끝냈을 수 있기 때문이다.
...
내려받기 준비에 성공한 뒤 현재 계정이나 곡 목록을 확인하지 못해 대상 곡을 판정하지 못하면, 아무것도 알리지 않고 어떤 곡도 받지 않은 채 다운로드가 끝난다. 이때 어떤 곡에도 상태가 생기지 않으며, 사용자는 다운로드를 다시 실행할 수 있다.
...
stateDiagram-v2
    ...
    Failed --> Failed: 다운로드 다시 실행
    Done --> Done: 다운로드 다시 실행
    Pending --> Done: 이미 파일이 있음
    Pending --> Running: 차례가 됨
    Failed --> Done: 다시 실행한 뒤 차례가 됨, 이미 파일이 있음
    Failed --> Running: 다시 실행한 뒤 차례가 됨
...
다운로드를 다시 실행하면 이전에 실패한 곡은 대기로 바뀌지 않고, 자기 차례가 올 때까지 실패 상태로 보인다. 차례가 되면 다른 대기 곡과 같은 기준으로 진행 중이나 완료가 된다.

진행 중인 곡의 백분율은 다음 시점부터 있다.

| 플랫폼 | 백분율이 있는 구간 |
| --- | --- |
| 데스크톱 앱 | 진행 중이 된 때부터 끝까지 |
...
데스크톱 앱은 한 곡의 화면과 소리를 차례로 따로 받으므로, 먼저 받는 화면이 0%에서 90%까지, 이어서 받는 소리가 90%에서 99%까지를 채운다. 화면이 곡 파일의 대부분을 차지하므로 이렇게 나누어도 실제로 받은 양과 크게 어긋나지 않는다. 두 가지를 하나의 파일로 합치는 동안에는 99%에 머물고, 합치기가 끝나면 백분율을 거치지 않고 완료가 된다.

### 상태의 유지 범위

다운로드 상태는 앱이 실행되어 있는 동안에만 유지한다. 화면을 떠났다 돌아오거나 목록을 새로고침해도 상태는 그대로 유지된다.

곡의 다운로드 상태는 그 상태를 얻을 때 곡의 링크가 가리키던 영상에 대한 것이다. 곡의 링크를 바꾸거나 비워 링크가 가리키는 영상이 그 영상과 달라지면 그 곡에 이전 상태를 표시하지 않는다. 링크의 형태만 바뀌고 같은 영상을 가리키면, 링크를 다시 그 영상으로 되돌린 경우를 포함해 같은 영상의 상태이므로 그대로 표시한다. 링크를 바꾼 곡은 다음에 다운로드를 실행할 때 바뀐 링크로 대상을 다시 판정하며, 진행 중이던 다운로드는 `다운로드 대상 곡`에 따라 판정한 때의 영상으로 이어진다. 이렇게 이어지는 곡은 그 곡에 상태가 표시되지 않아도 대기나 진행 중인 곡으로 남으므로, 끝날 때까지 다운로드 버튼의 진행 표시에 포함되고 `다시 실행 성립 조건`에 따라 새 실행 요청을 처리하지 않는다.

앱을 다시 실행하면 대기, 진행 중, 실패는 남지 않고, 다운로드를 다시 실행하기 전까지는 어떤 곡에도 상태가 없다. 다운로드를 실행하면 파일이 남아 있는 곡도 먼저 대기가 되고, 차례가 되면 다시 받지 않고 곧바로 완료가 된다. 완료의 근거는 그 곡의 파일이 기기에 있다는 사실이며, 별도로 기록해 둔 상태가 아니다.

### 진행 유지 범위

| 플랫폼 | PlaylistHome을 떠남 | 앱이 화면 뒤로 감 | 앱 종료 |
| --- | --- | --- | --- |
| 데스크톱 앱 | 계속 진행 | 계속 진행 | 중단 |
| Android, iOS | 계속 진행 | 진행을 보장하지 않음 | 중단 |
| 웹 | 해당 없음 | 해당 없음 | 해당 없음 |

PlaylistHome을 떠나는 것만으로는 다운로드가 멈추지 않는다. 사용자가 곡 상세나 다른 화면으로 이동했다가 돌아오면 그동안 진행된 상태를 그대로 본다.

데스크톱 앱은 앱이 창 뒤에 있거나 최소화되어 있어도 진행이 이어진다. 앱이 살아 있는 동안 진행하고, 앱이 사라지면 함께 멈춘다.
...
모든 곡이 완료나 실패로 끝났거나, 대상 곡이 하나도 없거나, 내려받기 준비에 실패해 끝났거나, 대상 곡을 판정하지 못해 끝났으면 다시 실행할 수 있다.
...
데스크톱 앱은 곡의 링크에서 얻은 영상 ID의 영상을 `yt-dlp`에 맡긴다. `yt-dlp`는 그 링크의 가장 좋은 화질의 영상과 소리를 받아 `ffmpeg`으로 합쳐 하나의 mp4 파일로 만든다. 받는 동안 `yt-dlp`가 알리는 진행률을 `domain`의 `곡의 다운로드 상태`가 정한 구간에 맞춰 곡의 진행 중 상태에 반영한다.
...
받은 파일은 앱 전용 보관 공간에 둔다. 사용자가 저장 위치를 고르지 않고, 앱을 지우면 받아 둔 파일도 함께 사라진다. 저장에 사용자 권한이 필요하지 않다.
...
파일을 영상 ID로 구분하는 것은 데스크톱 앱이 자기 곡으로 받은 파일을 다른 기기의 프록시 요청에 그대로 전달하고, 다른 기기를 대신해 받은 파일을 자기 곡의 완료 근거로도 쓰기 위한 것이다.

받는 도중 중단된 파일은 남기지 않는다. 앱 전용 보관 공간에는 끝까지 받아 합친 파일만 남으므로, 파일이 있다는 것은 그 영상을 내려받아 두었다는 뜻이다.

곡을 삭제하거나 곡의 링크를 바꾸어도 이미 받아 둔 파일은 지우지 않는다. 받아 둔 파일을 정리하는 수단은 이번 범위에서 다루지 않는다.

### 완료 판정

곡을 완료로 보는 근거는 그 곡의 영상 ID로 구분되는 파일이 앱 전용 보관 공간에 있다는 사실 하나다. 다운로드를 실행할 때마다 대상 곡마다 그 곡의 차례가 되었을 때 파일이 있는지 확인하고, 있으면 받지 않고 완료로 다룬다.

### 저장하지 않는 것

다운로드 상태와 진행률은 기기에 기록하지 않고 서버에도 반영하지 않는다. 받아 둔 파일이 있는지도 서버에 알리지 않으므로, 한 기기에서 받아 둔 곡이 다른 기기에서 완료로 보이지 않는다.
