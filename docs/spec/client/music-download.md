...
데스크톱 앱은 곡의 링크에서 얻은 영상 ID의 영상을 `yt-dlp`에 맡긴다. `yt-dlp`는 그 링크의 가장 좋은 화질의 영상과 소리를 받아 `ffmpeg`으로 합쳐 하나의 mp4 파일로 만든다. 받는 동안 `yt-dlp`가 알리는 진행률을 그대로 곡의 진행 중 상태에 반영한다.
