...
인증 제공자가 세션을 아직 준비하는 중이거나, 세션 갱신에 실패했거나, 인증되지 않았음을 알리면 인증되지 않은 상태로 본다. 인증되지 않은 상태를 세션 확인 중과 인증되지 않은 것으로 확인됨으로 나누는 기준은 [계정 조회 스펙](./account.md)의 `세션 갱신 여부`를 따른다.
