...
화면 조회의 대상은 사용자가 이동해 들어가 화면 이동 기록에 쌓이는 화면이다. 이 기록은 뒤로가기로 하나씩 되돌아가는 화면의 순서이며, 아래 `화면 이름 목록`의 화면이 모두 여기에 든다. 한 화면 안에서 열었다가 닫는 다이얼로그와 시트는 화면 이동 기록에 쌓이지 않으므로 화면 조회로 남기지 않는다. 예: 장소 검색 다이얼로그, 로그아웃 확인 다이얼로그, 날짜 선택 다이얼로그. 이런 다이얼로그와 시트를 열거나 닫아도 현재 화면은 바뀌지 않는다.

현재 화면은 사용자가 보고 있는 화면 중 가장 나중에 진입한 화면 하나다. 목록과 상세처럼 두 화면을 함께 표시하는 동안에도 화면 조회는 가장 나중에 진입한 화면 하나만 남는다.

아래 화면을 닫지 않고 그 위에 겹쳐 표시하는 화면이라도 화면 이동 기록에 쌓이면 사용자가 이동해 들어간 화면이므로 화면 조회로 남긴다. 겹쳐 표시한 화면을 닫으면 다시 드러난 아래 화면의 화면 조회가 남는다.
...
| `QrAdd` | [QrAdd 화면](./qr-add.md) |
| `QrHome` | [QrHome 화면](./qr-home.md) |
| `QrScan` | [QrScan 화면](./qr-scan.md) |
