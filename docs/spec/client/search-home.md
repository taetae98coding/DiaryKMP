...
사용자는 유형을 직접 선택해 바꿀 수 있고, 결과 영역을 좌우로 밀어서도 바로 옆 유형으로 바꿀 수 있다. 다만 결과 카드 위에서 좌우로 미는 조작은 유형 전환보다 그 카드의 완료·다시 시작·삭제가 우선한다. 카드 위에서 밀면 유형은 바뀌지 않고 `결과에서 완료와 삭제`가 정한 동작이 실행된다. 이때 유형을 바꾸려면 유형을 직접 선택하거나 카드 밖 여백에서 민다. 기준은 [TagDetail 화면 스펙](./tag-detail.md)의 `탭 전환`과 같다.
...
사용자는 검색 결과에서 항목의 진행 상태를 바꾸거나 항목을 삭제할 수 있다. 결과 카드에서 동작을 실행하는 상호작용은 [SwipeToFinishAndDelete 컴포넌트 스펙](./swipe-to-finish-and-delete.md)을 따르며, 실행하기 전에 확인을 되묻지 않는다.

| 유형 | 진행 상태를 바꾸는 동작 | 삭제 |
| --- | --- | --- |
| 메모 | 완료되지 않은 메모는 완료, 완료된 메모는 다시 시작 | 제공 |
| 태그 | 완료되지 않은 태그는 완료, 완료된 태그는 다시 시작 | 제공 |
| 장소 | 없음 | 제공 |
| 웹 | 없음 | 제공 |

완료하거나 다시 시작한 메모와 태그는 `domain`의 `결과에서 바꾼 상태`에 따라 결과에 그대로 남고, 바뀐 진행 상태에 맞는 동작을 제공한다. 삭제한 항목은 결과에서 사라진다. 어느 경우든 동작의 결과를 안내하며 실행 취소 동작을 제공한다.

사용자가 실행 취소를 선택하면 완료는 다시 시작으로, 다시 시작은 완료로, 삭제는 삭제되지 않은 상태로 되돌아간다. 되돌아간 항목은 여전히 질의를 만족하면 결과의 정렬 기준에 맞는 위치에 보인다.

아직 준비되지 않은 자리는 완료, 다시 시작, 삭제할 수 없다.

### 안내와 실행 취소

안내와 실행 취소는 [MemoHome 목록 스펙](./memo-home.md)의 `안내와 실행 취소` 절 전체를 따른다. 마지막 동작에만 실행 취소가 적용되고, 저장되지 못한 동작에는 안내가 보이지 않으며, 안내가 보이는 동안 화면을 떠나면 안내가 닫히고 그 동작은 결과에서 되돌릴 수 없다. 결과를 선택해 상세로 이동하는 것도 화면을 떠나는 것이다.

네 유형은 하나의 안내 자리를 함께 쓴다. 안내가 보이는 동안 다른 유형으로 바꾸면 그 절의 탭을 바꾼 경우와 같이 안내가 바로 닫히고, 원래 유형으로 돌아와도 다시 나타나지 않는다.

안내가 보이는 동안 질의를 고치거나 정렬을 바꾸는 것은 같은 유형의 결과를 계속 보는 것이므로 안내는 그대로 남고, 사용자는 실행 취소를 선택할 수 있다.

각 동작의 안내는 그 항목의 목록 화면과 같다. 메모의 완료와 삭제는 [MemoHome 목록 스펙](./memo-home.md)의 `완료`, `삭제`와, 메모의 다시 시작은 [MemoFinishedList 화면 스펙](./memo-finished-list.md)의 `다시 시작`과, 태그의 완료와 삭제는 [TagHome 목록 스펙](./tag-home.md)의 `완료`, `삭제`와, 태그의 다시 시작은 [TagFinishedList 화면 스펙](./tag-finished-list.md)의 `다시 시작`과, 장소의 삭제는 [PlaceHome 화면 스펙](./place-home.md)의 `삭제`와, 웹 항목의 삭제는 [WebHome 화면 스펙](./web-home.md)의 `삭제`와 같다. 각 상세 화면의 `삭제`는 화면을 떠나는 동작이므로 실행 취소를 제공하지 않으며, 이 절은 그 규칙을 바꾸지 않는다.

### 뒤로가기

사용자가 뒤로가기를 실행하면 SearchHome으로 들어온 화면으로 돌아간다. `더보기`에서 들어왔으면 `더보기`로, MemoHome·TagHome·PlaceHome·WebHome에서 들어왔으면 그 화면으로 돌아간다.

돌아간 화면은 사용자가 검색으로 떠나기 전 상태를 그대로 유지한다. 검색 결과에서 항목을 완료·다시 시작·삭제했거나 상세로 이동해 항목을 수정하거나 삭제했으면 그 결과만 반영된다.
...
질의가 바뀌면 결과를 새 질의 기준으로 다시 구성한다. 사용자가 유형을 바꾸면 그 유형에서도 현재 질의의 결과를 본다. 유형을 바꾸는 것은 질의를 고치는 것이 아니므로, 바꾼 유형에는 입력을 멈추기를 기다리지 않고 현재 질의를 곧바로 반영한다. 그 유형이 전에 다른 질의의 결과를 보여 주었더라도 그 결과를 거치지 않는다.
...
결과에서 실행하는 완료, 다시 시작, 삭제와 그 실행 취소는 그 항목의 목록 화면에서 실행한 것과 같은 상태 전이를 따른다. 기준은 다음 절이 정한다.

| 동작 | 상태 전이 기준 |
| --- | --- |
| 메모 완료와 삭제 | [MemoHome 목록 스펙](./memo-home.md)의 `domain > 완료와 삭제의 상태 전이` |
| 메모 다시 시작 | [MemoFinishedList 화면 스펙](./memo-finished-list.md)의 `domain > 다시 시작과 삭제의 상태 전이` |
| 태그 완료와 삭제 | [TagHome 목록 스펙](./tag-home.md)의 `domain > 완료와 삭제의 상태 전이` |
| 태그 다시 시작 | [TagFinishedList 화면 스펙](./tag-finished-list.md)의 `domain > 다시 시작과 삭제의 상태 전이` |
| 장소 삭제 | [PlaceHome 화면 스펙](./place-home.md)의 `domain > 삭제와 실행 취소의 상태 전이` |
| 웹 항목 삭제 | [WebHome 화면 스펙](./web-home.md)의 `domain > 삭제와 실행 취소의 상태 전이` |

완료 여부는 `검색 대상`을 가르지 않으므로, 완료하거나 다시 시작한 메모와 태그는 결과에서 빠지지 않는다. 그 항목은 정렬 기준에 맞는 자리에 바뀐 진행 상태로 남고, 다음에는 바뀐 진행 상태에 맞는 동작을 제공한다. 완료의 실행 취소로 다시 미완료가 되거나 다시 시작의 실행 취소로 다시 완료가 되어도 같다.

삭제한 항목은 `검색 대상`에서 빠져 결과에서 사라지고, 삭제의 실행 취소로 돌아온 항목은 질의를 만족하면 다시 결과가 된다.

결과에서 실행하는 동작은 현재 사용자 계정과 연결된 항목에만 적용된다.
...
시스템이 앱을 메모리에서 정리한 뒤 화면을 복원해도 입력한 질의와 보고 있는 유형을 유지한다. 복원한 질의는 사용자가 이미 입력을 마친 질의이므로 입력을 멈추기를 기다리지 않고 곧바로 결과에 반영하며, 결과는 빈 결과를 거치지 않고 처음부터 그 질의의 결과로 나타난다. 이때 각 유형에서 고른 정렬은 [목록 정렬 스펙](./list-sort.md)의 `정렬 선택 유지`에 따라 처음 정렬로 돌아간다.
...
결과에서 실행한 완료, 다시 시작, 삭제와 그 실행 취소는 그 항목의 목록 화면에서 실행한 것과 같은 방식으로 기기에 즉시 저장되고 서버에 반영된다. 메모는 [MemoHome 목록 스펙](./memo-home.md)과 [MemoFinishedList 화면 스펙](./memo-finished-list.md)의 `data > 상태 변경의 저장`을, 태그는 [TagHome 목록 스펙](./tag-home.md)과 [TagFinishedList 화면 스펙](./tag-finished-list.md)의 `data > 상태 변경의 저장`을, 장소는 [PlaceHome 화면 스펙](./place-home.md)의 `data > 삭제와 실행 취소의 저장`을, 웹 항목은 [WebHome 화면 스펙](./web-home.md)의 `data > 삭제와 실행 취소의 저장`을 따른다.

저장된 상태는 `결과 조회`에 따라 결과 목록에 바로 반영된다.
...
질의를 입력하는 동안 서버로 나가는 요청은 없다. 결과에서 상태를 바꿔 저장한 것을 서버에 반영하는 것은 `결과에서 바꾼 상태의 저장`을 따른다. 로그인하지 않은 상태에서도 사용자는 게스트 계정과 연결된 항목을 같은 기준으로 검색할 수 있다.
