...
사용자가 질의를 바꾸면 결과가 새 질의 기준으로 다시 채워지고, 사용자는 결과 목록의 처음부터 새 결과를 본다. 다른 유형으로 바꿔도 새 질의 기준의 결과를 본다. 질의를 지워 비우면 어느 유형에도 결과가 남지 않는다.
...
네 유형의 결과는 현재 사용자 계정과 질의를 기준으로 기기에 저장된 자료에서 찾는다. 찾는 기준과 정렬은 `domain`의 검색 대상, 일치 판정, 결과 정렬을 따른다.
