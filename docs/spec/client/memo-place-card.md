...
장소 선택에서 사용자가 반영한 선택이 어디에 반영되는지는 화면마다 다르므로 이 문서에서 다루지 않는다. MemoAdd 화면에서는 추가할 메모의 선택으로 반영되고, MemoDetail 화면에서는 상세 대상 메모의 저장된 연결로 반영되며, 각각 [MemoAdd 화면 스펙](./memo-add.md)과 [MemoDetail 화면 스펙](./memo-detail.md)을 따른다. 메모와 장소의 연결 규칙은 [메모 장소 스펙](../common/memo-place.md)을 따른다.
...
다음 장소를 이어서 불러오는 동안에도 사용자는 이미 나타난 장소의 선택을 바꿀 수 있다. 목록을 처음 불러오거나 다음 장소를 이어서 불러오는 데 실패하면 이미 나타난 장소를 그대로 유지하며 별도 재시도 동작은 제공하지 않는다.

목록을 닫아도 목록에서 반영한 선택은 그대로 유지된다. 목록을 닫았다가 다시 열면 목록은 정렬 순서의 앞부분부터 다시 나타난다.

장소 선택 목록에서는 장소를 눌러도 장소 상세로 이동하지 않는다. 목록에서 장소를 누르는 것은 선택과 해제이며, 장소 상세로 이동하는 조작은 `장소 상세 이동`의 지도 위 핀에만 있다.
...
선택할 수 있는 장소의 확인은 카드가 있는 화면에 들어올 때 시작하며, 사용자가 장소 선택 목록을 연 적이 있는지와 관계없다. 따라서 화면에 들어온 뒤 선택할 수 있는 장소가 하나도 없는 것으로 확인되었으면 추가 항목을 처음 누를 때부터 목록을 열지 않고 PlaceAdd 화면으로 이동한다.
...
선택되어 있던 장소가 삭제되면 그 장소는 선택에서 제외된 것으로 표시한다. 이는 표시 기준이며, MemoDetail 화면에서 이미 저장된 메모와 장소의 연결을 해제하지는 않는다. 저장된 연결이 유지되는 규칙은 [항목 연결 공통 스펙](../common/entity-link.md)의 `항목의 상태 변화와 연결`를 따른다.

### 카드의 지도

카드는 [DiaryMap 컴포넌트](./diary-map.md)를 지점 선택은 사용하지 않고 핀 표시와 핀 선택은 사용하는 상태로 배치한다.

초기 지도 제공자는 사용자가 [SettingMap 화면](./setting-map.md)에서 고른 기본 지도다. 카드가 표시되어 있는 동안 기본 지도가 바뀌면 바뀐 기본 지도로 지도를 새로 시작한다. 새로 시작한 지도는 화면에 진입할 때 정한 `초기 지도 위치`에서 시작하며, 바뀌기 전에 보고 있던 위치와 확대 수준은 이어받지 않는다.

기본 지도를 확인하기 전이거나 읽지 못했으면 지도를 표시하지 않는다. 이는 PlaceHome과 PlaceAdd가 지도를 표시하는 조건과 같다.

### 초기 지도 위치

초기 지도 위치는 카드에 노출하는 장소와 현재 위치를 모두 확인한 뒤 정하며, 둘 중 하나라도 확인하기 전에는 지도를 표시하지 않는다. 노출하는 장소가 있어 현재 위치를 쓰지 않게 되더라도 현재 위치 확인이 끝날 때까지 지도를 표시하지 않는다. 노출하는 장소를 확인하지 못하면 지도를 표시하지 않는다.
...
화면이 회전하거나 창 크기가 바뀌어도 사용자가 보고 있던 지도 위치와 확대 수준, 지도 제공자는 유지되고, 장소 선택 목록이 열려 있으면 그 목록과 검색어도 유지된다. 다른 앱에 다녀와도 같다.

시스템이 앱을 메모리에서 정리한 뒤 화면을 복원해도 지도 위치와 확대 수준, 지도 제공자는 다시 나타나고, 목록이 열려 있던 경우 목록과 입력하던 검색어도 `장소 검색어`의 기준으로 다시 나타난다. 선택이 복원되는지는 화면마다 다르다. MemoAdd 화면은 [MemoAdd 화면 스펙](./memo-add.md)의 기준에 따라 선택을 복원하지 않고, MemoDetail 화면은 저장된 연결을 다시 표시한다.
...
장소 선택 목록은 기기에 저장된 현재 사용자 계정의 장소를 `domain`의 기준과 정렬 순서로 불러와 표시한다.

목록은 한 번에 전부 불러오지 않고 사용자가 목록을 내려 보는 만큼 이어서 불러온다. 아직 확인하지 않은 구간의 장소는 그 구간에 다가갈 때 불러온다.

검색어가 비어 있지 않으면 `domain`의 `장소 검색어` 기준을 함께 적용해 검색어를 만족하는 장소만 불러온다. 검색어가 바뀌면 새 검색어 기준으로 목록의 처음부터 다시 불러온다.

처음 불러오기나 이어서 불러오기에 실패하면 이미 표시된 장소를 유지하며 별도의 오류 안내나 재시도 동작을 제공하지 않는다.

현재 선택 상태는 선택 목록과 따로 불러오므로, 사용자가 목록을 얼마나 확인했는지와 목록에 입력한 검색어에 영향을 받지 않는다. MemoAdd 화면은 사용자가 선택한 장소만 따로 불러오고, MemoDetail 화면은 [MemoDetail 화면 스펙](./memo-detail.md)의 `장소 카드의 조회`를 따른다.

### 내용 갱신

장소가 추가되거나 저장된 장소의 제목·컬러·좌표·상태가 바뀌면, 다른 화면에서 바꾸었든 동기화로 다른 기기의 변경이 기기에 반영되었든 사용자가 아무것도 하지 않아도 카드의 장소 목록과 지도 위 핀, 열려 있는 장소 선택 목록에 바로 반영된다. 이때도 지도가 보고 있는 위치와 확대 수준은 바뀌지 않는다.

카드와 장소 선택 목록에는 새로고침 동작이 없다. 서버와 맞추는 시점은 [새로고침 스펙](./sync-refresh.md)과 [데이터 동기화 스펙](../common/data-sync.md)을 따른다.
