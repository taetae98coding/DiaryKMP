...
한 주의 날짜와 아이템 표시는 [CalendarWeekOfMonth 컴포넌트 스펙](./calendar-week-of-month.md)을, 날짜 기간 선택은 [Calendar 날짜 선택 스펙](./calendar-select.md)을, 공휴일 동기화와 기기 저장 규칙은 [공휴일 동기화 스펙](./holiday-fetch.md)과 [기기에 저장한 공휴일 스펙](./holiday-database.md)을 따른다.
...
그 년도의 공휴일이 준비되었지만 지금의 연차 개수로 성립하는 황금연휴가 하나도 없으면 목록은 비어 있고, 별도의 빈 상태 안내는 표시하지 않는다. 사용자는 연차 개수를 바꾸거나 다른 년도로 이동해 다시 찾아볼 수 있다.

### 황금연휴 항목 표시

각 황금연휴 항목에는 연휴에 포함된 공휴일 이름, 시작일과 종료일, 전체 일수가 표시된다. 같은 이름의 공휴일이 여러 개 포함되면 이름은 한 번만 표시된다. 연차를 사용하는 연휴이면 사용하는 연차 일수도 함께 표시된다.
...
사용자가 고른 대안은 화면이 재생성되거나 다른 앱에 다녀와도 유지된다. 연차 개수가 바뀌어 목록을 다시 계산하면 각 항목은 첫 번째 대안으로 돌아간다. 저장된 공휴일이나 국가 설정이 바뀌어 한 항목의 기간이나 담은 공휴일이 달라져도 그 항목은 첫 번째 대안으로 돌아간다. 다시 동기화해도 기간과 담은 공휴일이 그대로인 항목은 고른 대안을 유지한다. 다른 년도로 이동했다가 돌아오는 것은 목록을 다시 계산하는 일이 아니므로 고른 대안은 유지된다. 앱을 다시 실행하거나 화면을 벗어났다가 다시 진입하면 모든 항목이 첫 번째 대안부터 보인다.
...
선택하는 도중 화면이 회전하거나 시스템에 의해 재생성되면 [Calendar 날짜 선택 스펙](./calendar-select.md)의 `유지 경계`에 따라 진행 중이던 선택이 취소되므로 MemoAdd 화면으로 이동하지 않는다. 사용자는 보던 년도를 그대로 보여 주고 선택된 날짜가 없는 HolidayHome 화면을 본다.

### 공휴일 준비

공휴일 준비는 년도가 화면에 드러날 때마다, 그리고 다른 화면이나 다른 앱에서 HolidayHome 화면으로 돌아올 때마다 다시 요청된다. 당겨서 새로고침하는 동작은 제공하지 않는다.
...
[공휴일 국가 설정 스펙](./holiday-country.md)의 적용 국가가 비어 있을 때도 같은 안내를 보여 준다. 국가 선택지를 하나도 고르지 않았거나 `기기값`만 골랐는데 기기 지역이 한국·미국이 아닌 경우가 여기에 해당하며, 동기화할 국가가 없어 모든 년도의 동기화 결과가 비어 있기 때문이다. 이때도 재시도 동작은 제공하지 않는다.
...
연차 개수는 화면 회전처럼 화면이 재생성되거나 다른 앱에 다녀와도 유지된다.
...
화면이 재생성되거나 다른 앱에 다녀와도 사용자가 보던 년도는 유지된다. 앱을 다시 실행하거나 화면을 벗어났다가 다시 진입하면 오늘이 속한 년도부터 다시 보인다.
...
사용하는 공휴일은 [공휴일 국가 설정 스펙](./holiday-country.md)의 적용 국가 공휴일을 모두 합친 것이다. 적용 국가가 둘 이상이면 어느 한 국가라도 쉬는 날인 날을 쉬는 날로 본다. 황금연휴는 실제로 쉬는 날을 찾는 기능이므로 [공휴일 노출 설정 스펙](./holiday-visibility.md)의 숨김은 적용하지 않는다.
...
년도가 화면에 드러날 때마다, 그리고 년도가 드러난 채로 다른 화면이나 다른 앱에서 돌아올 때마다 그 년도의 공휴일 동기화를 요청한다. 이미 받아온 년도를 다시 받아올지는 [공휴일 동기화 스펙](./holiday-fetch.md)의 동기화 이력이 판단하므로 화면은 요청 횟수를 제한하지 않는다. 서로 다른 년도의 동기화는 동시에 진행될 수 있다.
...
다른 앱에서 기기 지역을 바꾸고 돌아오면, 돌아올 때의 동기화는 바뀐 지역으로 정한 적용 국가를 대상으로 하고 준비 상태도 그 결과로 정한다. 목록이 어느 지역을 기준으로 보이는지는 [공휴일 국가 설정 스펙](./holiday-country.md)의 `적용 국가`에 따라 화면이 보이지 않은 시간으로 정해지므로, 5초가 지나기 전에 돌아오면 두 기준이 달라질 수 있다. 이때 사용자가 보는 결과는 다음과 같다.

| 바뀐 지역으로 한 동기화 결과 | 5초가 지나기 전에 돌아온 경우 | 5초가 지난 뒤 돌아온 경우 |
| --- | --- | --- |
| 표시 중인 년도의 공휴일이 있음 | 완료 상태이며 목록은 이전 지역 기준 그대로다 | 완료 상태이며 목록은 바뀐 지역 기준이다 |
| 표시 중인 년도의 공휴일이 없음(적용 국가가 비어 있는 경우 포함) | 제공 없음 상태 | 제공 없음 상태 |
| 판단 대상 년도 중 실패가 있음 | 오류 상태 | 오류 상태 |

오류 상태의 년도는 사용자가 재시도를 선택하거나, 년도가 다시 화면에 드러나거나, 년도가 드러난 채로 다른 화면이나 다른 앱에서 돌아오면 로딩 상태로 돌아가 다시 동기화한다.

제공 없음 상태의 년도는 재시도 동작이 없으므로, 년도가 다시 화면에 드러나거나 년도가 드러난 채로 다른 화면이나 다른 앱에서 돌아올 때 로딩 상태로 돌아가 다시 동기화한다.
...
년도가 화면에 드러나거나, 드러난 채로 다른 화면이나 다른 앱에서 돌아오거나, 사용자가 오류 상태에서 재시도를 선택하면 그 년도의 공휴일 판단 대상에 해당하는 각 년도의 최신 공휴일을 원격에서 받아 기기에 저장한다. 같은 년도가 화면에 다시 드러나도 동기화를 다시 요청한다.

각 년도의 동기화, 이미 성공한 년도의 재조회 생략, 실패 시 기기에 저장해 둔 공휴일 유지, 공휴일을 제공하지 않는 연도 규칙은 [공휴일 동기화 스펙](./holiday-fetch.md)을 따른다. 다시 요청해도 이미 성공한 판단 대상 년도는 재조회 생략에 따라 건너뛰고, 실패했던 년도만 다시 받아온다.
...
황금연휴 계산에 사용하는 공휴일은 기기에 저장된 공휴일에서 판단 대상 년도별로 적용 국가의 공휴일을 조회해 합쳐 사용한다. 조회 규칙은 [기기에 저장한 공휴일 스펙](./holiday-database.md)의 `연도별 조회`를 따른다.

원격 동기화 결과로 대상 년도의 저장된 공휴일이 바뀌거나 국가 설정이 바뀌면 사용자가 별도 조작을 하지 않아도 바뀐 공휴일 기준으로 황금연휴 목록이 다시 표시된다.
