...
OpenWeatherMap은 이 밖의 정보도 함께 알려 주지만 제품에서 사용하지 않으므로 보관하지 않는다.
...
현재 날씨와 마찬가지로, 함께 알려 오는 그 밖의 정보는 보관하지 않는다.
...
flowchart TD
    ...
    ok -- 아니오 --> none["지역명 없음"]
    ok -- 예 --> any{"결과에 지역이 하나 이상 있는가?"}
    any -- 아니오 --> none
    any -- 예 --> ko{"첫 번째 지역에<br/>한국어 이름이 있는가?"}
...
지역명 조회가 성공했지만 결과에 지역이 하나도 없으면 지역명 없음으로 다룬다. 지역명 조회 실패와 마찬가지로 날씨 동기화의 성공 여부에는 영향을 주지 않는다.
...
저장된 날씨가 없으면 캘린더에 전달할 날짜별 날씨도 없다.
...
마지막 동기화 시각은 앱이 실행 중인 동안만 보관한다. 앱을 새로 실행하면 기록이 비워져 첫 동기화 계기에 다시 조회한다. 다른 앱에 다녀온 사이 시스템이 앱을 종료해 다시 열리는 경우도 새로 실행한 것과 같다. 저장된 날씨 목록과 지역명도 함께 비워지므로 기록과 저장된 결과는 언제나 같은 실행 안에서 짝을 이룬다.
...
현재 날씨 동기화 실패는 오류 보고 대상이다. 보고 방식은 [기능 실패 로깅 스펙](./usecase-failure-logging.md)의 오류 보고를 따른다.
...
flowchart TD
    ...
    record --> notify["날씨를 보여 주는 화면에<br/>캘린더 날짜별 날씨와 지역명을 바로 반영"]
```
...
날씨 상태의 아이콘은 OpenWeatherMap이 제공하는 아이콘 이미지 가운데 낮 아이콘의 기본 크기 이미지를 쓴다. 원격이 밤 아이콘을 알려 와도 같은 상태의 낮 아이콘으로 바꿔 저장하고, 두 배 크기 이미지는 쓰지 않는다.
...
날씨 목록, 지역명과 마지막 동기화 시각은 기기에 남기지 않고 앱이 실행 중인 동안만 보관한다. 앱을 새로 실행하면 날씨 없음, 지역명 없음, 기록 없음에서 시작한다.

저장된 날씨 목록이나 지역명이 바뀌면 날씨를 보여 주는 화면에 별도 요청 없이 새 결과가 반영된다. 날씨 목록과 지역명은 한 동기화 결과로 함께 바뀌므로, 새 날씨와 이전 지역명이 섞인 조합은 보이지 않는다.
