...
웹 정보 수정과 메모 탭은 같은 자리를 나눠 쓰므로 동시에 표시하지 않는다. 사용자는 탭을 직접 선택해 웹 정보 수정과 메모 탭을 오간다. 웹 페이지를 다른 탭과 함께 표시하는지, 아니면 웹 페이지도 따로 선택하는 탭으로 두는지는 창 너비에 따라 다르며 그 배치는 [WebDetail 화면 디자인](../../design/web-detail.md)이 정한다.

화면에 처음 진입하면 사용자가 저장한 주소의 결과를 먼저 보도록 웹 페이지가 보이는 상태로 시작한다. 웹 페이지를 따로 선택하는 탭으로 두는 배치에서는 웹 페이지 탭이, 웹 페이지를 늘 함께 표시하는 배치에서는 웹 정보 수정 탭이 선택된 상태로 시작한다. 메모 탭은 처음 선택되어 있지 않다.

웹 항목 조회 중에도 사용자는 메모 탭을 선택할 수 있다. 조회 결과가 탭 선택을 막지 않는다.

탭을 전환해도 다른 탭에서 하던 일은 사라지지 않는다. 수정 중이던 제목, 설명, URL과 요청 헤더는 메모 탭에 다녀와도 그대로 남아 있고, 불러오기 상태와 이미 받은 응답 본문, 고른 표시 방식도 유지해 웹 페이지를 다시 불러오지 않으며, 메모 탭에서 보던 목록 위치도 다른 탭에 다녀오면 그대로 유지된다.

다만 탭 전환이나 배치 변화로 웹 페이지가 화면에서 빠졌다가 다시 보이면 웹 페이지를 처음부터 다시 그린다. 이때 사용자가 웹 페이지 안에서 보고 있던 위치와 문서 안에서 이동한 결과는 유지하지 않으며, URL 방식에서는 저장된 URL을 다시 열고 응답 본문 방식에서는 이미 받은 응답 본문을 다시 그린다. 웹 페이지가 화면에서 빠지는 배치는 [WebDetail 화면 디자인](../../design/web-detail.md)이 정한다.
...
수정에 실패했을 때의 결과는 [항목 상세 화면 공통 스펙](./entity-detail.md)의 `내용 수정`을 따른다.
...
저장된 URL이 바뀌면 이 화면에서 수정했든 다른 경로에서 바뀌었든 웹 표시 수단이 최신 저장 URL을 다시 연다. 이 화면의 수정은 `수정 뒤의 웹 페이지`를, 다른 경로의 변경은 `외부 변경`을 따른다.
...
응답 본문 방식을 한 번이라도 연 뒤에는 수정에 성공하면 곧바로 최신 저장 URL과 요청 헤더로 웹 페이지를 다시 불러온다. 사용자가 다시 시도를 따로 실행하지 않아도 바뀐 내용의 결과를 그 자리에서 확인한다. 다시 불러오는 기준은 `불러오기 시점`을 따른다.

최신 저장 URL과 요청 헤더가 마지막으로 불러온 URL과 요청 헤더와 같으면 수정에 성공해도 다시 불러오지 않고, 보고 있던 결과나 안내를 그대로 둔다.
...
화면이 회전하거나 시스템에 의해 재생성되어도 고른 표시 방식을 유지하고, 불러오기 상태와 이미 받은 응답 본문도 유지하며 웹 페이지를 다시 불러오지 않는다. 다만 웹 페이지는 `메모 탭`에서 웹 페이지가 화면에서 빠졌다가 다시 보일 때와 같이 처음부터 다시 그리므로, 사용자가 웹 페이지 안에서 보고 있던 위치와 문서 안에서 이동한 결과는 유지하지 않는다. URL 방식에서는 저장된 URL을 다시 열고 응답 본문 방식에서는 이미 받은 응답 본문을 다시 그린다. 화면의 재생성은 로그인 정보를 가져오는 계기가 아니며, 이미 알린 가져오기 실패를 다시 알리지 않는다.
...
사용자가 뒤로가면 선택한 탭과 관계없이 WebDetail 화면에 진입하기 전 화면으로 돌아간다. 메모 탭을 선택한 상태에서 뒤로간다고 웹 정보 수정으로 먼저 돌아가지 않는다. WebHome에서 들어왔으면 WebHome으로, SearchHome에서 들어왔으면 SearchHome으로, TagDetail 웹 탭에서 들어왔으면 웹 탭이 선택된 같은 태그의 TagDetail 화면으로, MemoAdd 화면이나 MemoDetail 화면의 웹 입력에서 들어왔으면 그 화면으로 돌아간다. 반영하지 않은 수정 내용은 저장되지 않으며, 이미 반영된 태그 연결과 해제는 그대로 남는다.
...
stateDiagram-v2
    ...
    Address --> Fetching: 응답 본문 방식으로 바꿈, 받은 응답 본문이 없거나 다시 불러오는 중
    ...
    Page --> Fetching: 수정 성공 뒤 최신 URL이나 요청 헤더가 마지막으로 불러온 것과 다름
    Failure --> Address: URL 방식으로 바꿈
    Failure --> Fetching: 다시 시도, 또는 수정 성공 뒤 최신 URL이나 요청 헤더가 마지막으로 불러온 것과 다름
```
...
응답 본문 표시 상태에서는 다시 시도를 제공하지 않는다. 이 상태에서 다시 불러오는 계기는 사용자가 실행한 수정이 성공한 뒤 최신 저장 URL이나 요청 헤더가 마지막으로 불러온 것과 다른 경우뿐이다.
...
화면 재생성과 백그라운드 복귀 후에도 웹 정보 수정, 웹 페이지, 메모 중 선택한 탭을 유지한다.

사용자가 화면을 떠났다가 다시 진입하거나 앱을 종료하고 다시 실행하면 `메모 탭`이 정한 처음 선택으로 초기화한다.
...
URL 방식에서는 저장된 URL이 다른 경로에서 바뀌면 사용자가 아무것도 하지 않아도 웹 표시 수단이 새로 저장된 URL을 곧바로 다시 연다. 이때 사용자가 웹 페이지 안에서 이동해 있던 위치와 입력하던 내용은 유지하지 않는다. 다른 경로에서 요청 헤더만 바뀌었으면 URL 방식의 표시는 바뀌지 않는다. Android, iOS, 데스크톱, 웹 모두 같다.

응답 본문 방식에서 이미 표시하고 있던 웹 페이지는 다른 경로에서 저장 내용이 바뀌어도 다시 불러오지 않는다. 사용자가 보고 있던 응답 본문을 배경 변경이 치우지 않게 하기 위한 것이다. 다른 경로에서 바뀐 URL과 요청 헤더는 응답 본문 방식으로 처음 바꿀 때, 다시 시도를 실행할 때, 이 화면에서 실행한 수정이 성공할 때 최신 저장 내용으로 불러오면서 반영된다. 이때 사용자가 이 화면에서 제목이나 설명만 고쳐 수정해도, 최신 저장 URL이나 요청 헤더가 마지막으로 불러온 것과 다르면 다시 불러온다.

응답 본문 방식에서 다시 불러오는 계기는 사용자가 이 화면에서 실행한 수정, 다시 시도와 표시 방식 변경뿐이며, 이는 사용자가 스스로 일으킨 변화의 결과만 그 자리에서 보여 주기 위한 것이다.
...
서버가 다른 주소로 옮겨 가라고 응답하면 옮겨 간 주소까지 따라가 마지막에 받은 응답을 결과로 삼는다. 응답을 받은 주소는 이렇게 따라간 마지막 주소이며, 옮겨 가지 않았으면 저장된 URL과 같다.

### 불러오기 결과 판정

응답을 받았으면 서버가 응답에 성공을 알렸는지 오류를 알렸는지와 관계없이 불러오기 성공으로 다룬다. 요청한 주소가 없거나 서버가 오류를 알리는 응답을 보냈어도 그 응답 본문을 그대로 표시하며, 앱이 따로 오류 안내로 바꾸지 않는다. 사용자가 어떤 요청 헤더로 무엇이 돌아오는지 확인하는 것이 이 화면의 목적이기 때문이다.
...
응답 본문 안의 상대 경로는 응답을 받은 주소를 기준 주소로 삼아 해석한다. 응답 본문을 표시하는 동안 문서가 이미지나 글꼴 같은 부속 파일을 불러오는 요청과 사용자가 문서 안에서 이동한 결과의 요청에는 대상 웹 항목의 요청 헤더를 붙이지 않는다. 요청 헤더는 사용자가 상세에서 확인하는 요청 한 번에만 적용한다.
...
웹 페이지는 응답 본문 방식이 처음 되었을 때 한 번 불러온다. 이때는 그 시점의 최신 저장 URL과 요청 헤더로 불러오므로, 화면에 들어온 뒤 다른 경로나 수정으로 바뀐 내용도 반영된다. 응답 본문 방식으로 한 번도 바꾸지 않고 URL 방식에 머무르는 동안에는 앱이 웹 페이지를 불러오지 않는다. URL 방식에서 웹 표시 수단이 주소를 다시 여는 것은 이 불러오기에 해당하지 않으며, 그 시점은 `URL 방식의 표시`를 따른다.

그 뒤에는 사용자가 다시 시도를 실행할 때와 사용자가 실행한 수정이 성공했을 때만 불러오며, 두 경우 모두 그 시점의 최신 저장 URL과 요청 헤더로 불러온다. 수정에 성공하면 곧바로 최신 저장 URL과 요청 헤더를 확인해 마지막으로 불러온 것과 다르면 다시 불러온다. 지금 URL 방식에 있어도 곧바로 다시 불러오며, 응답 본문 방식으로 돌아오면 다시 불러온 결과를 보여 준다.

수정에 성공한 뒤의 최신 저장 URL과 요청 헤더가 마지막으로 불러온 것과 모두 같으면 다시 불러오지 않는다. 수정이 성립하지 않아 아무 내용도 바뀌지 않았을 때와 수정에 실패했을 때도 다시 불러오지 않는다.
...
화면 재생성과 백그라운드 복귀 뒤에도 입력 중이던 제목, 설명, URL과 요청 헤더를 유지하고, 고른 표시 방식과 받은 응답 본문과 화면 상태도 유지하며 웹 페이지를 다시 불러오지 않는다. 화면 재생성 뒤 웹 페이지를 처음부터 다시 그리는 기준은 `feature > 진행 상태 유지`를 따른다. 선택한 탭의 유지 기준은 `선택한 탭`을 따른다.
...
상세에 표시할 웹 항목은 현재 사용자 계정과 사용자가 고른 웹 항목을 기준으로 기기에 저장된 웹 항목에서 찾는다. 이 조회는 삭제 여부를 가리지 않으므로 삭제 상태인 웹 항목도 조회된다. 요청 헤더는 웹 항목에 딸린 순서 있는 목록으로 함께 조회한다.

WebDetail에 진입하는 것만으로는 서버에서 웹 항목을 다시 가져오지 않는다. 서버와 맞추는 조회는 [데이터 동기화 스펙](../common/data-sync.md)이 정한 계기에서만 일어난다.

### 수정의 저장

수정을 반영하면 대상 웹 항목의 제목, 설명, URL, 요청 헤더와 수정 시각을 기기에 저장해 기존 값을 덮어쓴다. 계정과의 연결은 바꾸지 않는다.

요청 헤더는 저장된 목록을 반영한 내용으로 바꾸며, 반영한 순서 그대로 저장한다. 헤더를 모두 지운 수정을 반영하면 그 웹 항목의 요청 헤더는 하나도 남지 않는다.

수정은 현재 사용자 계정에 연결된 대상 웹 항목에만 반영한다. 그 웹 항목이 없으면 아무것도 바꾸지 않는다.

### 웹 페이지 요청

응답 본문 방식에서 웹 페이지는 저장된 URL과 요청 헤더로 앱이 그 주소에 직접 요청해 받는다. 이 요청은 웹 항목을 조회하는 것과 별개이며, 앱의 계정이나 로그인 세션 정보를 함께 보내지 않는다.

URL 방식에서는 앱이 그 주소에 요청하지 않는다. 저장된 URL을 여는 요청은 웹 표시 수단이 만들며, 앱은 그 요청에 요청 헤더나 계정 정보를 붙이지 않는다.
...
삭제를 실행하면 대상 웹 항목의 삭제 여부와 수정 시각을 기기에 저장한다. 웹 항목 자체와 계정 연결, 요청 헤더는 기기에서 지우지 않는다.

삭제는 현재 사용자 계정에 연결된 대상 웹 항목에만 반영한다. 그 웹 항목이 없으면 아무것도 바꾸지 않는다.

### 저장 실패

수정이나 삭제에 실패한 결과는 [항목 상세 화면 공통 스펙](./entity-detail.md)의 `저장 실패`를 따른다. 수정에 실패하면 저장된 웹 항목의 내용은 바뀌지 않고, 삭제에 실패하면 대상 웹 항목은 삭제되지 않은 채로 남는다.
...
수정과 삭제를 기기에 반영하면 그 웹 항목은 서버에 올릴 변경으로 표시되고, 사용자가 기다리지 않아도 되도록 화면 뒤에서 동기화가 시작된다. 삭제한 웹 항목도 서버에서 제거하지 않고 삭제 상태로 반영한다.

수정과 삭제의 성공 여부는 기기에 저장한 결과로만 판단하며 서버 반영 결과를 기다리지 않는다. 동기화에 실패해도 기기에 반영된 수정과 삭제는 되돌아가지 않고 다음 동기화 계기에서 다시 시도된다. 업로드 단위와 충돌 기준, 실패 시 재시도는 [데이터 동기화 스펙](../common/data-sync.md)의 규칙을 따른다.

같은 웹 항목을 여러 기기에서 다르게 바꾼 경우 어느 내용을 최신으로 볼지는 [데이터 동기화 스펙](../common/data-sync.md)의 `충돌 기준`을 따른다. 내려받기로 서버의 더 늦은 수정 내용이 반영되면 그 결과는 `결과의 표시`와 같은 기준으로 나타나며, 화면에서 입력 중인 내용은 `외부 변경`에 따라 덮어쓰지 않는다.
