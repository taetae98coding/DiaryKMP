...
사용자는 언제든 도우미를 닫을 수 있다. 생성하는 동안 닫으면 취소한 것과 같이 요청을 멈추고 결과를 받지 않는다.
...
도우미는 메모의 컬러, 태그 연결, 대표 태그, 웹 연결, 장소 연결을 생성하지 않고 바꾸지도 않는다.
...
- 저장된 시스템 프롬프트. 비어 있으면 보내지 않는다.
- 사용자가 적은 프롬프트. 비어 있으면 보내지 않는다.
- 지금 입력 중인 제목, 설명, 기간. 저장된 메모의 값이 아니라 사용자가 화면에서 보고 있는 값이다.
- 요청 시점의 날짜와 시각, 그리고 기기의 시간대. 사용자가 `내일`이나 `이번 주말`처럼 적은 표현을 지금을 기준으로 해석할 수 있어야 하기 때문이다.

메모의 컬러, 태그, 웹, 장소와 식별자는 보내지 않는다.
...
생성을 실행하면 저장된 인증 정보와 모델로 Gemini에 생성을 요청한다. 보내는 내용은 `domain`의 `생성에 보내는 내용`을 따른다.
...
생성은 메모를 만들거나 바꾸지 않으므로 [데이터 동기화 스펙](./data-sync.md)의 동기화 대상이 아니다. 사용자가 반영한 뒤 메모를 추가하거나 수정해야 저장과 동기화가 일어난다.

## 참고

- [Gemini API generateContent](https://ai.google.dev/api/generate-content)
- [Gemini API 구조화된 출력](https://ai.google.dev/gemini-api/docs/structured-output)
