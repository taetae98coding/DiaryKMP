...
삭제한 태그가 영구히 제거되면 그 태그를 대표 태그로 가진 메모의 대표 태그 지정은 해제된다. 영구히 제거된 태그는 되살릴 수 없어 지정을 유지할 대상이 없기 때문이다. 메모 자체는 제거되지 않고 남으며, 제목·설명·컬러·기간과 완료 여부, 삭제 여부도 바뀌지 않는다. 영구 제거의 기준은 [데이터 동기화 스펙](./data-sync.md)의 `삭제된 항목의 보존과 영구 제거`를 따른다.
...
대표 태그로 지정된 태그가 서버에서 이미 영구히 제거되었으면 그 업로드는 반영하지 않는 대상이 아니다. 서버는 대표 태그 지정이 해제된 것으로 보고 메모의 나머지 내용을 그대로 반영한다. 기기가 아직 해제를 내려받지 못한 동안에도 그 메모의 업로드가 막히지 않게 하기 위해서다.
