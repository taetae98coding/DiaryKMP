...
앱 밖의 지도를 열지 못하는 환경에서는 화면을 그대로 유지하고 사용자에게 별도로 알리지 않는다.
...
여는 대상은 각 제공자가 제공하는 웹 지도의 위치이며, 기기에 설치된 그 제공자의 지도 앱을 직접 지정하지 않는다. 사용자의 환경이 그 위치를 설치된 지도 앱으로 대신 열어 주는 것은 허용하고, 어느 앱이나 브라우저로 열리는지는 각 환경의 정책을 따른다.
...
지도를 여는 것은 앱 밖에서 일어나므로 앱은 그 결과를 확인하지 않는다. 지도가 실제로 열렸는지 확인하지 않고, 열지 못한 경우에도 다시 시도하지 않는다.
