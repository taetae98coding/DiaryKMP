...
알림에는 오늘의 메모가 몇 개인지와 각 메모의 제목이 담긴다. 어떤 메모가 오늘의 메모인지는 `domain > 오늘의 메모`가, 알림 내용을 언제 정하는지는 `domain > 알림 내용을 정하는 시점`이 정한다. iOS에서는 앱을 마지막으로 실행한 뒤에 바뀐 메모가 알림에 반영되지 않을 수 있으며, 그 기준도 `domain > 알림 내용을 정하는 시점`이 정한다.
...
알림 내용을 정하는 시점은 알림이 발생하는 시점에 앱 코드가 실행되는지에 따라 다르다. 어떤 환경이 어디에 해당하는지는 `알림 예약이 유지되는 범위`를 따른다.

Android, JVM 데스크톱, 웹에서는 알림에 담는 오늘의 메모를 알림이 발생하는 시점에 정한다. 알림이 발생하는 날은 그 시점의 기기 현지 날짜다. 알림을 예약한 뒤 발생하기 전까지 메모가 추가·수정·완료·삭제되거나 현재 사용자 계정이 바뀌면, 알림에는 발생 시점의 결과가 담긴다.

iOS에서는 알림이 발생하는 시점에 앱 코드가 실행되지 않으므로, 알림 내용을 앱이 실행되는 동안 미리 정해 둔다. 앱이 실행되는 동안 기기에 저장된 메모나 현재 사용자 계정이 바뀌면 그 시점의 기기 데이터로 앞으로 발생할 알림의 내용을 다시 정한다. 알림에는 마지막으로 정한 내용이 담기므로, 앱을 마지막으로 실행한 뒤에 다른 기기에서 바뀐 메모는 다음에 앱을 실행하기 전까지 알림에 반영되지 않는다. 미리 정하는 알림의 범위는 `알림 예약이 유지되는 범위`를 따른다. 여기서 앱이 실행되는 동안은 사용자가 앱 화면을 열어 둔 동안을 뜻하며, [데이터 동기화 스펙](./data-sync.md)의 주기 동기화처럼 시스템이 화면 없이 앱을 깨운 경우는 포함하지 않는다.

각 알림이 발생하는 날의 오늘의 메모를 정하는 기준은 어느 환경에서나 `오늘의 메모`와 같다. iOS에서 앞으로 발생할 알림의 내용을 정할 때도 그 알림이 발생하는 날을 기준으로 겹침을 판단한다.

오늘의 메모를 가져오지 못하면 알림을 보내지 않는 대신, 오늘의 메모를 확인하라는 안내만 담고 개수와 제목 목록을 두지 않은 알림을 보낸다. 사용자에게 실패를 따로 알리지 않는다. iOS에서는 내용을 다시 정하는 시점에 가져오지 못하면 앞으로 발생할 알림을 모두 이 안내로 정한다.

```mermaid
flowchart TD
    Start["알림 내용을 정하는 시점"] --> Runnable{"발생 시점에 앱 코드가<br/>실행되는 환경인가"}
    Runnable -- "예 (Android, JVM 데스크톱, 웹)" --> AtFire["발생 시점의<br/>기기 데이터로 정함"]
    Runnable -- "아니오 (iOS)" --> Ahead["앱 실행 중 메모가 바뀔 때마다<br/>그 시점의 기기 데이터로 미리 정함"]
...
사용자가 기기의 시간대나 시각을 바꾸면 그 뒤에 정해지는 알림부터 바뀐 기준의 오전 8시를 따른다. 이미 기다리고 있는 알림은 정해질 때의 기준을 그대로 따르므로, 기준을 바꾼 뒤 첫 알림은 바뀌기 전 기준의 시각에 올 수 있다. iOS에서 미리 정해 둔 알림도 같은 규칙을 따르며, 앱이 실행되는 동안 알림 내용을 다시 정할 때 바뀐 기준으로 옮겨진다.
...
- Android에서는 앱을 종료하거나 기기를 다시 시작해도 예약이 유지되어 매일 오전 8시에 알림이 발생한다.
- iOS에서는 앱을 종료하거나 기기를 다시 시작해도 예약이 유지되지만, 앱이 실행되는 동안 마지막으로 알림 내용을 정한 시점부터 앞으로 7일의 알림만 예약된다. 7일이 지나도록 앱을 실행하지 않으면 그 뒤의 알림은 다음에 앱을 실행하기 전까지 발생하지 않는다.
- JVM 데스크톱과 웹에서는 앱이 실행 중일 때만 알림이 발생한다.

Android, JVM 데스크톱, 웹에서는 알림이 발생하는 시점에 앱 코드가 실행되어 오늘의 메모를 정한다. iOS에서는 알림을 미리 등록해 두고 시스템이 정해진 시각에 표시하므로 발생 시점에 앱 코드가 실행되지 않으며, 알림 내용은 `알림 내용을 정하는 시점`의 iOS 규칙을 따른다.

iOS에서 앞으로 7일은 알림 내용을 정하는 시점을 기준으로 다음에 오는 오전 8시가 첫째 날이다. 정하는 시점이 오전 8시 이전이면 그 날부터, 오전 8시 이후면 다음 날부터 7일이다. 앱을 실행하는 동안 알림 내용을 다시 정하면 그 시점 기준의 7일로 범위가 옮겨진다.
...
앱을 다시 실행하면 예약을 다시 확인한다. 이미 같은 예약이 있으면 예약을 하나로 유지하고, 발생 시각은 앞당겨지지 않는다. iOS에서 앱을 실행하거나 메모가 바뀌어 알림 내용을 다시 정하는 것은 같은 날의 알림을 새 내용으로 대신하는 것이며, 같은 날에 알림이 두 번 발생하게 하거나 발생 시각을 바꾸지 않는다.
...
iOS에서는 앞으로 7일의 알림 내용을 미리 정하므로, 각 날짜를 알림이 발생하는 날로 삼아 같은 기준으로 날짜마다 조회한다. 기기에 저장된 메모가 바뀌면 조회 결과도 따라 바뀌어 알림 내용을 다시 정하는 계기가 된다.
