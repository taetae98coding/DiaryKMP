...
사용자가 질의를 바꾸면 결과가 새 질의 기준으로 다시 채워진다. 다른 유형으로 바꿔도 새 질의 기준의 결과를 본다. 질의를 지워 비우면 어느 유형에도 결과가 남지 않는다.
...
검색 대상은 현재 사용자 계정과 연결되어 있고 삭제되지 않은 메모, 태그, 장소, 웹 항목이다. 계정 조회 규칙은 [계정 조회 스펙](./account.md)을 따르며, 현재 계정을 확인할 수 없으면 어느 유형에도 결과가 없다.
...
flowchart TD
    ...
    Query -- 아니오 --> Account{"현재 사용자 계정과<br/>연결되어 있는가"}
    Account -- 아니오 --> Hide
...
화면을 떠난 뒤 다시 들어오면 질의는 비어 있고 보고 있는 유형은 그때 들어온 경로의 시작 유형으로 되돌아간다. 질의와 유형 선택은 저장소에 남기지 않으므로, 앱을 다시 실행해도 마지막에 검색한 질의는 남지 않는다.
...
네 유형 모두 결과에서 항목을 선택하면 그 항목이 상세 대상이 되며, 상세 대상은 항목의 식별자로만 넘긴다. 질의와 보고 있던 유형은 상세로 넘기지 않는다.
...
네 유형의 결과는 현재 사용자 계정과 질의를 기준으로 로컬 저장소에서 조회한다. 조회 기준과 정렬은 `domain`의 검색 대상, 일치 판정, 결과 정렬을 따른다.

각 유형의 결과는 유형마다 따로 페이지 단위로 조회하며, 조회를 시작하는 시점은 `domain`의 결과 갱신을 따른다. 페이지를 이어서 조회하는 기준은 [페이지 조회 목록의 자리 표시 스펙](./paged-list-placeholder.md)의 `페이지 조회`를 따른다. 한 유형을 얼마나 확인했는지는 다른 유형의 조회 범위에 영향을 주지 않는다.

저장된 메모, 태그, 장소, 웹 항목이 바뀌면 조회 결과도 그에 맞게 갱신되어 결과 목록에 반영된다.

### 서버 요청

검색은 로컬 저장소만 조회하며 서버에서 항목을 다시 가져오지 않는다. SearchHome은 당겨서 새로고침을 제공하지 않으며, 서버와 맞추는 조회는 [데이터 동기화 스펙](./data-sync.md)의 기준으로만 일어난다.
