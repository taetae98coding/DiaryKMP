...
고른 태그가 표시 기준에서 벗어나면 태그 입력은 그 태그를 제외된 것으로 표시한다. 이는 태그 입력의 표시 기준이며, 이미 저장된 연결을 해제하지는 않는다. 저장된 연결이 유지되는 규칙은 [항목 연결 공통 스펙](./entity-link.md)의 `항목의 상태 변화와 연결`을 따른다.
...
선택할 수 있는 태그의 정렬은 [TagHome 목록 스펙](./tag-home.md)의 정렬 기준을 따른다.
...
검색어는 목록이 열려 있는 동안 유지한다. 목록을 닫으면 사라지며, 화면이 회전하거나 시스템에 의해 화면이 재생성되어 목록이 열린 채로 유지되면 검색어와 그 결과도 유지한다.

### 태그 선택 목록의 열림 상태

태그 선택 목록의 열림 상태는 화면이 회전하거나 시스템에 의해 재생성되어도 유지된다.
...
태그 선택 목록은 저장소에서 현재 사용자 계정의 태그를 `domain`의 `태그 선택 목록에 나타나는 태그` 기준과 정렬 순서로 조회해 표시한다.

이 조회는 정렬 순서대로 페이지 단위로 이루어지며, 사용자가 목록의 끝에 다가가면 다음 페이지를 이어서 조회한다.

검색어가 비어 있지 않으면 `domain`의 `태그 검색어` 기준을 조회 조건에 함께 적용해 검색어를 만족하는 태그만 조회한다. 검색어가 바뀌면 새 검색어 기준으로 첫 페이지부터 다시 조회한다.

목록에 나타나는 태그의 기준이 화면마다 다르므로 조회 조건도 화면마다 다르다. 추가 화면은 선택할 수 있는 태그만 조회하고, 상세 화면은 각 화면 스펙의 조회 기준을 따른다.

표시 대상 태그는 선택 목록의 조회와 분리해 조회하며, 이 조회는 페이지로 나누지 않고 검색어도 적용하지 않는다. 추가 화면은 사용자가 고른 태그의 식별자로 `표시 대상 태그` 기준을 만족하는 태그만 조회하고, 상세 화면은 각 화면 스펙의 조회 기준을 따른다.

태그가 추가되거나 저장된 태그의 이모지·제목·컬러·상태가 바뀌면 조회 결과가 갱신되어 표시 대상 태그와 선택 목록에 반영된다. 선택 목록은 사용자가 이미 확인한 범위를 포함해 갱신된다.
