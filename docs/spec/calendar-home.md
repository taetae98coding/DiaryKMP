...
캘린더 홈 화면이 표시되거나 표시 중인 달이 바뀌면 음력 자료 동기화 대상에 해당하는 각 연도의 음력 자료를 원격에서 받아 로컬 캐시에 저장한다.

각 연도의 동기화, 이미 성공한 연도의 재조회 생략, 실패 시 기존 캐시 유지 규칙은 [음력 자료 동기화 스펙](lunar-fetch.md)을 따른다. 앱 실행 중 이미 동기화에 성공한 연도는 다시 표시되거나 다시 이동해도 원격에서 받지 않는다.

동기화 실패는 화면에 별도로 알리지 않으며 다른 대상 연도의 동기화와 캘린더의 다른 표시를 막지 않는다. 음력 자료 동기화는 [새로고침 스펙](sync-refresh.md)의 동기화 진행 표시에 포함하지 않는다.

음력 자료가 로컬 캐시에 저장되면 사용자가 별도 조작을 하지 않아도 그 자료로 놓인 음력 생일이 캘린더에 반영된다.
